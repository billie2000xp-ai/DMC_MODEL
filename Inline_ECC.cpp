...
SvZMzBtMrQ+w1lxbG5+EFWux51V2bvtZUTAAA+en/pM7ZB5Cy3u0JTs1VqxXwgfw9ciNVBlCfyZ3
fs/j4XIqQYUL7eSLkZsZJy9yx4rQF49zMT4YUFZu6EvlaDjo9YzFasSEcyeZjpvyU07RLnZ/k4M4
LJeaouahIW+aXUuf# */
...
Inline_ECC::Inline_ECC(unsigned index, MemorySystemTop *top, unsigned id, ofstream &DDRSim_log_)
    : id_(index), memorySystemTop_(top), channel(id), DDRSim_log(DDRSim_log_)
{
...
uint32_t Inline_ECC::get_avail_rd_ecc_buf_id()
{
    ...
    for (std::map<uint32_t, bool>::iterator it = index_recycle_rd_ecc_buf.begin(); it != index_recycle_rd_ecc_buf.end();
         ++it) {
        if (it->second) {
    ...
    if (this->avail_rd_ecc_buf_id != 0xffff) {
        index_recycle_rd_ecc_buf.erase(this->avail_rd_ecc_buf_id);
        ;  // 9/11
    }
...
uint32_t Inline_ECC::get_avail_wr_ecc_buf_id()
{
    ...
    if (!ECC_MERGE_ENABLE) {
        for (std::map<uint32_t, bool>::iterator it = index_recycle_wr_ecc_buf.begin();
             it != index_recycle_wr_ecc_buf.end();
             ++it) {
            if (it->second) {
            ...
            if (it->second) {
                if (wr_ecc_buf[it->first].wr_ecc_pos == 0xffff) {
                    for (std::map<uint32_t, bool>::iterator it2 = index_recycle_wr_ecc_buf.begin();
                         it2 != index_recycle_wr_ecc_buf.end();
                         ++it2) {
                        if (it2->second) {
                            if ((wr_ecc_buf[it->first].pdu_addr - wr_ecc_buf[it2->first].pdu_addr ==
                                        0xffffffffffffffff ||
    ...
    if (this->avail_wr_ecc_buf_id != 0xffff) {
        index_recycle_wr_ecc_buf.erase(this->avail_wr_ecc_buf_id);
        ;  // 10.18
    }
        ...
        for (uint32_t i = 0; i < rd_ecc_buf.at(index).task_list.size(); i++) {
            ...
            if (it != pfq_->getPTC(chl)->tasks_info.end()) {
                if (it->second.rd_finish) {
                    ...
                    rd_ecc_buf.at(index).task_list.erase(rd_ecc_buf.at(index).task_list.begin() + i);
                }
            }
        }
        ...
        for (uint32_t i = 0; i < wr_ecc_buf.at(index).task_list.size(); i++) {
            ...
            if (it != pfq_->getPTC(chl)->tasks_info.end()) {
                if (it->second.wr_finish) {
                    ...
                    wr_ecc_buf.at(index).task_list.erase(wr_ecc_buf.at(index).task_list.begin() + i);
                }
            }
        }
...
bool Inline_ECC::ecc_try_add_rd_trans(Transaction *trans, uint32_t rd_ecc_buf_id)
{
    ...
    if (ret) {
        ...
        if (DEBUG_BUS) {
            PRINTN(setw(10) << now() << " -- IECC SCH :: READ_CMD task=" << trans->task
                            << " type=" << trans->transactionType << " mask_write=" << trans->mask_wcmd
                            << " ecc_flag=" << trans->ecc_flag << " qos=" << trans->qos
                            << " burst_length=" << trans->burst_length << " channel=" << trans->channel
                            << " data_ready_cnt=" << trans->data_ready_cnt << " address=" << hex << trans->address
                            << dec << " rank=" << trans->rank << " bank=" << trans->bankIndex << " row=" << trans->row
                            << " col=" << trans->col << " addr_col=" << trans->addr_col << endl);
        }
    }
    return ret;
}
...
bool Inline_ECC::ecc_try_add_wr_trans(Transaction *trans, uint32_t wr_ecc_buf_id)
{
    ...
    if (ret) {
        ...
        wr_ecc_buf.at(wr_ecc_buf_id).wr_ecc_pos |= (uint32_t(pow(2, n_bit)) - 1)
                                                   << (trans->col / 16) % 16;  // update 24/9/4
        if (DEBUG_BUS) {
            PRINTN(setw(10) << now() << " -- IECC SCH :: WRITE_CMD task=" << trans->task
                            << " type=" << trans->transactionType << " mask_write=" << trans->mask_wcmd
                            << " ecc_flag=" << trans->ecc_flag << " qos=" << trans->qos
                            << " burst_length=" << trans->burst_length << " channel=" << trans->channel
                            << " data_ready_cnt=" << trans->data_ready_cnt << " address=" << hex << trans->address
...
bool Inline_ECC::try_add_ecc_rd(Transaction *trans, uint32_t rd_ecc_buf_id)
{
    ...
    if (ret) {
        ...
        ecc_task++;
        if (DEBUG_BUS) {
            PRINTN(setw(10) << now() << " -- IECC SCH :: ECC_RD_CMD task=" << trans_rd_ecc->task
                            << " type=" << trans_rd_ecc->transactionType << " mask_write=" << trans_rd_ecc->mask_wcmd
                            << " ecc_flag=" << trans_rd_ecc->ecc_flag << " qos=" << trans_rd_ecc->qos
                            << " burst_length=" << trans_rd_ecc->burst_length << " channel=" << trans_rd_ecc->channel
                            << " data_ready_cnt=" << trans_rd_ecc->data_ready_cnt << " address=" << hex
                            << trans_rd_ecc->address << dec << " rank=" << trans_rd_ecc->rank
                            << " bank=" << trans_rd_ecc->bankIndex << " row=" << trans_rd_ecc->row
                            << " col=" << trans_rd_ecc->col << " addr_col=" << trans_rd_ecc->addr_col << endl);
        }
    } else {
        delete trans_rd_ecc;
    }
...
bool Inline_ECC::try_add_ecc_wr(Transaction *trans, uint32_t wr_ecc_buf_id)
{
    ...
    if (ret) {
        ...
        ecc_merge_flag = false;
        if (DEBUG_BUS) {
            PRINTN(setw(10) << now() << " -- IECC SCH :: ECC_WRITE_CMD task=" << trans_wr_ecc->task
                            << " type=" << trans_wr_ecc->transactionType << " mask_write=" << trans_wr_ecc->mask_wcmd
                            << " ecc_flag=" << trans_wr_ecc->ecc_flag << " qos=" << trans_wr_ecc->qos
                            << " burst_length=" << trans_wr_ecc->burst_length << " channel=" << trans_wr_ecc->channel
                            << " data_ready_cnt=" << trans_wr_ecc->data_ready_cnt << " address=" << hex
                            << trans_wr_ecc->address << dec << " rank=" << trans_wr_ecc->rank
                            << " bank=" << trans_wr_ecc->bankIndex << " row=" << trans_wr_ecc->row
                            << " col=" << trans_wr_ecc->col << " addr_col=" << trans_wr_ecc->addr_col << endl);
        }
    } else {
        delete trans_wr_ecc;
    }
                ...
                if (ecc_try_add_rd_trans(trans_cmd, avail_rd_ecc_buf_id)) {
                    ...
                    */
                    if (ecc_model_state == TRY_HIT_ECC_BUF) {
                        if (DEBUG_BUS) {
                            PRINTN(setw(10)
                                   << now() << " -- ECC_ADD_TRANS :: address=0x" << hex << trans->address
        ...
        case ECC_RD_HIT_DIRTY: {
            ...
            if (iecc_add_cnt == 0) {
                ...
                if (ret0) {
                    ...
                    if (DEBUG_BUS) {
                        PRINTN(setw(10) << now() << " -- ECC_RD_HIT_DIRTY :: address=0x" << hex << trans->address
                                        << ", task=" << dec << trans->task << ", nxt_state=WAIT_ECC_DATA_FINISH"
                                        << endl);
                    }
                } else {
                    if (DEBUG_BUS) {
...
bool Inline_ECC::addTransaction(Transaction *trans)
{
    if (IECC_ENABLE && (!IECC_PARTIAL_BYPASS || trans->address < IECC_BYPASS_ADDRESS) && !trans->pre_act) {
        ...
        if (WRITE_BUFFER_ENABLE && pfq_) {
            ...
            if (pfq_->getPTC(trans->channel)->full())
                return false;
        }
        if (full())
            return false;
...
void Inline_ECC::addr_exp(Transaction *trans)
{
    ...
    if (DEBUG_BUS) {
        PRINTN(setw(10) << now() << " -- ADD_EXP :: task=" << trans->task << ", rank=" << trans->rank << ", row=" << row
                        << ", bank=" << bank << ", col=" << col << ", addr_col=" << trans->addr_col_ini
                        << ", exp_row=" << trans->row << ", exp_bank=" << trans->bankIndex << ", exp_col=" << trans->col
                        << ", exp_addr_col=" << trans->addr_col << endl);
    }
}
...
void Inline_ECC::addr_ecc_map(Transaction *trans)
{
    ...
    if (DEBUG_BUS) {
        PRINTN(setw(10) << now() << " -- ADD_ECC_MAP :: task=" << trans->task << ", rank=" << trans->rank
                        << ", row=" << trans->row << ", bank=" << trans->bankIndex << ", col=" << col << ", addr_col="
                        << trans->addr_col_ini << ", exp_row=" << trans->row << ", exp_bank=" << trans->bankIndex
...
void Inline_ECC::show_buf_state()
{
    ...
    for (auto &bufs : rd_ecc_buf) {
        PRINTN("BUF time: " << now() << " | buffer=" << bufs.buf_id << " | pdu_addr=0x" << hex << bufs.pdu_addr << dec
                            ...
                            << " | wr_ecc=" << bufs.wr_ecc << " | slt_cnt=" << bufs.slt_cnt << " | vld=" << bufs.vld
                            << " | pri=" << bufs.ecc_pri << endl);
    }
    ...
    for (auto &bufs : wr_ecc_buf) {
        PRINTN("BUF time: " << now() << " | buffer=" << bufs.buf_id << " | pdu_addr=0x" << hex << bufs.pdu_addr << dec
                            << " | wr_ecc_pos=" << hex << bufs.wr_ecc_pos << dec << " | slt_cnt=" << bufs.slt_cnt
                            << " | vld=" << bufs.vld << endl);
    }
    PRINTN(
        "--------------------------------------------------------------------------------------------------" << endl);
    for (std::map<uint32_t, bool>::iterator it = index_recycle_wr_ecc_buf.begin(); it != index_recycle_wr_ecc_buf.end();
         ++it) {
