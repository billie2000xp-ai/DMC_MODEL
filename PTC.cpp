...
SvZMzBtMrQ+w1lxbG5+EFWux51V2bvtZUTAAA+en/pM7ZB5Cy3u0JTs1VqxXwvF+ip6xfXk6/dZ6
ATG0+e3T26erHdOTI09j+6yJwr6rrkCTLwDgN+WtWvEw1UgVjQK4qgfKq1F1D99Px153NlQ9f/4B
MrAqUSPLcZFJ6a6M# */
...
PTC::PTC(unsigned index, MemorySystemTop *top, ofstream &DDRSim_log_, ofstream &trace_log_, ofstream &cmdnum_log_)
    : id_(index), memorySystemTop_(top), DDRSim_log(DDRSim_log_), trace_log(trace_log_), cmdnum_log(cmdnum_log_)
{
    ...
    ch = index;
    ch = memorySystemTop_->hhaId * NUM_CHANS + index;
    PostCalcTiming();
    // bus related fields
    cmdCyclesLeft = 0;
    ...
    if (PD_ENABLE) {
        for (size_t i = 0; i < NUM_RANKS; i++)
            RankState[i].lp_state = IDLE;
    }
    if (ASREF_ENABLE) {
        for (size_t i = 0; i < NUM_RANKS; i++)
            RankState[i].asref_cnt = ASREF_PRD;
    }
    total_latency = 0;
    dly_ex2000_cnt = 0;
    ...
    lat_dly_step[34] = 800;
    lat_dly_step[35] = 900;
    lat_dly_step[36] = 1000;
    lat_dly_step[37] = 1500;
    lat_dly_step[38] = 2000;
    ...
    for (size_t i = 0; i < TRANS_QUEUE_DEPTH; i++) {
        for (size_t j = 0; j < TRANS_QUEUE_DEPTH; j++) {
            if (i < j)
                ptc_lru_matrix[i][j] = 1;
            else
                ptc_lru_matrix[i][j] = 0;
        }
    }
    ...
    for (size_t i = 0; i < (NUM_BANKS * NUM_RANKS); i++) {
        for (size_t j = 0; j < (NUM_BANKS * NUM_RANKS); j++) {
            if (i < j)
                bank_lru_matrix[i][j] = 1;
            else
                bank_lru_matrix[i][j] = 0;
        }
    }
    ...
    page_timeout_window[0]  = 1;
    page_timeout_window[1]  = 5;
    page_timeout_window[2]  = 10;
    page_timeout_window[3]  = 20;
    page_timeout_window[4]  = 30;
    page_timeout_window[5]  = 40;
    page_timeout_window[6]  = 50;
    page_timeout_window[7]  = 60;
    page_timeout_window[8]  = 70;
    page_timeout_window[9]  = 80;
    page_timeout_window[10] = 90;
    page_timeout_window[11] = 100;
    page_timeout_window[12] = 110;
    page_timeout_window[13] = 120;
    page_timeout_window[14] = 130;
    page_timeout_window[15] = 140;
    page_timeout_window[16] = 150;
    page_timeout_window[17] = 160;
    page_timeout_window[18] = 170;
    page_timeout_window[19] = 180;
    page_timeout_window[20] = 190;
    page_timeout_window[21] = 200;
    page_timeout_window[22] = 210;
    page_timeout_window[23] = 220;
    page_timeout_window[24] = 230;
    page_timeout_window[25] = 240;
    page_timeout_window[26] = 250;
    page_timeout_window[27] = 260;
    page_timeout_window[28] = 270;
    page_timeout_window[29] = 280;
    page_timeout_window[30] = 290;
    page_timeout_window[31] = 300;
    page_timeout_window[32] = 400;
    ...
    for (size_t i = 0; i < NUM_RANKS; i++) {
        ...
        if (AREF_OFFSET_EN)
            ref_offset.push_back((tREFI / NUM_RANKS) * i);
        else
            ref_offset.push_back(0);
        for (size_t j = 0; j < NUM_BANKS; j++) {
...
void PTC::receive_rdata(unsigned long long task, bool mask_wcmd)
{
    ...
    if (DEBUG_BUS) {
        PRINT_SPD_PTC(ch, "{:10} -- R DDR :: Receiving From Data Bus, task={}", now(), task);
    }
}

void PTC::Cmd2Dfi_statistics(
    uint64_t task, uint64_t timeAdded, unsigned qos, unsigned mid, unsigned pf_type, unsigned rank)
{
    uint32_t delay = (now() + 1 - timeAdded);

    if (float(delay) * tDFI < 50) {
        qos_level_cnt[qos][0]++;
    } else if (float(delay) * tDFI < 100) {
        qos_level_cnt[qos][1]++;
    } else if (float(delay) * tDFI < 150) {
        qos_level_cnt[qos][2]++;
    } else if (float(delay) * tDFI < 200) {
        qos_level_cnt[qos][3]++;
    } else if (float(delay) * tDFI < 300) {
        qos_level_cnt[qos][4]++;
    } else if (float(delay) * tDFI < 500) {
        qos_level_cnt[qos][5]++;
    } else if (float(delay) * tDFI < 1000) {
        qos_level_cnt[qos][6]++;
    } else if (float(delay) * tDFI < 2000) {
        qos_level_cnt[qos][7]++;
    } else {
        qos_level_cnt[qos][8]++;
    }
...
void PTC::ReturnData_statistics(
    uint64_t task, uint64_t timeAdded, unsigned qos, unsigned mid, unsigned pf_type, unsigned rank)
{
    uint32_t delay = (now() - timeAdded);
    if (DEBUG_BUS) {
        PRINT_SPD_PTC(ch, "{:10} -- OVER :: Read DELAY ({}) task={}", now(), delay, task);
    }

    // if (PRINT_RDATA) {
    //     TRACE_PRINT(setw(10)<<now()<<" -- Rdata task="<<task<<", latency="<<delay);
    // }
    if (DEBUG_BUS) {
        if (delay > 2500) {
            dly_ex2000_cnt++;
            PRINT_SPD_PTC(ch, "{:16} -- Read DMC[{}], num [{}]", now(), delay, dly_ex2000_cnt);
        }
    }
    com_read_cnt++;
    total_latency += delay;
...
void PTC::receive_wdata(unsigned int *data, uint64_t task)
{
    ...
    if (DEBUG_BUS) {
        PRINT_SPD_PTC(ch, "{:10} -- R_CPU :: Get Data from CPU, task={}", now(), task);
    }
}
    ...
    if (IS_LP5) {
        if (cmd_bl == BL32) {
            ...
            if (timing < ceil(float(BL32) / 2 / WCK2DFI_RATIO)) {
                ...
                return timing - 4; // diff bl/n between bl16 and bl32
            }
        } else {
        ...
        if (cmd_bl == BL48) {
            ...
            if (timing < ceil(float(BL48) / 2 / WCK2DFI_RATIO)) {
                ...
                return timing - 6;// diff bl/n between bl24 and bl48
            }
        } else {
            ERROR(setw(10) << now() << " -- DMC[" << channel << "] No such BLEN: " << cmd_bl);
            assert(0);
        ...
        if (is_trtp) {
            if (IS_LP4 && cmd_bl == BL32) {
                ...
                if (cmd_bl == BL32)
                    ...
                    return timing - (8 / unsigned(WCK2DFI_RATIO));
            }
        } else {
...
unsigned PTC::CalcTccd(bool is_samebg, unsigned cmd_bl, unsigned tccd)
{
    if (IS_LP5) {
        if (cmd_bl == BL32) {
            ...
            if (is_samebg) {
                if (NUM_GROUPS > 1)
                    return (BL16 / unsigned(WCK2DFI_RATIO));
                else
                    return (BL16 / 2 / unsigned(WCK2DFI_RATIO));
            } else {
                return tccd;
            }
        } else {
        ...
        if (cmd_bl == BL48) {
            ...
            if (is_samebg) {
                if (NUM_GROUPS > 1)
                    ...
                    return (BL24 / 2 / unsigned(WCK2DFI_RATIO));
            } else {
                return tccd;
            }
        } else {
    ...
    if (IS_LP4) {
        if (is_sameba) {
            if (cmd_bl == BL16)
                ...
                return tCCDMW + unsigned(float(BL16) / 2 / WCK2DFI_RATIO);
        } else {
        ...
        if (NUM_GROUPS > 1) {  // BG mode
            if (is_sameba) {
                ...
                if (cmd_bl == BL16 || cmd_bl == BL24)
                    ...
                    return unsigned(float(cmd_bl) / 4 / WCK2DFI_RATIO);
            }
        } else {  // Bank mode
            if (is_sameba) {
                if (cmd_bl == BL16 || cmd_bl == BL24)
                    return unsigned(4 * float(cmd_bl) / 2 / WCK2DFI_RATIO);
                else
                    return unsigned(2.5 * float(cmd_bl) / 2 / WCK2DFI_RATIO);
            } else {
                return unsigned(float(cmd_bl) / 2 / WCK2DFI_RATIO);
            }
        }
...
unsigned PTC::CalcMwrite2Write(bool is_samebg, bool is_sameba, unsigned cmd_bl)
{
    if (IS_LP4) {
        ...
        if (NUM_GROUPS > 1) {  // BG mode
            if (is_samebg)
                return unsigned(2 * float(cmd_bl) / 2 / WCK2DFI_RATIO);
            else
                return unsigned(float(cmd_bl) / 2 / WCK2DFI_RATIO);
        } else {  // Bank mode
            return unsigned(float(cmd_bl) / 2 / WCK2DFI_RATIO);
...
{
    for (size_t rank = 0; rank < NUM_RANKS; rank++) {
        if (RankState[rank].lp_state == PD)
            continue;
        if (RankState[rank].lp_state == PDLP)
            continue;
        if (RankState[rank].lp_state == ASREF)
            continue;
        if (RankState[rank].lp_state == SRPD)
            continue;
        if (RankState[rank].lp_state == SRPDLP)
            continue;
...
void PTC::update_wdata()
{
    ...
    for (auto &t : WtransQueue) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- MATCH :: data ready cnt={}, burst_length={}, data_size={}, task={}",
                now(),
                t->data_ready_cnt,
                t->burst_length,
                t->data_size,
...
void PTC::update_lp_state()
{
    ...
    if (PhyLpState.lp_cnt > 0) {
        PhyLpState.lp_cnt--;
        if (PhyLpState.lp_cnt == 0)
            phylp_timing_met = true;
    }

    for (size_t rank = 0; rank < NUM_RANKS; rank++) {
        if (deqCmdWakeupLp[rank].front() > 0)
            rank_has_cmd[rank] = true;
        else
            rank_has_cmd[rank] = false;
    }
    ...
    switch (PhyLpState.phylp_state) {
        case PHYLP_IDLE: {
            if (is_allrank_phylp) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- PHY_LP :: cur_state=PHYLP_IDLE, next_state=PHYLPE", now());
                }
            }
            phy_notlp_cnt++;
            break;
        ...
        case PHYLPE: {
            if (phylp_wakeup) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- PHY_LP :: cur_state=PHYLPE, next_state=PHYLPX", now());
                }
            } else if (phylp_timing_met) {
                PhyLpState.phylp_state = PHYLP;
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- PHY_LP :: cur_state=PHYLPE, next_state=PHYLP", now());
                }
            }
            phy_notlp_cnt++;
            break;
        ...
        case PHYLP: {
            if (phylp_wakeup) {
                ...
                PhyLpState.phylp_state = PHYLPX;
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- PHY_LP :: cur_state=PHYLP, next_state=PHYLPX", now());
                }
            }
            phy_lp_cnt++;
            break;
        ...
        case PHYLPX: {
            if (phylp_timing_met) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- PHY_LP :: cur_state=PHYLPX, next_state=PHYLP_IDLE", now());
                }
            }
            phy_notlp_cnt++;
            break;
    ...
    for (size_t rank = 0; rank < NUM_RANKS; rank++) {
        if (has_rank_blocking && blocking_rank != rank)
            continue;
        if (RankState[rank].pd_cnt > 0)
            RankState[rank].pd_cnt--;
        if (RankState[rank].asref_cnt > 0)
            RankState[rank].asref_cnt--;
        if (RankState[rank].state_cnt > 0)
            RankState[rank].state_cnt--;
    }
    ...
    for (size_t rank = 0; rank < NUM_RANKS; rank++) {
        ...
        if (RankState[rank].pd_cnt == 0 && now() >= funcState[rank].nextPde)
            pd_timing_met = PD_ENABLE && even_cycle;  // every other command, even;
        if (RankState[rank].asref_cnt == 0) {
            bool all_bank_idle = true;
            for (size_t bank = 0; bank < NUM_BANKS; bank++) {
                unsigned sub_channel = bank / sc_bank_num;
                if (combo_emode_unable && sub_channel == 1)
                    continue;  // rank1, sc1 forbidden under combo e-mode
                if (bankStates[rank * NUM_BANKS + bank].state->currentBankState == Idle)
                    continue;
                all_bank_idle = false;
                break;
            }
            if (all_bank_idle)
                asref_timing_met = ASREF_ENABLE && even_cycle;  // every other command, even;
        }
        if (RankState[rank].state_cnt == 0) {
        ...
        switch (RankState[rank].lp_state) {
            case IDLE: {
                if (rank_has_cmd[rank] || fast_wakeup[rank] || has_fastwakeup) {
                    ...
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=IDLE, next_state=ASREFE", now(), rank);
                    }
                } else if (pd_timing_met && PD_ENABLE && !funcState[rank].wakeup && refresh_all) {
                    RankState[rank].pd_cnt = tPDE;
                    RankState[rank].lp_state = PDE;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=IDLE, next_state=PDE", now(), rank);
                    }
                }
                WakeUpTime[rank]++;
                break;
            ...
            case PDE: {
                if (pd_timing_met) {
                    ...
                    PdEnterCnt[rank]++;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=PDE, next_state=PD", now(), rank);
                    }
                }
                WakeUpTime[rank]++;
                break;
            ...
            case PD: {
                if ((rank_has_cmd[rank] || fast_wakeup[rank] || funcState[rank].wakeup) && state_timing_met) {
                    ...
                    packet.push_back(p);
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=PD, next_state=PDX", now(), rank);
                    }
                } else if (pd_timing_met) {
                    RankState[rank].lp_state = PDLP;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=PD, next_state=PDLP", now(), rank);
                    }
                }
                PdTime[rank]++;
                break;
            ...
            case PDLP: {
                if ((rank_has_cmd[rank] || fast_wakeup[rank] || asref_timing_met || funcState[rank].wakeup) &&
                    state_timing_met) {
                    RankState[rank].pd_cnt = tXP;
                    RankState[rank].lp_state = PDX;
                    phy p;
                    ...
                    packet.push_back(p);
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=PDLP, next_state=PDX", now(), rank);
                    }
                }
                PdTime[rank]++;
                break;
            ...
            case PDX: {
                if (pd_timing_met) {
                    ...
                    PdExitCnt[rank]++;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=PDX, next_state=IDLE", now(), rank);
                    }
                }
                WakeUpTime[rank]++;
                break;
            ...
            case ASREFE: {
                if (asref_timing_met) {
                    ...
                    AsrefEnterCnt[rank]++;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=ASREFE, next_state=ASREF", now(), rank);
                    }
                }
                WakeUpTime[rank]++;
                break;
            ...
            case ASREF: {
                if (rank_has_cmd[rank] || fast_wakeup[rank] || funcState[rank].wakeup) {
                    ...
                    packet.push_back(p);
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=ASREF, next_state=ASREFX", now(), rank);
                    }
                } else if (pd_timing_met) {
                    if (DMC_V590) {
                        RankState[rank].pd_cnt = tPDLP;
                        ...
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, 
                                "{:10} -- LP_STATE :: rank={}, cur_state=SRPDE, next_state=SRPD", now(), rank);
                        }
                    } else {
                        RankState[rank].pd_cnt = tPDE;
                        ...
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, 
                                "{:10} -- LP_STATE :: rank={}, cur_state=ASREF, next_state=SRPDE", now(), rank);
                        }
                    }
                }
                AsrefTime[rank]++;
            ...
            case ASREFX: {
                if (asref_timing_met) {
                    ...
                    AsrefExitCnt[rank]++;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=ASREFX, next_state=IDLE", now(), rank);
                    }
                }
                WakeUpTime[rank]++;
                break;
            ...
            case SRPDE: {
                if (pd_timing_met) {
                    ...
                    PdEnterCnt[rank]++;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=SRPDE, next_state=SRPD", now(), rank);
                    }
                }
                AsrefTime[rank]++;
                break;
            ...
            case SRPD: {
                if ((rank_has_cmd[rank] || fast_wakeup[rank] || funcState[rank].wakeup) && state_timing_met) {
                    ...
                    packet.push_back(p);
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=SRPD, next_state=SRPDX", now(), rank);
                    }
                } else if (pd_timing_met) {
                    RankState[rank].lp_state = SRPDLP;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=SRPD, next_state=SRPDLP", now(), rank);
                    }
                }
                SrpdTime[rank]++;
                break;
            ...
            case SRPDLP: {
                if ((rank_has_cmd[rank] || fast_wakeup[rank] || funcState[rank].wakeup) && state_timing_met) {
                    ...
                    packet.push_back(p);
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=SRPDLP, next_state=SRPDX", now(), rank);
                    }
                }
                SrpdTime[rank]++;
                break;
            ...
            case SRPDX: {
                if (pd_timing_met) {
                    ...
                    packet.push_back(p);
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LP_STATE :: rank={}, cur_state=SRPDX, next_state=ASREFX", now(), rank);
                    }
                }
                AsrefTime[rank]++;
                break;
...
void PTC::dfi_pipe()
{
    ...
    if (!packet.empty()) {
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- packet_size={}", now(), packet.size());
        }
            ...
            if (packet[i - erase_cnt].delay == 0) {
                ...
                if (PRINT_IDLE_LAT) {
                    if (command.type == READ_CMD) {
                        DEBUG(setw(10) << now() << " -- EXEC(DRAM) :: send [READ_CMD] to DFI task=" << command.task
                                       << " bank=" << command.bankIndex << " row=" << command.row << " wcnt="
                                       << que_write_cnt << " rcnt=" << que_read_cnt << " bl=" << command.bl);
                    } else if (command.type == READ_P_CMD) {
                        DEBUG(setw(10) << now() << " -- EXEC(DRAM) :: send [READ_P_CMD] to DFI task=" << command.task
                                       << " bank=" << command.bankIndex << " row=" << command.row << " wcnt="
                                       << que_write_cnt << " rcnt=" << que_read_cnt << " bl=" << command.bl);
                    } else if (command.type == ACTIVATE1_CMD) {
                        DEBUG(setw(10) << now() << " -- EXEC(DRAM) :: send [ACTIVATE1_CMD] to DFI task=" << command.task
                                       << " bank=" << command.bankIndex << " row=" << command.row
                                       << " bl=" << command.bl);
                    } else if (command.type == ACTIVATE2_CMD) {
                        DEBUG(setw(10) << now() << " -- EXEC(DRAM) :: send [ACTIVATE2_CMD] to DFI task=" << command.task
                                       << " bank=" << command.bankIndex << " row=" << command.row
                                       << " bl=" << command.bl);
                    }
                }
    ...
    if (exec_valid) {
        if (command_pend != 0 && core_concurr_en) {
            ...
            if (DEBUG_BUS && command_pend > 0) {
                PRINT_SPD_PTC(ch, "{:10} -- CMD_PEND :: task={}", now(), command.task);
            }
        }
        if (command_pend == 0) {
            phy p;
            ...
            if (!odd_cycle) {
                ERROR(setw(10) << now() << " Every other command at even cycle violated"
                               << " task=" << command.task << " cmd_type=" << command.type << " rank=" << command.rank
                               << " group=" << command.group << " bank=" << command.bankIndex << " row" << command.row);
                assert(0);
            }
            ...
            if (PRINT_EXEC) {
                ...
                if (command.type == READ_CMD) {
                    DEBUGN(setw(10) << now() << " -- EXEC :: send [READ_CMD] to DFI task=" << command.task
                                    << " rank=" << command.rank << " sid=" << command.sid << " group=" << command.group
                                    << " bank=" << command.bankIndex << " row=" << command.row << " addr_col="
                                    << command.addr_col << " wcnt=" << que_write_cnt << " rcnt=" << que_read_cnt
                    ...
                    DEBUGN(setw(10) << now() << " -- EXEC :: send [READ_P_CMD] to DFI task=" << command.task
                                    << " rank=" << command.rank << " sid=" << command.sid << " group=" << command.group
                                    << " bank=" << command.bankIndex << " row=" << command.row << " addr_col="
                                    << command.addr_col << " wcnt=" << que_write_cnt << " rcnt=" << que_read_cnt
                                    << " bl=" << command.bl << " interval=" << (now() - pre_time_exec));
                    ...
                    DEBUGN(setw(10) << now() << " -- EXEC :: send [WRITE_CMD] to DFI task=" << command.task
                                    << " rank=" << command.rank << " sid=" << command.sid << " group=" << command.group
                                    << " bank=" << command.bankIndex << " row=" << command.row << " addr_col"
                                    << command.addr_col << " wcnt=" << que_write_cnt << " rcnt=" << que_read_cnt
                                    << " bl=" << command.bl << " interval=" << (now() - pre_time_exec));
                    ...
                    DEBUGN(setw(10) << now() << " -- EXEC :: send [WRITE_P_CMD] to DFI task=" << command.task
                                    << " rank=" << command.rank << " sid=" << command.sid << " group=" << command.group
                                    << " bank=" << command.bankIndex << " row=" << command.row << " addr_col="
                                    << command.addr_col << " wcnt=" << que_write_cnt << " rcnt=" << que_read_cnt
                                    << " bl=" << command.bl << " interval=" << (now() - pre_time_exec));
                    ...
                    DEBUGN(setw(10) << now() << " -- EXEC :: send [WRITE_MASK_CMD] to DFI task=" << command.task
                                    << " rank=" << command.rank << " sid=" << command.sid << " group=" << command.group
                                    << " bank=" << command.bankIndex << " row=" << command.row << " addr_col="
                                    << command.addr_col << " wcnt=" << que_write_cnt << " rcnt=" << que_read_cnt
                                    << " bl=" << command.bl << " interval=" << (now() - pre_time_exec));
                    ...
                    DEBUGN(setw(10) << now() << " -- EXEC :: send [WRITE_MASK_P_CMD] to DFI task=" << command.task
                                    << " rank=" << command.rank << " sid=" << command.sid << " group=" << command.group
                                    << " bank=" << command.bankIndex << " row=" << command.row << " addr_col="
                                    << command.addr_col << " wcnt=" << que_write_cnt << " rcnt=" << que_read_cnt
                                    << " bl=" << command.bl << " interval=" << (now() - pre_time_exec));
            ...
            if (PRINT_IDLE_LAT) {
                if (command.type == READ_CMD) {
                    DEBUG(setw(10) << now() << " -- EXEC(DFI) :: send [READ_CMD] to DFI task=" << command.task
                                   << " rank=" << command.rank << " sid=" << command.sid << " bank="
                                   << command.bankIndex << " row=" << command.row << " addr_col=" << command.addr_col
                                   << " wcnt=" << que_write_cnt << " rcnt=" << que_read_cnt << " bl=" << command.bl);
                } else if (command.type == READ_P_CMD) {
                    DEBUG(setw(10) << now() << " -- EXEC(DFI) :: send [READ_P_CMD] to DFI task=" << command.task
                                   << " rank=" << command.rank << " sid=" << command.sid << " bank="
                                   << command.bankIndex << " row=" << command.row << " addr_col=" << command.addr_col
                                   << " wcnt=" << que_write_cnt << " rcnt=" << que_read_cnt << " bl=" << command.bl);
                } else if (command.type == ACTIVATE1_CMD) {
                    DEBUG(setw(10) << now() << " -- EXEC(DFI) :: send [ACTIVATE1_CMD] to DFI task=" << command.task
                                   << " rank=" << command.rank << " sid=" << command.sid
                                   << " bank=" << command.bankIndex << " row=" << command.row
                                   << " addr_col=" << command.addr_col << " bl=" << command.bl);
                } else if (command.type == ACTIVATE2_CMD) {
                    DEBUG(setw(10) << now() << " -- EXEC(DFI) :: send [ACTIVATE2_CMD] to DFI task=" << command.task
                                   << " rank=" << command.rank << " sid=" << command.sid
                                   << " bank=" << command.bankIndex << " row=" << command.row
                                   << " addr_col=" << command.addr_col << " bl=" << command.bl);
                }
            ...
            if (DEBUG_BUS) {
                ...
                if (command.type == READ_CMD) {
                    PRINT_SPD_PTC(ch, "{:10} -- EXEC :: send [READ_CMD] to DFI task={} rank={} sid={} group={} bank={} "
                                  "row={} wcnt={} rcnt={} bl={} interval={}",
                        now(),
                        command.task,
                        command.rank,
                        command.sid,
                        ...
                        (now() - pre_time));
                    for (size_t i = 0; i < NUM_RANKS; i++) {
                        PRINT_SPD_PTC(ch, "{:10} -- SCH :: R{}R={} R{}W={}", now(), i, +r_rank_cnt[i], i, +w_rank_cnt[i]);
                    }

                    pre_time = now();
                } else if (command.type == READ_P_CMD) {
                    PRINT_SPD_PTC(ch, "{:10} -- EXEC :: send [READ_P_CMD] to DFI task={} rank={} sid={} group={} bank={} "
                                  "row={} wcnt={} rcnt={} bl={} interval={}",
                        now(),
                        command.task,
                        command.rank,
                    ...
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- EXEC :: send [ACTIVATE1_CMD] to DFI task={} rank={} sid={} group={} bank={} row={}",
                        now(),
                        command.task,
                        command.rank,
                        command.sid,
                    ...
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- EXEC :: send [ACTIVATE2_CMD] to DFI task={} rank={} sid={} group={} bank={} row={}",
                        now(),
                        command.task,
                        command.rank,
                        command.sid,
                    ...
                    PRINT_SPD_PTC(ch, "{:10} -- EXEC :: send [WRITE_CMD] to DFI task={} rank={} sid={} group={} bank={} "
                                  "row={} wcnt={} rcnt={} bl={} interval={}",
                        now(),
                        command.task,
                        command.rank,
                    ...
                    PRINT_SPD_PTC(ch, "{:10} -- EXEC :: send [WRITE_P_CMD] to DFI task={} rank={} sid={} group={} bank={} "
                                  "row={} wcnt={} rcnt={} bl={} interval={}",
                        now(),
                        command.task,
                        command.rank,
                    ...
                    PRINT_SPD_PTC(ch, "{:10} -- EXEC :: send [WRITE_MASK_CMD] to DFI task={} rank={} sid={} group={} "
                                  "bank={} row={} wcnt={} rcnt={} bl={} interval={}",
                        now(),
                        command.task,
                        command.rank,
                        command.sid,
                    ...
                    PRINT_SPD_PTC(ch, "{:10} -- EXEC :: send [WRITE_MASK_P_CMD] to DFI task={} rank={} sid={} group={} "
                                  "bank={} row={} wcnt={} rcnt={} bl={} interval={}",
                        now(),
                        command.task,
                        command.rank,
                    ...
                    PRINT_SPD_PTC(ch, "{:10} -- EXEC :: send [PRECHARGE_SB_CMD] to DFI task={} rank={} bank={} row={}",
                        now(),
                        command.task,
                        command.rank,
                        command.bankIndex,
                    ...
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- EXEC :: send [PRECHARGE_PB_CMD] to DFI task={} rank={} sid={} bank={} row={}",
                        now(),
                        command.task,
                        command.rank,
                        command.sid,
                    ...
                    PRINT_SPD_PTC(ch, "{:10} -- EXEC :: send [PRECHARGE_AB_CMD] to DFI task={} rank={}",
                        now(),
                        command.task,
                        command.rank);
                } else if (command.type == REFRESH_CMD) {
                    PRINT_SPD_PTC(ch, "{:10} -- EXEC :: send [REFRESH_CMD] to DFI task={}, rank={}",
                        now(),
                        command.task,
                        command.rank);
                } else if (command.type == PER_BANK_REFRESH_CMD) {
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- EXEC :: send [PER_BANK_REFRESH_CMD] to DFI task={} rank={} sid={} group={} bank={}",
                        now(),
                        command.task,
                        command.rank,
                        command.sid,
    ...
    if (core_concurr_en) {
        ...
        for (auto &state : bankStates) {
            if (state.state->currentBankState == RowActive)
                BankRowActCnt[state.bankIndex]++;
        }
    }
}
...
void PTC::state_fresh()
{
    ...
    for (auto &state : bankStates) {
        ...
        if (EM_ENABLE && EM_MODE == 2 && state.rank == 1 && state_channel == 1)
            continue;  // rank1, sc1 forbidden under combo e-mode
        if (state.state->rwIntlvCountdown > 0 && core_concurr_en)
            state.state->rwIntlvCountdown--;
            ...
            if (state.state->stateChangeCountdown == 0) {
                switch (state.state->lastCommand) {  // only these commands have an implicit state change
                    ...
                    case WRITE_MASK_P_CMD: {
                        ...
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, "{:10} -- CHANGE :: READ/WRITE/WRITE_MASK_P_CMD, rank={}, bank={}",
                                now(),
                                state.rank,
                                state.bankIndex);
                        }
                    ...
                    case REFRESH_CMD: {
                        ...
                        if (DEBUG_BUS && (state.bankIndex % NUM_BANKS == 0)) {
                            PRINT_SPD_PTC(ch, 
                                "{:10} -- CHANGE :: REFRESH_CMD, rank={}, bank={}", now(), state.rank, state.bankIndex);
                        }
                        break;
                    }
                    ...
                    case PRECHARGE_PB_CMD: {
                        ...
                        state.state->stateChangeEn = false;
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, "{:10} -- CHANGE :: PRECHARGE_P/SB_CMD, rank={}, bank={}",
                                now(),
                                state.rank,
                                state.bankIndex);
                        }
                    ...
                    case PRECHARGE_AB_CMD: {
                        ...
                        state.state->stateChangeEn = false;
                        if (DEBUG_BUS && (state.bankIndex % NUM_BANKS == 0)) {
                            PRINT_SPD_PTC(ch, "{:10} -- CHANGE :: PRECHARGE_AB_CMD, rank={}, bank={}",
                                now(),
                                state.rank,
                                state.bankIndex);
                        }
                    ...
                    case PER_BANK_REFRESH_CMD: {
                        ...
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, "{:10} -- CHANGE :: PER_BANK_REFRESH_CMD, rank={}, bank={}",
                                now(),
                                state.rank,
                                state.bankIndex);
                        }
                    ...
                    case ACTIVATE2_CMD: {
                        ...
                        state.hold_refresh_pb = false;
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, "{:10} -- CHANGE :: ACTIVATE2/_DST_CMD, rank={}, bank={}",
                                now(),
                                state.rank,
                                state.bankIndex);
                        }
    ...
    if (!WCK_ALWAYS_ON && (IS_LP5 || IS_LP6)) {
        for (size_t i = 0; i < NUM_RANKS; i++) {
            if (!RankState[i].wck_on)
                continue;
            if (now() < RankState[i].wck_off_time)
                continue;
...
bool PTC::check_rankStates(uint8_t rank)
{
    for (size_t i = 0; i < NUM_BANKS; i++) {
        ...
        if (bankStates[bank].state->currentBankState == RowActive)
            return false;
    }
    return true;
}
void PTC::refresh(unsigned sc)
{
    ...
    if (AREF_EN || PBR_EN) {  // other spec
        if (DMC_V590 && SBR_IDLE_ADAPT_EN) {
            if (now() % SBR_IDLE_ADAPT_WIN == 0 && now() != 0) {
                for (size_t rank = 0; rank < NUM_RANKS; rank++) {
                    if (combo_emode_unable && rank == 1)
                        continue;  // rank1, sc1 forbidden under combo e-mode
                    rank_send_pbr[rank][sc] =
                        rank_cnt_sbridle[rank][sc] >= SBR_IDLE_ADAPT_LEVEL;  // todo: revise for e-mode
        ...
        for (size_t rank = 0; rank < NUM_RANKS; rank++) {
            if (refreshALL[rank][sc].refresh_cnt > 8) {
                ERROR(setw(10) << now() << " Postponed exceed 8, rank=" << rank << ", sc=" << sc
                               << ", cnt=" << refreshALL[rank][sc].refresh_cnt);
                assert(0);
            }
        }
        for (size_t rank = 0; rank < NUM_RANKS; rank++) {
            ...
            if ((RankState[rank].lp_state == IDLE || RankState[rank].lp_state == PDE ||
                    ...
                    RankState[rank].lp_state == PDX) &&
                ((now() + ref_offset[rank] - AsrefTime[rank] - SrpdTime[rank]) % tREFI) ==
                    0) {  // todo: revise for e-mode
                refreshALL[rank][sc].refresh_cnt++;
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- PSTPND_ADD :: POSTPND={}, rank={}, sc={}, AsrefTime={}, SrpdTime={}",
                        now(),
                        refreshALL[rank][sc].refresh_cnt,
                        rank,
                        sc,
            ...
            if (refresh_pbr_has_finish[rank][sc]) {
                refreshALL[rank][sc].refresh_cnt--;
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- PSTPND_SUB :: POSTPND={}, rank={}, sc={}, AsrefTime={}, SrpdTime={}",
                        now(),
                        refreshALL[rank][sc].refresh_cnt,
                        rank,
                        sc,
    ...
    for (size_t i = 0; i < NUM_RANKS; i++) {
        for (size_t j = 0; j < NUM_BANKS / sc_num; j++) {
            ...
            if (refreshPerBank[bank].refreshing && bank_cnt[bank] > 0 &&
                (refreshALL[i][sc].refresh_cnt < (PBR_PSTPND_LEVEL - 1) && PBR_PSTPND_LEVEL > 0)) {
                // ERROR(setw(10) << now() << " -- Perf Slot Wasted When Rrefreshing, rank=" << i << ", sc=" << sc
                            //    << ", bank=" << bank);
                // assert(0);
            }
        }
    ...
    if (AREF_EN) {
        for (size_t rank = 0; rank < NUM_RANKS; rank++) {
            if (RankState[rank].lp_state == ASREFE)
                continue;
            if (RankState[rank].lp_state == ASREF)
                continue;
            if (RankState[rank].lp_state == ASREFX)
                continue;
            if (RankState[rank].lp_state == SRPDE)
                continue;
            if (RankState[rank].lp_state == SRPD)
                continue;
            if (RankState[rank].lp_state == SRPDLP)
                continue;
            if (RankState[rank].lp_state == SRPDX)
                continue;
            if (combo_emode_unable && rank == 1)
                continue;  // rank1, sc1 forbidden under combo e-mode
    ...
    if (PBR_EN) {
        for (size_t rank = 0; rank < NUM_RANKS; rank++) {
                    ...
                    for (size_t i = 0; i < pbr_sb_group_num; i++) {
                        for (size_t j = 0; j < pbr_sb_num; j++) {
                            ...
                            if (refreshPerBank[bank_tmp].refreshWaiting || refreshPerBank[bank_tmp].refreshing) {
                                if ((bank_tmp % pbr_sb_group_num) != (pre_bank_tmp % pbr_sb_group_num) &&
                                    (((pbr_bank_cnt == 1) && (pre_bank_tmp == 0)) || pre_bank_tmp != 0)) {
                                    ERROR(setw(10) << now() << " -- DMC[" << channel
                                                   << "] Enhanced Refresh Wrong Bank pairs, rank=" << rank
                                                   << " bank=" << bank_tmp << " pre bank=" << pre_bank_tmp);
                                    assert(0);
                                }
                                ...
                                if (pbr_bank_cnt > 2) {
                                    ERROR(setw(10) << now() << " -- DMC[" << channel
                                                   << "] Enhanced Refresh bank exceed 2, rank=" << rank);
                                    assert(0);
                                }
                            }
                        }
                        ...
                        if (refreshPerBank[bank_tmp].refreshWaiting || refreshPerBank[bank_tmp].refreshing) {
                            for (size_t j = i + 1; j < pbr_bank_num; j++) {
                                ...
                                if (refreshPerBank[bank_loop].refreshWaiting || refreshPerBank[bank_loop].refreshing) {
                                    ERROR(setw(10) << now() << " -- DMC[" << channel
                                                   << "] Refresh bank exceed 2, rank=" << rank << ", sc=" << sc << " bank1="<<bank_tmp <<" bank2="<<bank_loop);
                                    // assert(0);
                                }
                            }
                            break;
            ...
            if (RankState[rank].lp_state == SRPDX)
                continue;
            if ((RankState[rank].lp_state == PD || RankState[rank].lp_state == PDLP) && !PD_PBR_EN)
                continue;
            if (ENH_PBR_EN) {
                enh_per_bank_refresh(rank, sc);
            } else {
                per_bank_refresh(rank, sc); 
                // pull_in_refresh(rank,sc);
            }
        }
    }
}
...
{
    if (RankState[rank].lp_state == ASREFE || RankState[rank].lp_state == ASREF || RankState[rank].lp_state == ASREFX ||
        RankState[rank].lp_state == SRPDE || RankState[rank].lp_state == SRPD || RankState[rank].lp_state == SRPDLP ||
    ...
    if (has_issue_size) {
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, 
                "{:10} -- Rd/Wr cmd (issue!=0) must be send before all bank refresh rank={}, sc={}", now(), rank, sc);
        }
        return;
    }
    ...
    if (act2_left) {  // todo: revise for e-mode
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- Act2 must be send before refresh rank={}, sc={}", now(), rank, sc);
        }
        return;
    }
    ...
    if (refreshALL[rank][sc].refreshWaiting && !refreshALL[rank][sc].refreshing) {
        ...
        if (arb_enable && !idle && precharge_en) {
            ...
            if (RankState[rank].lp_state == IDLE && even_cycle && (!IS_HBM3 || now() % 2 == 1 || ODD_TDM)) {  // every other command, even;
                ...
                while (!rw_cmdqueue.empty())
                    rw_cmdqueue.erase(rw_cmdqueue.begin());
                while (!act_cmdqueue.empty())
                    act_cmdqueue.erase(act_cmdqueue.begin());
                while (!pre_cmdqueue.empty())
                    pre_cmdqueue.erase(pre_cmdqueue.begin());
                Cmd *c = new Cmd;
                c->rank = rank;
                c->group = 0;
                c->bank = 0;
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- REQ :: refresh, precharge bank{}, task={}, rank={}, sc={}",
                        now(),
                        c->bank,
                        c->task,
                        rank,
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- REQ_LP :: hold PRECHARGE in lp state, rank={}, sc={}", now(), rank, sc);
                }
            }
        }
        ...
        if (idle) {
            ...
            if (arb_enable && abr_timing_met) {
                ...
                if (RankState[rank].lp_state == IDLE && even_cycle) {  // every other command, even;
                    ...
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- REQ :: REFRESH rank{}, task={} rank={}, sc={}",
                            now(),
                            c->rank,
                            c->task,
                            rank,
                    ...
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- REQ_LP :: hold REFRESH in lp state, rank={}, sc={}", now(), rank, sc);
                    }
                }
            }
        }
    ...
    if (refreshALL[rank][sc].refresh_cnt >= ABR_PSTPND_LEVEL) {
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- POS :: refresh cnt={}, force refresh, rank={}, sc={}",
                now(),
                refreshALL[rank][sc].refresh_cnt,
                rank,
                sc);
        ...
        if (sc_cnt[rank][sc] == 0) {  // todo: revise for e-mode
            if (SBR_IDLE_EN) {
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- POS :: refresh_cnt={}, rank={}, sc={} is Idle, will refresh by pbr",
                        now(),
                        refreshALL[rank][sc].refresh_cnt,
                        rank,
                        sc);
                ...
                if (no_bank_pbr && idle_abr_send) {
                    refreshALL[rank][sc].refreshWaiting = true;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- POS :: refresh_cnt={}, rank={}, sc={} is Idle, will refresh by abr",
                            now(),
                            refreshALL[rank][sc].refresh_cnt,
                            rank,
                            sc);
...
void PTC::per_bank_refresh(unsigned rank, unsigned sc)
{
    if (refreshALL[rank][sc].refresh_cnt == 0 || refreshALL[rank][sc].refreshWaiting ||
            ...
            RankState[rank].lp_state == ASREFX || RankState[rank].lp_state == SRPDE || RankState[rank].lp_state ==
            SRPD || RankState[rank].lp_state == SRPDLP || RankState[rank].lp_state == SRPDX)
        return;
    if (now() < sbr_gap_cnt[rank])
        return;  // todo: revise for e-mode
    ...
    if (act2_left) {  // todo: revise for e-mode
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- Act2 must be send before refresh, sc={}", now(), sc);
        }
        return;
    }
    ...
    for (size_t bank = 0; bank < pbr_bank_num; bank++) {
        unsigned bank_tmp = rank * NUM_BANKS + bank + bank_start;
        if (IS_HBM2E || IS_HBM3 || PBR_PARA_EN) {
            if (!refreshPerBank[bank_tmp].refreshWaiting)
                continue;
        } else {
            if (!refreshPerBank[bank_tmp].refreshWaiting && !refreshPerBank[bank_tmp].refreshing)
                continue;
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, 
                "{:10} -- PBR :: has bank refresh, rank={}, bank={}, sc={}, pbrw={}, pbr={}", 
                now(), 
                rank, 
                bank_tmp, 
                sc, 
                refreshPerBank[bank_tmp].refreshWaiting, 
                refreshPerBank[bank_tmp].refreshing);
        }
    ...
    if (!have_bank_refresh) {
        for (size_t i = 0; i < pbr_bank_num; i++) {
            PbrWeight[rank][i + bank_pair_start].bank_pair_cnt = 1;
        ...
        for (size_t bank = 0; bank < pbr_bank_num; bank++) {
            ...
            for (size_t sbr_bank = 0; sbr_bank < pbr_bg_num; sbr_bank++) {
                unsigned bank_tmp = rank * NUM_BANKS + sbr_bank * pbr_bank_num + bank + bank_start;
                if(pfq_->wbuff_state == WBUFF_WRITE && GROUP_REF_EN){
                    if(pfq_->rb_bank_cnt[rank][bank_tmp] > 0 && r_bank_cnt[bank_tmp] == 0){
                        has_rcmd = 1;
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, "{:10} -- PBR :: BANK {}, SC {} has cmd in read group during write group", now(), bank_tmp, sc);
                        }
                        break;
                    }
                }
            }
            if (SBR_WEIGHT_MODE == 0)
                PbrWeight[rank][bank + bank_pair_start].bank_pair_cnt += (has_bank_open - has_rcmd);
            else if (SBR_WEIGHT_MODE == 1)
                PbrWeight[rank][bank + bank_pair_start].bank_pair_cnt += (has_cmd_vld   - has_rcmd);
            else if (SBR_WEIGHT_MODE == 2)
                PbrWeight[rank][bank + bank_pair_start].bank_pair_cnt += (has_bank_open + has_cmd_vld - has_rcmd);
            else if (SBR_WEIGHT_MODE == 3)
                PbrWeight[rank][bank + bank_pair_start].bank_pair_cnt += (has_bank_open + has_cmd_vld * 2 - has_rcmd);
            else
                PbrWeight[rank][bank + bank_pair_start].bank_pair_cnt += bank_cnt[bankIndex] + bank_cnt[another_bankIndex];  
            // added for SBR_WEIGHT_MODE=4/5, select bank pair with least/most cmd
...
#if 1
        // group_refresh 
        if (refreshALL[rank][sc].refresh_cnt < PBR_PSTPND_LEVEL && pfq_->wbuff_state == WBUFF_IDLE && GROUP_REF_EN){
        ...
        if (refreshALL[rank][sc].refresh_cnt < PRE_PBR_PSTPND_LEVEL && ORI_REF_EN) {  // todo: revise for e-mode
        ...
        if (PBR_LOCK_MODE == 0) {
            if (refreshALL[rank][sc].refresh_cnt < PBR_PSTPND_LEVEL&& ORI_REF_EN) {  // todo: revise for e-mode
        ...
        // pull in refresh
        if(pfq_->wbuff_state == WBUFF_WRITE && pfq_->wbuff_state_pre == WBUFF_IDLE && GROUP_REF_EN){
                unsigned has_rcmd_bank = 0;
                for (size_t i = 0; i < NUM_BANKS / sc_num; i++) {
                    unsigned bank_idx = rank * NUM_BANKS + i + bank_start;
                    if(pfq_->rb_bank_cnt[rank][bank_idx] > 0 && r_bank_cnt[bank_idx] == 0 && pfq_->wb_bank_cnt[rank][bank_idx]== 0 &&
!bankStates[bank_idx].finish_refresh_pb){
                        // has_rcmd_bank = 1;
                        PbrWeight[rank][i % pbr_bank_num + bank_pair_start].bank_pair_cnt = 0;
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, 
                                "{:10} -- PIR :: send pull in refresh, rank={}, bank={}, sc={}", now(), rank, bank_idx, sc);
                        }
                    }
                }
        }
        ...
        for (size_t bank = 0; bank < pbr_bank_num; bank++) {
            ...
            if ((pbr_bankidx == pre_sch_bankIndex[rank] || (pbr_bankidx_another == pre_sch_bankIndex[rank])) &&
                (SBR_WEIGHT_ENH_MODE == 2) &&
        ...
        if (SBR_WEIGHT_ENH_MODE == 2 && pre_sch_bankIndex_met && pre_sch_bank_pair_cnt != 0xFFFFFFFF &&
            pre_sch_bank_pair != 0xFFFFFFFF) {
        ...
        for (size_t bank = 0; bank < pbr_bank_num; bank++) {
            ...
            if (bank_pair_cnt_min_num != 0xFFFFFFFF) {
                if (PbrWeight[rank][bank_pair].bank_pair_cnt == bank_pair_cnt_min &&
                        ((bank_pair_cmd_cnt[rank][bank_pair] < bank_pair_cmd_cnt[rank][bank_pair_cnt_min_num] &&
                             SBR_WEIGHT_ENH_MODE == 3) ||
                            (bank_pair_cmd_cnt[rank][bank_pair] > bank_pair_cmd_cnt[rank][bank_pair_cnt_min_num] &&
                                SBR_WEIGHT_ENH_MODE == 4)) &&bank_pair_cnt_min_num !=
                        bank_pair &&bank_pair_cnt_min_num != 0xFFFFFFFF && !bankStates[bankIndex].finish_refresh_pb) {
        ...
        if (bank_pair_cnt_min_num != 0xFFFFFFFF) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- PBR_WEIGHT :: rank={} bank={} matgrp={} sc={} win the arbitration!",
                    now(),
                    rank,
                    (bank_pair_cnt_min_num / NUM_MATGRPS),
                    +bank_pair_cnt_min_num,
                    sc);
            }
        }
    }
    ...
    for (size_t bank = 0; bank < pbr_bank_num; bank++) {  // todo: revise for e-mode
        ...
        if (bankStates[bank_tmp].finish_refresh_pb)
            refresh_cnt_pb[rank][sc]++;
        else
            forceRankBankIndex[rank][sc] = bank + bank_start;
        ...
        if (SBR_REQ_MODE == 0) {
            if (bank == pbr_bank_num - 1) {
                if (refresh_cnt_pb[rank][sc] == pbr_bank_num) {
                    ...
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- PBR :: RANK {}, SC {} has been refreshed by pbr", now(), rank, sc);
                    }
                } else {
                    force_pbr_refresh[rank][sc] = true;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, 
                            "{:10} -- PBR :: Force pbr rank {}, SC {} has no idle bank not refreshed", now(), rank, sc);
                    }
                }
            }
        } else {
            if (bank == pbr_bank_num - 1) {
                if (refresh_cnt_pb[rank][sc] == pbr_bank_num) {
                    ...
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, 
                            "{:10} -- PBR :: Force pbr for the left banks in RANK {}, SC {}", now(), rank, sc);
                    }
                }
            }
        }
...
void PTC::send_pb_precharge(unsigned rank, unsigned sc)
{
    ...
    for (size_t bank = 0; bank < pbr_bank_num; bank++) {
        ...
        if (refresh_pbr_has_finish[rank][sc])
            continue;
        if (force_pbr_refresh[rank][sc] && !bankStates[bank_tmp].finish_refresh_pb && !have_bank_refresh &&
            PbrWeight[rank][bank + bank_pair_start].send_pbr) {
            for (size_t matgrp = 0; matgrp < NUM_MATGRPS; matgrp++) {
                if (arb_enable) {
                    ...
                    bool rw_switch_trig =
                        ((PreCmd.trans_type == DATA_READ) && que_read_cnt < PBR_LESS_CMD_LEVEL && que_write_cnt > 0) ||
                    ...
                    for (size_t sbr_bank = 0; sbr_bank < pbr_bg_num; sbr_bank++) {
                        ...
                        if (PBR_LESS_CMD_EN && PBR_LESS_CMD_MODE == 0) {  // rw_switch_trig
                            for (size_t i = 0; i < transactionQueue.size(); i++) {
                                if (transactionQueue[i]->bankIndex != bank_idx)
                                    continue;
                                if (transactionQueue[i]->transactionType != PreCmd.trans_type)
                                    continue;
                                force_sch_cmd = true;
                            }
                        }

                        if (bank_cnt[bank_idx] > 0 && PBR_LESS_CMD_EN && PBR_LESS_CMD_MODE == 0 && rw_switch_trig &&
                            force_sch_cmd) {  // PBR_LOCK_MODE=0
                            ptc_cmd_exist = true;
                            if (DEBUG_BUS) {
                                PRINT_SPD_PTC(ch, "{:10} -- Force Pbr Hold :: Cmd Exist, rank={}, bankIndex={}, sc={}",
                                    now(),
                                    rank,
                                    bank_idx,
                                    sc);
                            ...
                            break;
                        } else if (bank_cnt[bank_idx] > 0 && PBR_LESS_CMD_EN &&
                                   PBR_LESS_CMD_MODE == 1) {  // PBR_LOCK_MODE=1
                            ptc_cmd_exist = true;
                            if (DEBUG_BUS) {
                                PRINT_SPD_PTC(ch, "{:10} -- Force Pbr Hold :: Cmd Exist, rank={}, bankIndex={}, sc={}",
                                    now(),
                    ...
                    if (ready_send_pbr &&
                        ((tFAWCountdown[rank].size() < 4 && sc == 0)  // todo: revise for e-mode
                            || (tFAWCountdown_sc1[rank].size() < 4 && sc == 1)) &&
                        !ptc_cmd_exist && (!ASREF_ENABLE || (ASREF_ENABLE && RankState[rank].asref_cnt != 0))) {
                        funcState[rank].wakeup = true;
                        if (RankState[rank].lp_state == IDLE && even_cycle && (!IS_HBM3 || now() % 2 == 1 || ODD_TDM)) {  // every other command, even;
                            Cmd *c            = new Cmd;
                                c->state      = working;
                            ...
                            if (DEBUG_BUS) {
                                PRINT_SPD_PTC(ch, 
                                    "{:10} -- REQ :: force pbr, rank={}, bank={}, sc={}", now(), rank, bank_tmp, sc);
                            }
                            return;
                        } else {
                            if (DEBUG_BUS) {
                                PRINT_SPD_PTC(ch, "{:10} -- REQ_LP :: hold FPBR in lp, rank={}, bank={}, sc={}",
                                    now(),
                                    rank,
                                    bank_tmp,
                                    sc);
                        ...
                        for (size_t sbr_bank = 0; sbr_bank < pbr_bg_num; sbr_bank++) {
                            ...
                            if (bankStates[bank_idx].state->currentBankState == RowActive) {
                                ...
                                if (IS_LP4 || IS_LP5 || IS_LP6) {
                                    if ((now() + 1) < bankStates[bank_idx].state->nextPrecharge ||
                                        issue_state[bank_idx]) {
                                ...
                                if (sb_ready_send_pre && !cmd_exist) {
                                    ...
                                    if (RankState[rank].lp_state == IDLE && even_cycle) {  // every other command, even;
                                        ...
                                        if (DEBUG_BUS) {
                                            PRINT_SPD_PTC(ch, "{:10} -- REQ :: force precharge, rank={}, bank={}, sc={}",
                                                now(),
                                                rank,
                                                bank_tmp,
                                                sc);
                                        ...
                                        if (DEBUG_BUS) {
                                            PRINT_SPD_PTC(ch, "{:10} -- REQ_LP :: hold PRECHARGE PB/SB in lp state, "
                                                          "rank{}, sc={}, bank={}",
                                                now(),
                                                rank,
                                                sc,
...
void PTC::send_pb_refresh(unsigned rank, unsigned sc)
{
    ...
    for (size_t bank = 0; bank < pbr_bank_num; bank++) {  // todo: revise for e-mode
        unsigned bank_tmp = rank * NUM_BANKS + bank + bank_start;
        if (force_pbr_refresh[rank][sc])
            continue;
        if (refresh_pbr_has_finish[rank][sc])
            continue;
        if (bankStates[bank_tmp].en_refresh_pb && bankStates[bank_tmp].hold_refresh_pb &&
            PbrWeight[rank][bank + bank_pair_start].send_pbr) {
            for (size_t matgrp = 0; matgrp < NUM_MATGRPS; matgrp++) {
                ...
                for (size_t sbr_bank = 0; sbr_bank < pbr_bg_num; sbr_bank++) {
                    ...
                    if (bank_cnt[bank_idx] > 0 && PBR_LESS_CMD_EN && PBR_LESS_CMD_MODE == 0 && rw_switch_trig &&
                        ...
                        ptc_cmd_exist = true;
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, "{:10} -- Idle Pbr Hold :: Cmd Exist, rank={}, bankIndex={}, sc={}",
                                now(),
                                rank,
                                bank_idx,
                                sc);
                        ...
                        break;
                    } else if (bank_cnt[bank_idx] > 0 && PBR_LESS_CMD_EN && PBR_LESS_CMD_MODE == 1 && rw_switch_trig &&
                               force_sch_cmd) {
                        ptc_cmd_exist = true;
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, "{:10} -- Idle Pbr Hold :: Cmd Exist, rank={}, bankIndex={}, sc={}",
                ...
                if (arb_enable && ((now() + 1) >= bankStates[bank_tmp].state->nextPerBankRefresh) &&
                    ...
                    if (RankState[rank].lp_state == IDLE && even_cycle && (!IS_HBM3 || now() % 2 == 1 || ODD_TDM)) {  // every other command, even;
                        ...
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, 
                                "{:10} -- REQ :: PBR IDLE, rank={}, bank={}, sc={}", now(), rank, bank_tmp, sc);
                        }
                        return;
                    } else {
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, "{:10} -- REQ_LP :: hold FPBR in lp, rank={}, sc={}, bank={}",
                                now(),
                                rank,
                                sc,
                                bank_tmp);
                    ...
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- PBR :: rank{}, bank={}, arb_enable={}, sc={}",
                            now(),
                            rank,
                            bank_tmp,
                            arb_enable,
...
void PTC::enh_per_bank_refresh(unsigned rank, unsigned sc)
{
    ...
    if (!have_bank_refresh) {
        ...
        for (size_t bank = 0; bank < pbr_sb_group_num; bank++) {
            ...
            if (SBR_WEIGHT_MODE == 0)
                SbGroupWeight[rank][bank].bank_pair_cnt = has_bank_open;
            else if (SBR_WEIGHT_MODE == 1)
                SbGroupWeight[rank][bank].bank_pair_cnt = has_cmd_vld;
            else if (SBR_WEIGHT_MODE == 2)
                SbGroupWeight[rank][bank].bank_pair_cnt = has_bank_open + has_cmd_vld;
            else if (SBR_WEIGHT_MODE == 3)
                SbGroupWeight[rank][bank].bank_pair_cnt = has_bank_open + has_cmd_vld * 2;
        ...
        for (size_t bank = 0; bank < pbr_sb_group_num; bank++) {
            for (size_t sb_bank = 0; sb_bank < pbr_sb_num; sb_bank++) {
                for (size_t sb_bank_tmp = (sb_bank + 1); sb_bank_tmp < pbr_sb_num; sb_bank_tmp++) {
                    ...
                    unsigned lst_bank_tmp =
                        rank * NUM_BANKS + bank + sb_bank_tmp * pbr_sb_num;  // another bank of a bank pair
                    if (bankStates[fst_bank_tmp].state->currentBankState == RowActive ||
                        bankStates[lst_bank_tmp].state->currentBankState == RowActive) {
                        has_bank_open_sb = 1;
                    }
                    ...
                    if (SBR_WEIGHT_MODE == 0)
                        SbWeight[rank][sbweight_idx].bank_pair_cnt = has_bank_open_sb;
                    else if (SBR_WEIGHT_MODE == 1)
                        SbWeight[rank][sbweight_idx].bank_pair_cnt = has_cmd_vld_sb;
                    else if (SBR_WEIGHT_MODE == 2)
                        SbWeight[rank][sbweight_idx].bank_pair_cnt = has_bank_open_sb + has_cmd_vld_sb;
                    else if (SBR_WEIGHT_MODE == 3)
                        SbWeight[rank][sbweight_idx].bank_pair_cnt = has_bank_open_sb + has_cmd_vld_sb * 2;
                    ...
                    //                    DEBUG(now()<<" sbweight, bank pair="<<sbweight_idx<<"
                    //                    fst_bankIndex="<<fst_bank_tmp<<" lst_bankIndex="<<lst_bank_tmp);
                }
            }
        }
        ...
        if (ENH_PBR_CEIL_EN) {  // 1 in 24, the best for enhanced pbr
            for (size_t group = 0; group < pbr_sb_group_num; group++) {
                for (size_t bank = 0; bank < SbWeight[rank].size(); bank++) {
                    ...
                    if (group == bagroup_cnt) {
                        ...
                        // added for SBR_WEIGHT_ENH_MODE=2
                        if ((fst_bank == pre_sch_bankIndex[rank] || lst_bank == pre_sch_bankIndex[rank]) &&
                            (SBR_WEIGHT_ENH_MODE == 2) &&
                            (!bankStates[fst_bank].finish_refresh_pb && !bankStates[lst_bank].finish_refresh_pb) &&
                            (!DMC_V580 || SbWeight[rank][bank].bank_pair_cnt != 0xFFFFFFFF)) {
                            pre_sch_bankIndex_met = true;
            ...
            for (size_t group = 0; group < pbr_sb_group_num; group++) {  // 4 in 1
                ...
                if (bagroup_cnt_min != 0xFFFFFFFF && bagroup_cnt_min != bank_pair_group)
                    break;  // standard 4in1 && 6in1
                if (pbr_bank_left && bank_pair_group != pre_enh_pbr_bagroup[rank] &&
                    pre_enh_pbr_bagroup[rank] != 0xFFFFFFFF && SBR_WEIGHT_ENH_MODE == 1)
                    continue;  // same bank pair group priority, added for SBR_WEIGHT_ENH_MODE=1
                for (size_t bank = 0; bank < SbWeight[rank].size(); bank++) {
                    unsigned fst_bank = SbWeight[rank][bank].fst_bankIndex;
                    unsigned lst_bank = SbWeight[rank][bank].lst_bankIndex;
                    unsigned bagroup_cnt = SbWeight[rank][bank].bagroup;
                    if (bank_pair_group == bagroup_cnt) {  // 6 in 1
                        if (bank_pair_cnt_min > SbWeight[rank][bank].bank_pair_cnt &&
                            (!bankStates[fst_bank].finish_refresh_pb && !bankStates[lst_bank].finish_refresh_pb) &&
                            (!DMC_V580 || SbWeight[rank][bank].bank_pair_cnt != 0xFFFFFFFF) && (SBR_WEIGHT_MODE != 5)) {
                            bank_pair_cnt_min         = SbWeight[rank][bank].bank_pair_cnt;
                            bank_pair_cnt_min_num_fst = fst_bank;
                            bank_pair_cnt_min_num_lst = lst_bank;
                            bagroup_cnt_min           = bagroup_cnt;
        ...
        for (size_t bank = 0; bank < SbWeight[rank].size(); bank++) {
            ...
            if (bank_pair_cmd_cnt_min_num != 0xFFFFFFFF) {
                if (bagroup_cnt_min == bagroup_cnt && bagroup_cnt_min != 0xFFFFFFFF &&
                    bank_pair_cnt_min == SbWeight[rank][bank].bank_pair_cnt &&
                    (!bankStates[fst_bank].finish_refresh_pb && !bankStates[lst_bank].finish_refresh_pb)) {
                    if ((bank_pair_cmd_cnt[rank][bank] < bank_pair_cmd_cnt[rank][bank_pair_cmd_cnt_min_num] &&
                            SBR_WEIGHT_ENH_MODE == 3) ||
                        (bank_pair_cmd_cnt[rank][bank] > bank_pair_cmd_cnt[rank][bank_pair_cmd_cnt_min_num] &&
                            SBR_WEIGHT_ENH_MODE == 4)) {
                        bank_pair_cnt_min         = SbWeight[rank][bank].bank_pair_cnt;
                        bank_pair_cnt_min_num_fst = fst_bank;
                        bank_pair_cnt_min_num_lst = lst_bank;
                        bagroup_cnt_min           = bagroup_cnt;
        ...
        if ((bank_pair_cnt_min_num_fst != 0xFFFFFFFF) && (bank_pair_cnt_min_num_lst != 0xFFFFFFFF) &&
            (bagroup_cnt_min != 0xFFFFFFFF)) {
            //            DEBUG(now()<<" min_fst_bankindex="<<bank_pair_cnt_min_num_fst<<"
            //            min_lst_bankindex="<<bank_pair_cnt_min_num_lst<<" min_bagroup="<<bagroup_cnt_min);
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, 
                    "{:10} -- ENHANCED PBR_WEIGHT :: rank={} fst_bank={} lst_bank={} ba group={} win the arbitration!",
                    now(),
                    rank,
                    bank_pair_cnt_min_num_fst,
                    bank_pair_cnt_min_num_lst,
                    bagroup_cnt_min);
            }
            // bankIndex check
            if (bank_pair_cnt_min_num_fst == bank_pair_cnt_min_num_lst) {
                ERROR(setw(10) << now() << " Not allowed same bank in a bank pair, bagroup=" << bagroup_cnt_min
                               << " fst_bankIndex" << bank_pair_cnt_min_num_fst << " lst_bankIndex"
                               << bank_pair_cnt_min_num_lst);
                assert(0);
            ...
            if (((bank_pair_cnt_min_num_lst % pbr_sb_num) != bagroup_cnt_min) ||
                ((bank_pair_cnt_min_num_fst % pbr_sb_num != bagroup_cnt_min))) {
                ERROR(setw(10) << now() << " Wrong bagroup, bgroup=" << bagroup_cnt_min << " fst_bankIndex"
                               << bank_pair_cnt_min_num_fst << " lst_bankIndex" << bank_pair_cnt_min_num_lst);
                assert(0);
            }
        }
    }
    ...
    for (size_t bank = 0; bank < NUM_BANKS; bank++) {
        ...
        if (bankStates[bank_tmp].finish_refresh_pb)
            refresh_cnt_pb[rank][sc]++;  // 2X refresh commands
        else
            forceRankBankIndex[rank][sc] = bank;
        ...
        if (SBR_REQ_MODE == 0) {
            if (bank == NUM_BANKS - 1) {
                if (refresh_cnt_pb[rank][sc] == NUM_BANKS) {
                    ...
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- ENHANCED PBR :: RANK {} has been refreshed by pbr", now(), rank);
                    }
                } else {
                    force_pbr_refresh[rank][sc] = true;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, 
                            "{:10} -- ENHANCED PBR :: Force pbr rank {} has no idle bank not refreshed", now(), rank);
                    }
                }
            }
        } else {
            if (bank == NUM_BANKS - 1) {
                if (refresh_cnt_pb[rank][sc] == NUM_BANKS) {
                    ...
                    force_pbr_refresh[rank][sc] = true;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- ENHANCED PBR :: Force pbr for the left banks in RANK {}", now(), rank);
                    }
                }
            }
        }
...
void PTC::enh_send_pb_precharge(unsigned rank, unsigned sc)
{
    ...
    for (size_t bank = 0; bank < NUM_BANKS; bank++) {
        ...
        // bp_idx size check
        if (bp_idx.size() != 0) {
            ERROR(setw(10) << now() << " after clear, Vector bp_idx size check failed, size=" << bp_idx.size()
                           << " bank=" << bank_tmp << " fst_bankIndex=" << fst_bank_idx
                           << " lst_bankIndex=" << lst_bank_idx);
            assert(0);
        }
            ...
            for (size_t matgrp = 0; matgrp < NUM_MATGRPS; matgrp++) {
                if (arb_enable) {
                    ...
                    for (size_t i = 0; i < bp_idx.size(); i++) {
                        unsigned bank_idx = bp_idx[i];
                        if ((bankStates[bank_idx].state->currentBankState != Idle ||
                                (now() + 1) < bankStates[bank_idx].state->nextPerBankRefresh) ||
                            issue_state[bank_idx]) {
                            ready_send_pbr = false;
                            break;
                        }
                    ...
                    if (ready_send_pbr &&
                        ...
                        if (RankState[rank].lp_state == IDLE && even_cycle && (!IS_HBM3 || now() % 2 == 1 || ODD_TDM)) {  // every other command, even;
                            ...
                            CmdQueue.push_back(c);
                            if (DEBUG_BUS) {
                                PRINT_SPD_PTC(ch, "{:10} -- REQ :: force enhanced pbr, rank={}, fst_bank={}, lst_bank={}",
                                    now(),
                                    rank,
                                    fst_bank_idx,
                                    lst_bank_idx);
                            ...
                            if (DEBUG_BUS) {
                                PRINT_SPD_PTC(ch, 
                                    "{:10} -- REQ_LP :: hold enhanced FPBR in lp, rank={}, fst_bank={}, lst_bank={}",
                                    now(),
                                    rank,
                                    fst_bank_idx,
                                    lst_bank_idx);
                        ...
                        if (((now() + 1) < bankStates[bank_tmp].state->nextPrecharge || issue_state[bank_tmp]) &&
                            bankStates[bank_tmp].state->currentBankState == RowActive) {
                            sb_ready_send_pre = false;
                        }
                        ...
                        if (sb_ready_send_pre && (bankStates[bank_tmp].state->currentBankState == RowActive)) {
                            ...
                            if (RankState[rank].lp_state == IDLE && even_cycle) {  // every other command, evan;
                                ...
                                pre_cmdqueue.push_back(c);
                                if (DEBUG_BUS) {
                                    PRINT_SPD_PTC(ch, "{:10} -- REQ ::  force ENHANCED PRECHARGE, rank={}, bank={}",
                                        now(),
                                        rank,
                                        bank_tmp);
                                }
                                ...
                                if (DEBUG_BUS) {
                                    PRINT_SPD_PTC(ch, 
                                        "{:10} -- REQ_LP :: hold ENAHNCED PRECHARGE PB/SB in lp state, rank{}, bank={}",
                                        now(),
                                        rank,
                                        bank_tmp);
                                }
        ...
        if (bankStates[bank_tmp].en_refresh_pb && bankStates[bank_tmp].hold_refresh_pb &&
            ...
            for (size_t matgrp = 0; matgrp < NUM_MATGRPS; matgrp++) {
                ...
                if (bankStates[fst_bank_idx].state->currentBankState != Idle ||
                        bankStates[lst_bank_idx].state->currentBankState !=
                        Idle || issue_state[fst_bank_idx] || issue_state[lst_bank_idx]) {
                    diff_bg_idle = false;
                }
                ...
                if (arb_enable && ((now() + 1) >= bankStates[fst_bank_idx].state->nextPerBankRefresh) &&
                    ...
                    if (RankState[rank].lp_state == IDLE && even_cycle && (!IS_HBM3 || now() % 2 == 1 || ODD_TDM)) {  // every other command, even;
                        ...
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, "{:10} -- REQ :: ENHANCED PBR IDLE, rank={}, fst_bank={}, lst_bank{}",
                                now(),
                                rank,
                                fst_bank_idx,
                                lst_bank_idx);
                    ...
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- ENHANCED PBR :: rank{}, fst_bank={}, lst_bank={}, arb_enable={}",
                            now(),
                            rank,
                            fst_bank_idx,
                            lst_bank_idx,
        ...
        case DRAMSim::ACTIVATE2_CMD: {
            if (IS_LP5 || IS_LP6) {
                ...
                if (cmd->sid == PreCmd.sid) {
                    if (cmd->group == PreCmd.group)
                        level_ret = 310;
                    else
                        level_ret = (tCCD_R > tCCD_L) ? 320 : 300;
                } else {
...
void PTC::scheduler()
{
    ...
    if (act_cmdqueue.empty() && rw_cmdqueue.empty() && pre_cmdqueue.empty() && CmdQueue.empty())
        return;
    ...
    if (CORE_CONCURR == 1) {
        ...
        for (auto &cmd : pre_cmdqueue) {
            ...
            if (priority_pri(cmd) > priority_pri(prec)) {
                ...
                if (lru_arb(cmd->bankIndex, prec->bankIndex, 1))
                    prec = cmd;
            }
        }
        if (DEBUG_BUS && prec != NULL) {
            PRINT_SPD_PTC(ch, "{:10} -- precmd_task={}", now(), prec->task);
        }
        if (prec != NULL)
        ...
        if (c->cmd_type == PRECHARGE_PB_CMD || c->cmd_type == PRECHARGE_SB_CMD)
            update_lru(c->bankIndex, 1, c);
        pregrt_cmd = c;
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- pregrt_cmd_task={}", now(), c->task);
        }
            ...
            for (auto &cmd : CmdQueue) {
                if (cmd->cmd_type >= WRITE_CMD && cmd->cmd_type <= READ_P_CMD) {
                    ...
                    if (priority_pri(cmd) > priority_pri(rwc)) {
                        ...
                        if (lru_arb(cmd->ptc_slot, rwc->ptc_slot, 0))
                            rwc = cmd;
                    }
                }
            }
    ...
    if (c->cmd_type >= WRITE_CMD && c->cmd_type <= READ_P_CMD) {
        ...
        if (PreCmd.trans_type != c->type) {
            rw_switch_cnt++;
            if (PreCmd.trans_type != DATA_READ && c->type == DATA_READ)
                w2r_switch_cnt++;
            else
                r2w_switch_cnt++;
            if (PRINT_EXEC) {
                ...
                for (size_t i = 0; i < NUM_RANKS; i++) {
                    DEBUGN(" R" << i << "R=" << +r_rank_cnt[i] << " R" << i << "W=" << +w_rank_cnt[i] << " R" << i
                                << "PR=" << +pfq_->rb_rank_cnt[i] << " R" << i << "PW=" << +pfq_->wb_rank_cnt[i] << " R"
                                << i << "PRR=" << +pfq_->rrel_rank_cnt[i] << " R" << i
                                << "PRW=" << +pfq_->wrel_rank_cnt[i]);
                }
                DEBUGN(" wcnt=" << que_write_cnt << " rcnt=" << que_read_cnt << " perf_wcnt=" << pfq_->wcmd_cnt
                                << " perf_rcnt=" << pfq_->rcmd_cnt << " rel_wcmd_cnt=" << pfq_->rel_wcmd_cnt
                                << " rel_rcmd_cnt=" << pfq_->rel_rcmd_cnt);
            }
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- EXEC :: [RW_SWITCH] type={} ser_rw_cnt={}", now(), c->type, ser_rw_cnt);
                for (size_t i = 0; i < NUM_RANKS; i++) {
                    PRINT_SPD_PTC(ch, " R{}R={} R{}W={} R{}PR={} R{}PW={} R{}PRR={} R{}PRW={}",
                        i,
                        +r_rank_cnt[i],
                        i,
                        +w_rank_cnt[i],
                        ...
                        +pfq_->wb_rank_cnt[i],
                        i,
                        +pfq_->rrel_rank_cnt[i],
                        i,
                        +pfq_->wrel_rank_cnt[i]);
                }
                PRINT_SPD_PTC(ch, " wcnt={} rcnt={} perf_wcnt={} perf_rcnt={} rel_wcmd_cnt={} rel_rcmd_cnt={}",
                    que_write_cnt,
                    que_read_cnt,
                    pfq_->wcmd_cnt,
                    pfq_->rcmd_cnt,
                    ...
                    for (size_t i = 0; i < NUM_RANKS; i++) {
                        for (size_t j = 0; j < NUM_BANKS; j++) {
                            ...
                            for (auto &r : pfq_->PerfQue) {
                                ...
                                if (r->rank != i)
                                    continue;
                                if ((r->bankIndex % NUM_BANKS) != j)
                                    continue;
                                Rcmd_Dist[i][j]++;
                            }
                            for (unsigned l = 0; l < transactionQueue.size(); l++) {
                                if (transactionQueue[l]->rank != i)
                                    continue;
                                if ((transactionQueue[l]->bankIndex % NUM_BANKS) != j)
                                    continue;
                                if (transactionQueue[l]->transactionType != DATA_READ)
                                    continue;
                                Rcmd_Dist[i][j]++;
                            }
                            PRINT_SPD_PTC(ch, "{:10} -- W2R, R{}_Ba{}={}", now(), i, j, Rcmd_Dist[i][j]);
                        }
                    }
                    ...
                    for (size_t i = 0; i < NUM_RANKS; i++) {
                        for (size_t j = 0; j < NUM_BANKS; j++) {
                            ...
                            for (unsigned l = 0; l < transactionQueue.size(); l++) {
                                ...
                                if ((transactionQueue[l]->bankIndex % NUM_BANKS) != j)
                                    continue;
                                if (transactionQueue[l]->transactionType != DATA_WRITE)
                                    continue;
                                Wcmd_Dist[i][j]++;
                            }
                            PRINT_SPD_PTC(ch, "{:10} -- R2W, R{}_Ba{}={}", now(), i, j, Wcmd_Dist[i][j]);
                        }
                    }
                }
            }
        ...
        if (PreCmd.rank != c->rank) {
            ...
            if (PreCmd.trans_type == DATA_READ && c->type == DATA_READ)
                r_rank_switch_cnt++;
            if (PreCmd.trans_type == DATA_WRITE && c->type == DATA_WRITE)
                w_rank_switch_cnt++;
            if (PRINT_EXEC) {
                DEBUGN(setw(10) << now() << " -- EXEC :: [RANK_SWITCH] rank=" << c->rank
                                << " ser_rank_cnt=" << ser_rank_cnt);
                for (size_t i = 0; i < NUM_RANKS; i++) {
                    DEBUGN(" R" << i << "R=" << +r_rank_cnt[i] << " R" << i << "W=" << +w_rank_cnt[i] << " R" << i
                                << "PR=" << +pfq_->rb_rank_cnt[i] << " R" << i << "PW=" << +pfq_->wb_rank_cnt[i] << " R"
                                << i << "PRR=" << +pfq_->rrel_rank_cnt[i] << " R" << i
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- EXEC :: [RANK_SWITCH] rank={} ser_rank_cnt={}", now(), c->rank, ser_rank_cnt);
                for (size_t i = 0; i < NUM_RANKS; i++) {
                    PRINT_SPD_PTC(ch, " R{}R={} R{}W={} R{}PR={} R{}PW={} R{}PRR={} R{}PRW={}",
                        i,
                        +r_rank_cnt[i],
        ...
        if (PreCmd.sid != c->sid) {
            ...
            if (PRINT_EXEC) {
                ...
                DEBUGN(" wcnt=" << que_write_cnt << " rcnt=" << que_read_cnt);
            }
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- EXEC :: [SID_SWITCH] ser_sid_cnt={}", now(), ser_sid_cnt);
                for (size_t i = 0; i < NUM_RANKS; i++) {
                    PRINT_SPD_PTC(ch, "{:10} -- SCH :: R{}R={} R{}W={}", now(), i, +r_rank_cnt[i], i, +w_rank_cnt[i]);
                }
                PRINT_SPD_PTC(ch, " wcnt={} rcnt={}", que_write_cnt, que_read_cnt);
            }
            ser_sid_cnt = 0;
        }
    ...
    switch (c->cmd_type) {
        case READ_CMD: {
            ...
            if ((IS_LP5 && c->bl == BL32) || (IS_LP6 && c->bl == BL48)) {
                for (auto &bp : bp_step)
                    bp_cycle.push_back(now() + bp);
            } else if (IS_DDR5) {
                for (size_t i = 1; i < tCCD_NSR; i++)
                    bp_cycle.push_back(now() + i + c->bl / 2 / unsigned(WCK2DFI_RATIO));
            }
            if (PRINT_SCH) {
                DEBUGN(setw(10) << now() << " -- SCH :: READ_CMD task=" << c->task << " QoS=" << c->pri
                                << " rank=" << c->rank << " sid=" << c->sid << " group=" << c->group
                                << " bank=" << c->bankIndex << " row=" << c->row << " trans_size=" << c->trans_size
                                << " addr_col=" << c->addr_col << " bl=" << c->bl);
                for (size_t i = 0; i < NUM_RANKS; i++) {
                    DEBUGN(" R" << i << "R=" << +r_rank_cnt[i] << " R" << i << "W=" << +w_rank_cnt[i]);
                }
                DEBUGN(endl);
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- SCH :: READ_CMD task={} QoS={} rank={} sid={} group={} bank={} row={} "
                              "trans_size={} addr_col={} bl={}",
                    now(),
                    c->task,
                    c->pri,
                    c->rank,
        ...
        case READ_P_CMD: {
            ...
            if (now() - bankStates[c->bankIndex].state->pageOpenTime >= tREFI) {
                page_exceed_cnt++;
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- PAGE_EXCEED :: bank={} pageOpenTime={} open_time={} tREFI={}",
                        now(),
                        c->bankIndex,
                        bankStates[c->bankIndex].state->pageOpenTime,
                        (now() - bankStates[c->bankIndex].state->pageOpenTime),
                        tREFI);
                }
            }
            if (PRINT_SCH) {
                DEBUGN(setw(10) << now() << " -- SCH :: READ_P_CMD task=" << c->task << " QoS=" << c->pri
                                << " rank=" << c->rank << " sid=" << c->sid << " group=" << c->group
                                << " bank=" << c->bankIndex << " row=" << c->row << " trans_size=" << c->trans_size
                                << " addr_col=" << c->addr_col << " bl=" << c->bl);
                for (size_t i = 0; i < NUM_RANKS; i++) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- SCH :: READ_P_CMD task={} QoS={} rank={} sid={} group={} bank={} row={} "
                              "trans_size={} addr_col={} bl={}",
                    now(),
                    c->task,
                    c->pri,
        ...
        case WRITE_CMD: {
            ...
            if ((IS_LP5 && c->bl == BL32) || (IS_LP6 && c->bl == BL48)) {
                ...
                for (size_t i = 1; i < tCCD_NSW; i++)
                    bp_cycle.push_back(now() + i + c->bl / 2 / unsigned(WCK2DFI_RATIO));
            }
            if (PRINT_SCH) {
                DEBUGN(setw(10) << now() << " -- SCH :: WRITE_CMD task=" << c->task << " QoS=" << c->pri
                                << " rank=" << c->rank << " sid=" << c->sid << " group=" << c->group
                                << " bank=" << c->bankIndex << " row=" << c->row << " trans_size=" << c->trans_size
                                << " addr_col=" << c->addr_col << " bl=" << c->bl);
                for (size_t i = 0; i < NUM_RANKS; i++) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- SCH :: WRITE_CMD task={} QoS={} rank={} sid={} group={} bank={} row={} "
                              "trans_size={} addr_col={} bl={}",
                    now(),
                    c->task,
                    c->pri,
        ...
        case WRITE_P_CMD: {
            ...
            if (PRINT_SCH) {
                DEBUGN(setw(10) << now() << " -- SCH :: WRITE_P_CMD task=" << c->task << " QoS=" << c->pri
                                << " rank=" << c->rank << " sid=" << c->sid << " group=" << c->group
                                << " bank=" << c->bankIndex << " row=" << c->row << " trans_size=" << c->trans_size
                                << " addr_col=" << c->addr_col << " bl=" << c->bl);
                for (size_t i = 0; i < NUM_RANKS; i++) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- SCH :: WRITE_P_CMD task={} QoS={} rank={} sid={} group={} bank={} row={} "
                              "trans_size={} addr_col={} bl={}",
                    now(),
                    c->task,
                    c->pri,
        ...
        case WRITE_MASK_CMD: {
            ...
            if (PRINT_SCH) {
                DEBUGN(setw(10) << now() << " -- SCH :: WRITE_MASK_CMD task=" << c->task << " QoS=" << c->pri
                                << " rank=" << c->rank << " sid=" << c->sid << " group=" << c->group
                                << " bank=" << c->bankIndex << " row=" << c->row << " trans_size=" << c->trans_size
                                << " addr_col=" << c->addr_col << " bl=" << c->bl);
                for (size_t i = 0; i < NUM_RANKS; i++) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- SCH :: WRITE_MASK_CMD task={} QoS={} rank={} sid={} group={} bank={} row={} "
                              "trans_size={} addr_col={} bl={}",
                    now(),
                    c->task,
                    c->pri,
        ...
        case WRITE_MASK_P_CMD: {
            ...
            if (PRINT_SCH) {
                DEBUGN(setw(10) << now() << " -- SCH :: WRITE_MASK_P_CMD task=" << c->task << " QoS=" << c->pri
                                << " rank=" << c->rank << " sid=" << c->sid << " group=" << c->group
                                << " bank=" << c->bankIndex << " row=" << c->row << " trans_size=" << c->trans_size
                                << " addr_col=" << c->addr_col << " bl=" << c->bl);
                for (size_t i = 0; i < NUM_RANKS; i++) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- SCH :: WRITE_MASK_P_CMD task={} QoS={} rank={} sid={} group={} bank={} row={} "
                              "trans_size={} addr_col={} bl={}",
                    now(),
                    c->task,
                    c->pri,
        ...
        case ACTIVATE1_CMD: {
            ...
            if (PRINT_SCH) {
                DEBUGN(setw(10) << now() << " -- SCH :: ACTIVATE1_CMD task=" << c->task << " QoS=" << c->pri
                                << " rank=" << c->rank << " sid=" << c->sid << " group=" << c->group
                                << " bank=" << c->bankIndex << " row=" << c->row << " matgrp=" << matgrp);
            }
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, 
                    "{:10} -- SCH :: ACTIVATE1_CMD task={} QoS={} rank={} sid={} group={} bank={} row={} matgrp={}",
                    now(),
                    c->task,
                    c->pri,
                    c->rank,
        ...
        case ACTIVATE2_CMD: {
            ...
            if (bankStates[c->bankIndex].state->lastPrechargeSource == 1 &&
            ...
            act_cmd_num++;
            for (auto &trans : transactionQueue) {
                if (trans->task != c->task)
                    continue;
                if (trans->transactionType != DATA_READ)
                    trans->trcd_met = now() + tRCD_WR;
                else
                    trans->trcd_met = now() + tRCD;
                break;
            }
            page_act_cnt++;
            ...
            for (auto &t : transactionQueue) {
                if (c->bankIndex != t->bankIndex)
                    continue;
                if (t->row == c->row)
                    bankStates[c->bankIndex].row_hit_cnt++;
                else
                    bankStates[c->bankIndex].row_miss_cnt++;
            }
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, 
                    "{:10} -- SCH :: ACTIVATE2_CMD task={} QoS={} rank={} sid={} group={} bank={} row={} matgrp={}",
                    now(),
                    c->task,
                    c->pri,
                    c->rank,
        ...
        case PRECHARGE_SB_CMD: {
            ...
            for (auto &trans : transactionQueue) {
                for (size_t sbr_bank = 0; sbr_bank < pbr_bg_num; sbr_bank++) {
                    unsigned bank_tmp = c->bankIndex + sbr_bank * pbr_bank_num;
                    if (bank_tmp == trans->bankIndex)
                        trans->act_executing = false;
                }
            }
            if (PRINT_SCH) {
                DEBUGN(setw(10) << now() << " -- SCH :: PRECHARGE_SB_CMD task=" << c->task << " QoS=" << c->pri
                                << " rank=" << c->rank << " sc=" << sub_channel << " sid=" << c->sid
                                << " group=" << c->group << " bank=" << c->bankIndex
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, 
                    "{:10} -- SCH :: PRECHARGE_SB_CMD task={} QoS={} rank={} sc={} sid={} group={} bank={} POSTPND={}",
                    now(),
                    c->task,
                    c->pri,
                    c->rank,
        ...
        case PRECHARGE_PB_CMD: {  // todo: revise for enh_send_precharge
            ...
            bankStates[c->bankIndex].has_dummy_tout = false;
            for (auto &trans : transactionQueue) {
                if (c->bankIndex == trans->bankIndex)
                    trans->act_executing = false;
            }
            if (c->cmd_source == 0)
                rowconf_pre_cnt[c->bankIndex]++;
            else if (c->cmd_source == 1)
                pageto_pre_cnt[c->bankIndex]++;
            else if (c->cmd_source == 2)
                func_pre_cnt[c->bankIndex]++;
            else if (c->cmd_source == 3)
                rhit_break_pre_cnt[c->bankIndex]++;
            else if (c->cmd_source == 4)
                dummy_tout_pre_cnt[c->bankIndex]++;
            if (c->cmd_source == 2) {
            ...
            if (PRINT_SCH) {
                DEBUGN(setw(10) << now() << " -- SCH :: PRECHARGE_PB_CMD task=" << c->task << " QoS=" << c->pri
                                << " rank=" << c->rank << " sc=" << sub_channel << " sid=" << c->sid << " group="
                                << c->group << " bank=" << c->bankIndex << " row=" << c->row << " matgrp=" << matgrp
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- SCH :: PRECHARGE_PB_CMD task={} QoS={} rank={} sc={} sid={} group={} bank={} "
                              "row={} matgrp={} POSTPND_AB={}",
                    now(),
                    c->task,
                    c->pri,
                    c->rank,
        ...
        case PRECHARGE_AB_CMD: {
            ...
            for (size_t bank = 0; bank < NUM_BANKS; bank++) {
                ...
                unsigned sub_channel = bank / sc_bank_num;
                if (EM_ENABLE && EM_MODE == 2 && c->rank == 1 && sub_channel == 1)
                    continue;  // rank1, sc1 forbidden under combo e-mode
                access_bank_delay[bank_tmp].enable = false;
                access_bank_delay[bank_tmp].cnt = 0;
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, 
                    "{:10} -- SCH :: PRECHARGE_AB_CMD task={} QoS={} rank={} sc={} sid={} group={} bank={} POSTPND={}",
                    now(),
                    c->task,
                    c->pri,
                    c->rank,
        ...
        case REFRESH_CMD: {
            ...
            if (PRINT_SCH) {
                DEBUGN(setw(10) << now() << " -- SCH :: REFRESH_CMD task=" << c->task << " QoS=" << c->pri
                                << " rank=" << c->rank << " sc=" << sub_channel
                                << " POSTPND=" << refreshALL[c->rank][sub_channel].refresh_cnt);
                for (size_t rank = 0; rank < NUM_RANKS; rank++) {
                    DEBUGN(
                        " rank" << rank << "r=" << +r_rank_cnt[rank] << " rank" << rank << "w=" << +w_rank_cnt[rank]);
                }
                DEBUGN(endl);
            }
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- SCH :: REFRESH_CMD task={} QoS={} rank={} sc={} POSTPND={}",
                    now(),
                    c->task,
                    c->pri,
                    c->rank,
                    ...
                    refreshALL[c->rank][sub_channel].refresh_cnt);
                for (size_t rank = 0; rank < NUM_RANKS; rank++) {
                    PRINT_SPD_PTC(ch, " rank{}r={} rank{}w={}", rank, +r_rank_cnt[rank], rank, +w_rank_cnt[rank]);
                }
            }
            break;
        }
        case PER_BANK_REFRESH_CMD: {
            if (ENH_PBR_EN) {
                if ((c->fst_bankIndex % NUM_BANKS) >= NUM_BANKS || (c->lst_bankIndex % NUM_BANKS) >= NUM_BANKS) {
                    ERROR(setw(10) << now() << " -- DMC[" << channel << "] ENHANCED PER_BANK_REFRESH_CMD, rank="
                                   << c->rank << " sid=" << c->sid << ", bank=" << c->bankIndex
                                   << ", fst_bank=" << c->fst_bankIndex << ", lst_bank=" << c->lst_bankIndex);
                    assert(0);
                }
                refresh_pb_cnt++;
                ...
                if (PRINT_SCH) {
                    DEBUGN(setw(10) << now() << " -- SCH :: ENHANCED PER_BANK_REFRESH_CMD task=" << c->task << " rank="
                                    << c->rank << " sc=" << sub_channel << " sid=" << c->sid << " bank=" << c->bankIndex
                                    << " fst_bank=" << c->fst_bankIndex << " lst_bank=" << c->lst_bankIndex
                                    << " POSTPND=" << refreshALL[c->rank][sub_channel].refresh_cnt);
                    for (size_t i = 0; i < NUM_RANKS; i++) {
                        DEBUGN(" R" << i << "R=" << +r_rank_cnt[i] << " R" << i << "W=" << +w_rank_cnt[i]);
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- SCH :: ENHANCED PER_BANK_REFRESH_CMD task={} rank={} sc={} sid={} bank={} "
                                  "fst_bank={} lst_bank={} POSTPND={}",
                        now(),
                        c->task,
                        c->rank,
                        sub_channel,
                ...
                refresh_pb_cnt++;
                perbank_refresh_cnt[c->rank * pbr_bank_num * sc_num + c->bankIndex % pbr_bank_num +
                                    sub_channel * bank_pair_start]++;  // todo: revise for e-mode
                for (size_t sbr_bank = 0; sbr_bank < pbr_bg_num; sbr_bank++) {
                    unsigned bank_tmp = c->bankIndex + sbr_bank * pbr_bank_num;
                    access_bank_delay[bank_tmp].enable = false;
                    access_bank_delay[bank_tmp].cnt = 0;
                ...
                if (PRINT_SCH) {
                    DEBUGN(setw(10) << now() << " -- SCH :: PER_BANK_REFRESH_CMD task=" << c->task << " rank="
                                    << c->rank << " sc=" << sub_channel << " sid=" << c->sid << " bank=" << c->bankIndex
                                    << " POSTPND=" << refreshALL[c->rank][sub_channel].refresh_cnt);
                    for (size_t i = 0; i < NUM_RANKS; i++) {
                        DEBUGN(" R" << i << "R=" << +r_rank_cnt[i] << " R" << i << "W=" << +w_rank_cnt[i]);
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- SCH :: PER_BANK_REFRESH_CMD task={} rank={} sc={} sid={} bank={} POSTPND={}",
                        now(),
                        c->task,
                        c->rank,
                        sub_channel,
    ...
    if ((c != NULL) && !SLOT_FIFO && c->cmd_type >= WRITE_CMD && c->cmd_type <= READ_P_CMD &&
        c->issue_size + c->trans_size >= c->data_size) {
        arb_group_cnt[c->arb_group]--;
        if (arb_group_cnt[c->arb_group] > arb_per_group) {
            ERROR(setw(10) << now() << " Wrong Arb Group Cnt, arb_group=" << c->arb_group
                           << ", cnt=" << arb_group_cnt[c->arb_group]);
            assert(0);
        }
    }
...
void PTC::update_arb_group_pri(Cmd *c, vector<unsigned> &arb_group_pri, unsigned arb_type)
{
    ...
    if (DEBUG_BUS) {
        PRINT_SPD_PTC(ch, "{:10} -- ARB_CMD :: Arb_Group={} task={} cmd_type={} QoS={} rank={} sid={} group={} bank={} "
                      "row={} trans_size={} addr_col={} bl={}",
            now(),
            c->arb_group,
            c->task,
            c->cmd_type,
    ...
    if (DEBUG_BUS) {
        for (size_t arb_grp_idx = 0; arb_grp_idx < ARB_GROUP_NUM; arb_grp_idx++) {
            PRINT_SPD_PTC(ch, "{:10} -- Updated Arb_Group_Pri={} Arb_Group={} Arb_Type={}",
                now(),
                arb_group_pri[arb_grp_idx],
                arb_grp_idx,
                arb_type);
        }
    }
...
void PTC::faw_update()
{
    for (size_t i = 0; i < NUM_RANKS; i++) {
        if (EM_ENABLE) {
            ...
            for (size_t j = 0; j < tFAWCountdown_sc1[i].size(); j++)
                tFAWCountdown_sc1[i][j]--;
            // tfaw the head will always be the smallest counter, so check if it has reached 0
            if (tFAWCountdown_sc1[i].size() > 0 && tFAWCountdown_sc1[i][0] == 0)
                tFAWCountdown_sc1[i].erase(tFAWCountdown_sc1[i].begin());
        } else {
            // tfaw decrement all the counters we have going
            for (size_t j = 0; j < tFAWCountdown[i].size(); j++)
                tFAWCountdown[i][j]--;
    ...
    for (size_t i = 0; i < NUM_RANKS; i++) {
        ...
        for (size_t j = 0; j < tFPWCountdown[i].size(); j++)
            tFPWCountdown[i][j]--;
        // tfpw the head will always be the smallest counter, so check if it has reached 0
        if (tFPWCountdown[i].size() > 0 && tFPWCountdown[i][0] == 0)
            tFPWCountdown[i].erase(tFPWCountdown[i].begin());
    }
}
...
void PTC::update_statistics()
{
    ...
    if (PRINT_BW && PRINT_BW_WIN != 0 && now() != 0 && now() % PRINT_BW_WIN == 0) {
        ...
        // TRACE_PRINT(", Bank: "<<setw(2)<<bw_bank_cnt);
    }
}
...
void PTC::update_state()
{
    ...
    PRINT_SPD_PTC(ch, "--------------------------------------------------------------------------------------------------")
    PRINT_SPD_PTC(ch, 
        "Total Status: R:{} W:{} rw_group_state={} | in_write_group={} | availability={} | row_hit_ratio={} | "
        "PreCmd.rank={} | PreCmd.type={} | rk_grp_state={} | real_rk_grp_state={}{} | bank={} | sc={} | Rcnt={} | "
        "Wcnt={} | state={} | OpenRowAddr={} | nAct1={} | nAct2={} | nR={} | nW={} | nWRmw={} | nMW={} | nRAp={} | "
        "nWAp={} | nWApRmw={} | nMWAp={} | nPre={} | hPre={} | lastCmd={} | lastCmdType={} | lastCmdPri={} | "
        "lastCmdSource={} | HasCmdRhit={} | perf_rd_rowhit={} | perf_rd_rowmiss={} | perf_wr_rowhit={} | "
        "perf_wr_rowmiss={} | act_met={} | samebank_rcnt={} | samebank_wcnt={} | ptc_samebank_rd={} | "
        "ptc_samebank_wr={} | perf_rd_conf={} | perf_wr_conf={} | rhit_brk={} | dummy_tout={} | ser_rhit_cnt={} | "
        "row_hit_cnt={} | row_miss_cnt={} | pbrW={} | pbr={} | pbr_pre={} | abrW={} | abr={} | abrW_sc1={} | "
        "abr_sc1={} | stc_cnt={} | stc_en={} | POSTPND={} | POSTPND_SC1={} | Fpbr={} | lp_state={} | rank_has_cmd={} | "
        "acc_en={} | acc_cnt={} | adpt_openpage_time={} | opc_cnt={} | ppc_cnt={} | act_executing={}",
        que_read_cnt,
        que_write_cnt,
        +rw_group_state[0],
        in_write_group,
    ...
    for (auto &state : bankStates) {
        unsigned sub_channel = (state.bankIndex % NUM_BANKS) / sc_bank_num;
        if (EM_ENABLE) {
            PRINT_SPD_PTC(ch, "ST time: ",
                now(),
                state.bankIndex,
                sub_channel,
                +r_bank_cnt[state.bankIndex],
                +w_bank_cnt[state.bankIndex],
                bank_state_opcode(state.state->currentBankState),
                state.state->openRowAddress,
                state.state->nextActivate1,
                state.state->nextActivate2,
                ...
                state.state->nextPrecharge,
                state.hold_precharge,
                state.state->lastCommand,
                state.state->lastCmdType,
                state.state->lastCmdPri,
                state.state->lastCmdSource,
                state.has_cmd_rowhit,
                state.perf_rd_rowhit,
                state.perf_rd_rowmiss,
                state.perf_wr_rowhit,
                state.perf_wr_rowmiss,
                state.act_timing_met,
                state.samebank_rcnt,
                state.samebank_wcnt,
                state.ptc_samebank_rd,
                state.ptc_samebank_wr,
                state.perf_bankrd_conflict,
                state.perf_bankwr_conflict,
                state.has_rhit_break,
                state.has_dummy_tout,
                state.ser_rhit_cnt,
                state.row_hit_cnt,
                state.row_miss_cnt,
                refreshPerBank[state.bankIndex].refreshWaiting,
                refreshPerBank[state.bankIndex].refreshing,
                refreshPerBank[state.bankIndex].refreshWaitingPre,
                refreshALL[state.rank][0].refreshWaiting,
                refreshALL[state.rank][0].refreshing,
                refreshALL[state.rank][1].refreshWaiting,
                refreshALL[state.rank][1].refreshing,
                state.state->stateChangeCountdown,
                state.state->stateChangeEn,
                refreshALL[state.rank][0].refresh_cnt,
                refreshALL[state.rank][1].refresh_cnt,
                state.finish_refresh_pb,
                RankState[state.rank].lp_state,
                rank_has_cmd[state.rank],
                access_bank_delay[state.bankIndex].enable,
                access_bank_delay[state.bankIndex].cnt,
                adpt_openpage_time,
                opc_cnt,
                ppc_cnt,
                state.state->act_executing);
        } else {
            PRINT_SPD_PTC(ch, 
                "ST time: {} | bank={} | sc={} | Rcnt={} | Wcnt={} | state={} | OpenRowAddr={} | nAct1={} | nAct2={} | "
                "nR={} | nW={} | nWRmw={} | nMW={} | nRAp={} | nWAp={} | nWApRmw={} | nMWAp={} | nPre={} | hPre={} | "
                "lastCmd={} | lastCmdType={} | lastCmdPri={} | lastCmdSource={} | HasCmdRhit={} | perf_rd_rowhit={} | "
                "perf_rd_rowmiss={} | perf_wr_rowhit={} | perf_wr_rowmiss={} | act_met={} | samebank_rcnt={} | "
                "samebank_wcnt={} | ptc_samebank_rd={} | ptc_samebank_wr={} | perf_rd_conf={} | perf_wr_conf={} | "
                "rhit_brk={} | dummy_tout={} | ser_rhit_cnt={} | row_hit_cnt={} | row_miss_cnt={} | pbrW={} | pbr={} | "
                "pbr_pre={} | abrW={} | abr={} | stc_cnt={} | stc_en={} | POSTPND={} | Fpbr={} | lp_state={} | "
                "rank_has_cmd={} | acc_en={} | acc_cnt={} | adpt_openpage_time={} | opc_cnt={} | ppc_cnt={} | "
                "act_executing={}",
                now(),
                state.bankIndex,
                sub_channel,
    ...
    PRINT_SPD_PTC(ch, "--------------------------------------------------------------------------------------------------")
    PRINT_SPD_PTC(ch, "Command Queue Status: Qsize={} | QRcnt:{} | QWcnt:{}{} | task={} | bank={} | rank={} | row={} | "
                  "addr_col={} | arb_time={} | type={} | nextCmd={} | arb_grp={} | arb_grp_pri_col={} | "
                  "arb_grp_pri_row={} | arb_grp_cnt={} | address={:x} | length={} | data_size={} | issue_size={} | "
                  "data_ready_cnt={} | timeout={} | bp_by_tout={} | qos={} | pri={} | addrconf={} | timeout_th={} | "
                  "ptc_timeAdded={} | pri_adapt_th={} | timeAdded={} | act_executing={} | pre_act={} mask_wcmd={}{}",
        transactionQueue.size(),
        que_read_cnt,
        que_write_cnt)
    for (auto &trans : transactionQueue) {
        PRINT_SPD_PTC(ch, "Que time: ",
            now(),
            trans->task,
            trans->bankIndex,
            trans->rank,
            ...
            trans->addr_col,
            trans->arb_time,
            trans_type_opcode(trans->transactionType),
            trans_cmd_opcode(trans->nextCmd),
            trans->arb_group,
            arb_group_pri_col[trans->arb_group],
            arb_group_pri_row[trans->arb_group],
            arb_group_cnt[trans->arb_group],
            trans->address,
            trans->burst_length,
            trans->data_size,
            trans->issue_size,
    ...
    if (ENH_PAGE_ADPT_EN && now() % ENH_PAGE_ADPT_WIN == 0 && now() != 0) {
        PRINT_SPD_PTC(ch, "EHS_PAGE_R: ", now());
        for (size_t i = 0; i < NUM_RANKS * NUM_BANKS; i++)
            PRINT_SPD_PTC(ch, " | R{}= {:3}{}", i, page_timeout_rd[i]);

        PRINT_SPD_PTC(ch, "EHS_PAGE_W: {}", now());
        for (size_t i = 0; i < NUM_RANKS * NUM_BANKS; i++)
            PRINT_SPD_PTC(ch, " | R{}= {:3}{}", i, page_timeout_wr[i]);
    }
    PRINT_SPD_PTC(ch, 
        "{}", "--------------------------------------------------------------------------------------------------")
}
...
void PTC::rank_group_weight(unsigned *rank, unsigned *type)
{
    ...
    for (size_t i = 0; i < NUM_RANKS; i++) {
        ...
        if (real_rk_grp_state != NO_RGRP && unsigned(real_rk_grp_state >> 1) == i) {
            if (*r_rank_mux[i] != 0)
                weight[i * NUM_RANKS] += 4;
            if (*w_rank_mux[i] != 0)
                weight[i * NUM_RANKS + 1] += 4;
        }
    }
...
BankTableState PTC::get_bank_state(unsigned bankIndex)
{
    ...
    // no match bankState
    ERROR(setw(10) << now() << " -- Error parameter get_bank_state, bankIndex=" << bankIndex
                   << ", num_banks=" << (NUM_RANKS * NUM_BANKS));
    assert(0);
}
...
bool PTC::get_bs_full(Transaction *trans)
{
    ...
    if ((BANK_SLOT_NUM - bs_idle) > bs_max)
        bs_max = (BANK_SLOT_NUM - bs_idle);
...
unsigned PTC::get_bank_cnt(unsigned bankIndex, bool isRd)
{
    if (bankIndex >= (NUM_RANKS * NUM_BANKS)) {
        ERROR(setw(10) << now() << " -- Error parameter get_bank_cnt, bankIndex=" << bankIndex
                       << ", num_banks=" << (NUM_RANKS * NUM_BANKS));
        assert(0);
    }
...
{
    if (bankIndex >= (NUM_RANKS * NUM_BANKS)) {
        ERROR(setw(10) << now() << " -- Error parameter get_refresh_state, bankIndex=" << bankIndex
                       << ", num_banks=" << (NUM_RANKS * NUM_BANKS));
        assert(0);
    }
...
void PTC::queue_update()
{
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (t->pre_act || (t->fast_rd && t->act_only)) {
            ...
            if (DEBUG_BUS) {
                if (!act_type) {
                    PRINT_SPD_PTC(ch, "{:10} -- DELETE PRE_ACT :: task={}, bank={}, row{}, pre_act={}",
                        now(),
                        t->task,
                        t->bankIndex,
                        t->row,
                    ...
                    PRINT_SPD_PTC(ch, "{:10} -- DELETE FAST_READ_ACT :: task={}, bank={}, row{}, byp_act={}",
                        now(),
                        t->task,
                        t->bankIndex,
                        t->row,
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (t->issue_size < t->data_size) {
            // if (DEBUG_BUS) {
            // PRINT_SPD_PTC(ch, "{:10} -- issue_size < data_size{},{}", now(), t->issue_size, t->data_size);
            // }
            continue;
        }
        if (t->issue_size > t->data_size) {
            ERROR(setw(10) << now() << " -- Error issue_size, task=" << t->task << " data_size=" << t->data_size
        ...
        if (t->transactionType == DATA_READ) {
            ...
            for (size_t j = i + 1; j < len; j++) {
                ...
                if (tmp->addrconf && ((tmp->address & ~ALIGNED_SIZE) == (t->address & ~ALIGNED_SIZE)) &&
                    ...
                    tmp->addrconf = false;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- RL_CONF :: release ,task={}", now(), tmp->task);
                    }
                }
            }
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- perf_release_wr :: release ,task={}", now(), t->task);
            }
        ...
        if (WRITE_BUFFER_ENABLE)
            pfq_->dmc_release_conflict(t);
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- DELETE :: task={}, bank={}, rowhit={}, fast_rd={}",
                now(),
                t->task,
                t->bankIndex,
                !t->act_executing,
        ...
        if (t->timeout && !t->dummy_push) {
            ...
            if (((now() - t->time_timeout) * tDFI) <= TOUT_SCH_TIME) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- Unqualified Timeout :: task={}, bank={}, rank={}, time_timeout={}, tDFI={}",
                        now(),
                        t->task,
                        t->bankIndex,
                        t->rank,
        ...
        if (ENH_PAGE_ADPT_EN && !t->has_send_act)
            bank_cnt_ehs[t->bankIndex]++;
    ...
    for (auto &state : bankStates) {
        ...
        if ((now() + ACT_LEFT_CYCLE) >= state.state->nextActivate1) {
            ...
            // if (DEBUG_BUS) {
            // PRINT_SPD_PTC(ch, "{:10} -- Trp Timing Met, rank={}, group={}, bank={}, bankIndex={}, state={}", now(),
            // state.rank, state.group
            //, state.bank, state.bankIndex, bank_state_opcode(state.state->currentBankState));
            // }
        }
    }
}
...
void PTC::bsc_update()
{
    ...
    if (BSC_EN) {
        for (auto &state : bankStates) {
            if (now() >= state.state->nextActivate2 &&
                (state.state->lastCommand == PRECHARGE_SB_CMD || state.state->lastCommand == PRECHARGE_PB_CMD ||
                    state.state->lastCommand == PRECHARGE_AB_CMD)) {
                bool sameidx_in_ptc = false;
                for (size_t i = 0; i < transactionQueue.size(); i++) {
                    if (state.bankIndex == transactionQueue[i]->bankIndex) {
                        sameidx_in_ptc = true;
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, "{:10} -- PTC QUE still have same bank cmd, rank={}, group={}, bank={}, "
                                          "bankIndex={}, state={} nextActivate2_time={} task={} i={}",
                                now(),
                                state.rank,
                                state.group,
                                state.bank,
                ...
                if (!sameidx_in_ptc) {
                    for (size_t j = 0; j < bank_slot.size(); j++) {
                        if (bank_slot[j] == state.bankIndex) {
                            bank_slot[j] = 0xFFFF;
                            if (DEBUG_BUS) {
                                PRINT_SPD_PTC(ch, "{:10} -- PTC QUE do not have same bank cmd recycle slot, rank={}, "
                                              "group={}, bank={}, bankIndex={}, state={} nextActivate2_time={}",
                                    now(),
                                    state.rank,
                                    state.group,
                                    state.bank,
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- BANK SLOT after update_que:: ", now());
            for (unsigned i = 0; i < bank_slot.size(); i++) {
                if (i != 0 && i % 4 == 0) {
                    PRINT_SPD_PTC(ch, "    ");
                }
                PRINT_SPD_PTC(ch, "{} ", bank_slot[i]);
            }
        }
    }
}
void PTC::sch_pipe()
{
    ...
    for (auto &trans : transactionQueue) {
        if (trans->issue_size != 0)
            rw_exec_cnt++;
        // if (DEBUG_BUS) {
        // PRINT_SPD_PTC(ch, "{:10} -- Trans issue_size={}, task={}", now(), trans->issue_size, trans->task);
        // }
    }
    ...
    for (auto &t : transactionQueue) {
        ...
        if (t->bp_by_tout)
            continue;
        if (refreshALL[t->rank][sub_channel].refreshing)
            continue;  // todo: revise for e-mode
        ...
        if (bankStates[t->bankIndex].state->currentBankState == RowActive &&
            ...
            if (rw_group_state[0] == READ_GROUP && t->qos >= PERF_SWITCH_HQOS_LEVEL &&
                t->transactionType == DATA_READ) {
    ...
    for (auto &trans : transactionQueue) {
        if ((now() - trans->timeAdded) > 1000000 || (now() - trans->ptc_timeAdded) > 100000) {
            ERROR(setw(10) << now() << " -- task=" << trans->task << " address=" << hex << trans->address << dec
                           << " rank=" << trans->rank << " bank=" << trans->bankIndex << " row=" << trans->row
                           << " matgrp=" << (trans->row & (NUM_MATGRPS - 1)));
            ERROR(setw(10) << now() << " -- error, qos=" << trans->qos << ", pri=" << trans->pri);
            ERROR(setw(10) << now() << " -- FATAL ERROR == big latency"
                           << ", chnl:" << channel);
            assert(0);
        }
        ...
        if (trans->transactionType != DATA_READ) {
            if (now() - trans->ptc_timeAdded > 100000 && trans->data_ready_cnt <= trans->burst_length) {
                ERROR(setw(10) << now() << " -- DMC[" << channel << "] task=" << trans->task
                               << " Wdata number miss match, EXP=" << trans->burst_length
                               << ", ACT=" << trans->data_ready_cnt);
                assert(0);
            }
        }
    }
    ...
    for (auto &trans : transactionQueue) {
        ...
        if (trans->rank == PreCmd.rank)
            continue;
        if (trans->transactionType != PreCmd.trans_type)
            continue;
        if (trans->addrconf)
            continue;
        if (trans->pre_act)
            continue;
        if (trans->fast_rd && trans->act_only)
            continue;
        //        if (now() < trans->enter_que_time) continue;
        if (refreshALL[trans->rank][sub_channel].refreshing)
            continue;
        if (refreshPerBank[trans->bankIndex].refreshing)
            continue;
        if (trans->bp_by_tout)
            continue;
        if (r_rank_cnt[trans->rank] < PTC_RRANK_SWITCH_LEVEL && trans->transactionType == DATA_READ)
            continue;
        if (w_rank_cnt[trans->rank] < PTC_WRANK_SWITCH_LEVEL && trans->transactionType == DATA_WRITE)
            continue;
        if (trans->transactionType == DATA_READ)
            has_other_rank_cmd = true;
    ...
    if (FAST_HQOS_PUSH_EN && FAST_HQOS_PUSH_MODE == 2) {
        for (auto &trans : transactionQueue) {
            ...
            if (!trans->hqos_push)
                continue;
            if (PreCmd.trans_type != trans->transactionType)
                continue;
    ...
    if (PTC_HQOS_RANK_SWITCH_EN) {
        ...
        for (size_t i = 0; i < NUM_RANKS; i++) {
            if (rank_cmd_high_qos[i])
                rank_cmd_high_qos[i] = que_read_highqos_cnt[i] >= PERF_RCMD_HQOS_RANK_SWITCH_LEVELL;
            else
                rank_cmd_high_qos[i] = que_read_highqos_cnt[i] >= PERF_RCMD_HQOS_RANK_SWITCH_LEVELH;
            if (rank_cnt[i] < PTC_HQOS_RANK_SWITCH_LEVEL1 && !rank_cmd_high_qos[i]) {
                all_rank_has_cmd = false;
            }
            for (size_t i = 0; i < NUM_RANKS; i++) {
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- read trig,rank_cmd_high_qos={},rank={},que_read_highqos_cnt={}",
                        now(),
                        rank_cmd_high_qos[i],
                        i,
                        que_read_highqos_cnt[i]);
                }
            }
        }
        for (auto &trans : transactionQueue) {
            if (trans->transactionType == DATA_WRITE)
                continue;
            if (trans->pri < PERF_SWITCH_HQOS_LEVEL)
                continue;
            rank_cmd_high_qos[trans->rank] = 1;  // write cmd or read cmd was hqos pushed ,both need mark hqos rank
            for (size_t i = 0; i < NUM_RANKS; i++) {
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- hqos push trig,rank_cmd_high_qos={},rank={},que_read_highqos_cnt={}",
                        now(),
                        rank_cmd_high_qos[i],
                        i,
                        que_read_highqos_cnt[i]);
        ...
        if (hqos_rank != 0xffffffff && PTC_HQOS_RANK_SWITCH_MODE == 0) {
            ...
            for (auto &trans : transactionQueue) {
                ...
                if (trans->rank == hqos_rank) {
                    ...
                    trans->pri = PTC_QOS_MAX;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- Label Timeout for Hqos, task={}, rank={}, bankIndex={}, hqos_rank={}",
                            now(),
                            trans->task,
                            trans->rank,
                            trans->bankIndex,
    ...
    if (FAST_READ_EN && FAST_READ_MODE == 1 && FAST_RD_CANCEL_EN) {
        for (auto &trans : transactionQueue) {
            ...
            if (pfq_->perf_conflict_intf(trans->task)) {
                ...
                total_fast_act_cnt++;
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} Fast Rd Cancel Success, task={}", now(), trans->task);
                }
                ...
                if (trans->data_size == 32)
                    TotalDmcRd32B--;
                else if (trans->data_size == 64)
                    TotalDmcRd64B--;
                else if (trans->data_size == 128)
                    TotalDmcRd128B--;
                else if (trans->data_size == 256)
                    TotalDmcRd256B--;
                if ((trans->address % trans->data_size) == 0)
                    rd_inc_cnt--;
                else
                    rd_wrap_cnt--;
                if (trans->qos >= PERF_SWITCH_HQOS_LEVEL) {
    ...
    if (!SLOT_FIFO) {
        if (DEBUG_BUS) {
            for (size_t arb_grp_idx = 0; arb_grp_idx < ARB_GROUP_NUM; arb_grp_idx++) {
                PRINT_SPD_PTC(ch, "{:10} -- Current Col Arb_Group_Pri={} Arb_Group={}",
                    now(),
                    arb_group_pri_col[arb_grp_idx],
                    arb_grp_idx);
                PRINT_SPD_PTC(ch, "{:10} -- Current Row Arb_Group_Pri={} Arb_Group={}",
                    now(),
                    arb_group_pri_row[arb_grp_idx],
                    arb_grp_idx);
            }
        }
        ...
        Cmd *rwc = NULL;
        for (auto &cmd : rw_cmdqueue) {
            if (prerw.task != uint64_t(-1)) {
                if (cmd->task == prerw.task && cmd->cmd_type == prerw.cmd_type && cmd->issue_size == prerw.issue_size) {
                    if (DEBUG_BUS && rwc != NULL) {
                        PRINT_SPD_PTC(ch, "{:10} -- has same cmd    ,task={} type={} issue_size={}",
                            now(),
                            cmd->task,
                            cmd->cmd_type,
                            cmd->issue_size);
                        PRINT_SPD_PTC(ch, "{:10} -- has same cmd pre,task={} type={} issue_size={}",
                            now(),
                            prerw.task,
                            prerw.cmd_type,
                            prerw.issue_size);
            ...
            if (DEBUG_BUS&&rwc!=NULL) {
                PRINT_SPD_PTC(ch, "{:10} -- rw_cmdque rwc,task={} type={} issue_size={}", now(), rwc->task,
                rwc->cmd_type, rwc->issue_size, rwc->ptc_slot); 
                PRINT_SPD_PTC(ch, "{:10} -- rw_cmdque cmd,task={} type={} issue_size={}", now(), cmd->task,
                cmd->cmd_type, cmd->issue_size, cmd->ptc_slot);
            }
            if (priority_pri(cmd) > priority_pri(rwc)) {
                rwc = cmd;
            } else if (priority_pri(cmd) == priority_pri(rwc)) {
            ...
            // if (DEBUG_BUS&&rwc!=NULL) {
            // PRINT_SPD_PTC(ch, "{:10} -- rw_cmdque win the arg,task={} type={} issue_size={}", now(), rwc->task,
            // rwc->cmd_type, rwc->issue_size); PRINT_SPD_PTC(ch, "{:10} -- rw_cmdque pop to pre,task={} type={}
            // issue_size={}", now(), prerw.task, prerw.cmd_type, prerw.issue_size);
            // }
        }
        // if (DEBUG_BUS&&rwc!=NULL) {
        //     PRINT_SPD_PTC(ch, "{:10} -- rw_cmdque pop to delay_buf,task={} type={} issue_size={}", now(), rwc->task,
        //     rwc->cmd_type, rwc->issue_size); PRINT_SPD_PTC(ch, "{:10} -- rw_cmdque pop to pre,task={} type={}
        //     issue_size={}", now(), prerw.task, prerw.cmd_type, prerw.issue_size);
        // }
        if (rwc != NULL) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- CmdQueue_size={}", now(), CmdQueue.size());
        }
    }
}
            ...
            if (arb_group_pri[j] == ((ARB_GROUP_NUM - 1) - i)) {
                for (auto &trans : transactionQueue) {
                    if (trans->arb_group == j) {
                        ...
                        if (arb_type == 0 && bankStates[trans->bankIndex].state->currentBankState != RowActive)
                            continue;
                        else if (arb_type == 1 && bankStates[trans->bankIndex].state->currentBankState == RowActive)
                            continue;
...
void PTC::ptc_constraint_check()
{
    ...
    for (auto &state : bankStates) {
        ...
        for (auto &trans : transactionQueue) {
            if (state.bankIndex != trans->bankIndex)
                continue;
            if (trans->row != state.state->openRowAddress) {
                ...
                if (samebank_rowmiss_cnt > SAME_BANK_CNT_ROWMISS && SAME_BANK_CNT_ROWMISS != 0) {
                    ERROR(setw(10) << now() << " row_miss cmd forbidden in PTC, task=" << trans->task
                                   << ", bankIndex=" << trans->bankIndex << ", row=" << trans->row
                                   << ", openrow=" << state.state->openRowAddress);
                    assert(0);
                }
            } else {
                if (trans->transactionType == DATA_READ) {
                    if (!((trans->timeout && trans->timeout_type == 0) ||
                            trans->hqos_timeout)) {  // perf timeout/perf hqos pesudo timeout exclude
                ...
                if (samebank_rd_rowhit_cnt > SAME_BANK_CNT_RD && SAME_BANK_CNT_RD != 0) {
                    ERROR(setw(10) << now() << " rd_row_hit cmd exceed 2 in PTC, task=" << trans->task
                                   << ", bankIndex=" << trans->bankIndex << ", row=" << trans->row
                                   << ", openrow=" << state.state->openRowAddress);
                    assert(0);
                } else if (samebank_wr_rowhit_cnt > SAME_BANK_CNT_WR && SAME_BANK_CNT_WR != 0) {
                    ERROR(setw(10) << now() << " wr_row_hit cmd exceed 2 in PTC, task=" << trans->task
                                   << ", bankIndex=" << trans->bankIndex << ", row=" << trans->row
                                   << ", openrow=" << state.state->openRowAddress);
                    assert(0);
                }
    ...
    for (size_t rank = 0; rank < NUM_RANKS; rank++) {
        ...
        for (auto &trans : transactionQueue) {
            ...
            if (trans->timeout && trans->timeout_type == 0)
                continue;  // perf timeout exclude
            if (trans->hqos_timeout)
                continue;  // perf hqos timeout exclude
            if (trans->rank == rank) {
                samerank_cnt++;
                if (PTC_RANK_NUM != 0) {
                    if (samerank_cnt > (TRANS_QUEUE_DEPTH / PTC_RANK_NUM)) {
                        // ERROR(setw(10) << now() << " Same Rank cnt exceed upper limit in PTC, task=" << trans->task
                                    //    << ", rank=" << rank << ", bankIndex=" << trans->bankIndex
                                    //    << ", row=" << trans->row);
                        // assert(0);
                    }
                }
...
{
    if (!PAGE_TIMEOUT_EN && !PERF_PRE_EN && !PTC_RHIT_BREAK_EN && !PERF_DUMMY_TOUT_EN)
        return;
    ...
    for (auto &state : bankStates) {
        ...
        bool perf_force_pre =
            ((!hold_pre && (state.perf_rd_rowmiss || state.perf_wr_rowmiss)) || state.perf_rowhit_break) && PERF_PRE_EN;

        // precharge forced by page tiemout
        uint32_t page_timeout_cnt;
        if (state.perf_rd_rowhit || state.perf_wr_rowhit) {
            page_timeout_cnt = PTC_PAGE_TIMEOUT_HIT;
        }
        else {
            page_timeout_cnt = PTC_PAGE_TIMEOUT_MISS;
        }

        bool page_force_pre = ((access_bank_delay[state.bankIndex].cnt >= page_timeout_cnt &&
                              state.state->lastCmdType == DATA_READ) ||
                              (state.state->lastCmdType == DATA_WRITE 
                              && access_bank_delay[state.bankIndex].cnt >= page_timeout_cnt)) 
                              && PAGE_TIMEOUT_EN;
        ...
        if (EM_ENABLE && EM_MODE == 2 && state.rank == 1 && sub_channel == 1)
            continue;  // rank1, sc1 forbidden under combo e-mode
        bank_cas_delay[state.bankIndex]++;
        if ((w_bank_cnt[state.bankIndex] + r_bank_cnt[state.bankIndex]) > 0)
            continue;
        if (state.state->currentBankState != RowActive)
            continue;
        if (page_force_pre || perf_force_pre || rhit_break_force_pre || dummy_tout_force_pre) {
            if ((now() + 1) >= state.state->nextPrecharge && tFPWCountdown[state.rank].size() < 4 &&
                even_cycle) {  // every other command, even;
                if (arb_enable) {
                    ...
                    if (RankState[state.rank].lp_state == IDLE) {
                        ...
                        pre_cmdqueue.push_back(c);
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, "{:10} -- REQ :: PAGE/PERF/RHIT_BRK/DUMMY, precharge bank={}, task={}, "
                                          "rank={}, bank={}, cmd_source={}",
                                now(),
                                c->bankIndex,
                                c->task,
                                c->rank,
                        ...
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, "{:10} -- REQ_LP :: hold PRECHARGE in lp state, rank{}, bank={}",
                                now(),
                                state.rank,
                                state.bankIndex);
                        }
...
void PTC::data_fresh()
{
    for (auto &cmd : CmdQueue) {
        ...
        if (RankState[cmd->rank].lp_state != IDLE) {
            ERROR(setw(10) << now() << " -- DMC[" << getID() << "] Send command in lp state, cmd_type=" << cmd->cmd_type
                           << ", task=" << cmd->task << ", rank=" << cmd->rank << ", bank=" << cmd->bankIndex);
            assert(0);
        }
    }
    ...
    if (size > 0) {
        ...
        if (writeDataToSend[0].delay == 0) {
            ...
            for (size_t j = 0; j < WtransQueue.size(); j++) {
                if (writeDataToSend[0].task != WtransQueue[j]->task)
                    continue;
                if (WtransQueue[j]->data_ready_cnt < (WtransQueue[j]->burst_length + 1))
                    continue;
                ...
                if (wdata_info[writeDataToSend[0].task] == 0) {
                    ...
                    if (IECC_ENABLE)
                        tasks_info[writeDataToSend[0].task].wr_finish = true;
                }
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- T_DDR :: write data on Data Bus, task={}", now(), writeDataToSend[0].task);
                }
    ...
    //             if (DEBUG_BUS) {
    //                 PRINT_SPD_PTC(ch, "{:10} -- T_CPU :: Issuing to CPU bus, task={}", now(), task);
    //             }
    ...
    //             if (DEBUG_BUS) {
    //                 PRINT_SPD_PTC(ch, "{:10} -- Rdata burst_cnt={}", now(), static_cast<int>(msg.burst_cnt));
    //             }
    ...
    // #endif
    //             if ((!IECC_ENABLE || !tasks_info[task].rd_ecc) && (!RMW_ENABLE_PERF ||
    //             (!read_data_buffer[i].mask_wcmd && RMW_ENABLE_PERF))) {
    ...
    //                     if (PRINT_READ) {
    //                         TRACE_PRINT(setw(10)<<now()<<" -- Rdata Back Pressure :: task="<<task<<" ch="<<channel);
    //                     }
    //                     if (DEBUG_BUS) {
    //                         PRINT_SPD_PTC(ch, "{:10} -- Rdata Back Pressure or rdata not fully collected:: task={} ch={}
    //                         burst_cnt={} wait_rdata_num{}", now(), task, channel, static_cast<int>(msg.burst_cnt),
    //                         wait_rdata_num);
    //                     }
    //                     return;
    //                 } else {
    ...
    //                                 - msg.reqEnterDmcBufTime) / tDFI))<<" ch="<<channel);
    //                     }
    //                     if (PRINT_IDLE_LAT) {
    ...
    //                     }
    //                     if (DEBUG_BUS) {
    //                         PRINT_SPD_PTC(ch, "{:10} -- Rdata Received :: task={}{}{}", now(), task, ", latency="
    //, ceil(((read_data_buffer[i].readDataEnterDmcTime
    //                                 - msg.reqEnterDmcBufTime) / tDFI)));
    //                     }
    //                 }
    ...
    //                 }
    //                 if (DEBUG_BUS) {
    //                     PRINT_SPD_PTC(ch, "{:10} -- Finish :: Issuing to CPU bus, task={}", now(), task);
    //                 }
    //             } else {
...
void PTC::update_state_pre()
{
    for (auto &state : bankStates) {
        unsigned state_channel = (state.bankIndex % NUM_BANKS) / sc_bank_num;
        if (EM_ENABLE && EM_MODE == 2 && state_channel == 1 && state.rank == 1)
            continue;  // rank1, sc1 forbidden under combo e-mode
        if (state.state->currentBankState == Idle)
            bank_idle_cnt[state.rank]++;
        if (state.state->currentBankState == RowActive)
            bank_act_cnt[state.rank]++;
    }
    ...
    for (auto &trans : transactionQueue) {
        ...
        if (trans->transactionType != DATA_READ)
            continue;
        if (RankState[trans->rank].lp_state == IDLE)
            continue;
        auto &st = RankState[trans->rank].lp_state;
        if (st >= PDE && st <= PDX)
            rd_met_pd_cnt++;
        else if (st >= ASREFE && st <= SRPDX)
            rd_met_asref_cnt++;
    }
    ...
    if (TRFC_CC_EN) {  // tdo: revise for e-mode
        ...
        for (size_t rank = 0; rank < NUM_RANKS; rank++) {
            for (size_t i = 0; i < sc_num; i++) {
                if (refreshALL[rank][i].refreshing)
                    has_abr = true;
                break;
            }
            ...
            for (size_t bank = 0; bank < NUM_BANKS; bank++) {
                ...
                if (bankStates[bank_tmp].state->currentBankState == RowActive)
                    break;
                if (bankStates[bank_tmp].state->currentBankState == Precharging)
                    break;
                cc++;
            }
        }
    ...
    // useless?
    for (size_t rank = 0; rank < NUM_RANKS; rank++) {
        if (!pbr_hold_pre[rank])
            continue;
        bool pbr_bank_open = false;
        for (size_t bank = 0; bank < NUM_BANKS; bank++) {
            unsigned sub_channel = bank / sc_bank_num;
            if (EM_ENABLE && EM_MODE == 2 && rank == 1 && sub_channel == 1)
                continue;  // rank1, sc1 forbidden under combo e-mode
            if (!refreshPerBank[rank * NUM_BANKS + bank].refreshWaiting)
                continue;
            if (bankStates[rank * NUM_BANKS + bank].state->currentBankState != RowActive)
                continue;
            pbr_bank_open = true;
            break;
        }
        if (pbr_bank_open)
            continue;
        if (now() >= pbr_hold_pre_time[rank])
            pbr_hold_pre[rank] = false;
    }
}
...
void PTC::update_state_post()
{
    ...
    if (DEBUG_BANK) {
        unsigned bkst;
        std::string bank_line = fmt::format("{:10} -- BANK STATE :: ", now());
        for (size_t bank = 0; bank < NUM_BANKS; bank++) {
            if (bankStates[bank].state->currentBankState == RowActive &&
                bankStates[bank].state->lastCmdType > 1)
                bkst = 8;
            else if (bankStates[bank].state->currentBankState == Refreshing &&
                     bankStates[bank].state->lastCommand == REFRESH_CMD)
                bkst = 9;
            else
                bkst = bankStates[bank].state->currentBankState;
            bank_line += fmt::format("{} ", bkst);
        }
        PRINT_SPD_PTC(ch, "{}", bank_line);
    }
}
...
{
    if (IS_GD2 && tfpw != 0)
        tFPWCountdown[rank].push_back(tfpw);
}
...
void PTC::generate_packet(Cmd *c)
{
    ...
    switch (c->cmd_type) {
        case PRECHARGE_PB_CMD: {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- RMHOLD :: PRECHARGE_PB Remove bank={} Hold Precharge, task={}",
                    now(),
                    c->bankIndex,
                    c->task);
            }
        ...
        case PRECHARGE_SB_CMD: {
            ...
            LoadTfpw(c->rank, tFPW);
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- RMHOLD :: PRECHARGE_SB Remove bank={} Hold Precharge, task={}",
                    now(),
                    c->bankIndex,
                    c->task);
            }
        ...
        case PRECHARGE_AB_CMD: {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, 
                    "{:10} -- RMHOLD :: PRECHARGE_AB Remove rank={} Hold Precharge, task={}", now(), c->rank, c->task);
            }
            break;
        }
        case READ_CMD:
        case READ_P_CMD: {
            if (c->cmd_type == READ_P_CMD && c->fg_ref)
                command_pend = pre_cycle;
            else if (!RankState[c->rank].wck_on)
                command_pend = rw_cycle * 2;
            else
                command_pend = rw_cycle;
        ...
        case WRITE_MASK_CMD:
        case WRITE_MASK_P_CMD: {
            if ((c->cmd_type == WRITE_P_CMD || c->cmd_type == WRITE_MASK_P_CMD) && c->fg_ref)
                command_pend = pre_cycle;
            else if (!RankState[c->rank].wck_on)
                command_pend = rw_cycle * 2;
            else
            ...
            if (c->cmd_type == WRITE_P_CMD || c->cmd_type == WRITE_MASK_P_CMD ||
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- RMHOLD :: WRITE Remove bank={} Hold Precharge,task={}", now(), c->bankIndex, c->task);
                }
            }
            TotalBytes += unsigned(PAM_RATIO * DmcLog2(c->bl, JEDEC_DATA_BUS_BITS)) / 8;
            flowStatisTotalBytes += unsigned(PAM_RATIO * DmcLog2(c->bl, JEDEC_DATA_BUS_BITS)) / 8;
            ...
            TotalBytesRank[c->rank] += unsigned(PAM_RATIO * DmcLog2(c->bl, JEDEC_DATA_BUS_BITS)) / 8;
            if (c->cmd_type == WRITE_P_CMD)
                LoadTfpw(c->rank, tFPW);
            if (c->cmd_type == WRITE_MASK_P_CMD)
                LoadTfpw(c->rank, tFPW);
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- RMHOLD :: Remove bank={} Hold Precharge,task={}", now(), c->bankIndex, c->task);
            }
            break;
        }
        case ACTIVATE1_CMD: {
        ...
        case ACTIVATE2_CMD: {
            ...
            if (DEBUG_BUS) {
                if (c->pre_act) {
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- HOLD :: bank={} PreAct Not Hold Precharge, task={}", now(), c->bankIndex, c->task);
                } else {
                    PRINT_SPD_PTC(ch, "{:10} -- HOLD :: bank={} Hold Precharge, task={}", now(), c->bankIndex, c->task);
                }
            }

            if (c->type == DATA_READ)
                bankStates[c->bankIndex].write_hold = false;
            else
                bankStates[c->bankIndex].write_hold = true;
            ...
            if (t->issue_size == t->data_size) {
                ...
                if (PRINT_LATENCY) {
                    DEBUG(setw(10) << now() << " -- LAT :: cnt=" << cmd_in2dfi_cnt << ", lat=" << rd_lat << ", lat_all="
                                   << cmd_in2dfi_lat << ", ave_lat=" << float(cmd_in2dfi_lat) / float(cmd_in2dfi_cnt)
                                   << ", task=" << t->task << ", inject_time=" << t->inject_time
                                   << ", timeAdded=" << t->timeAdded << ", qos=" << t->qos << ", pri=" << t->pri
                                   << ", perf_qos=" << t->perf_qos << ", group=" << t->group
                                   << ", bank=" << t->bankIndex);
                }
            }
            break;
...
void PTC::check_timeout_and_aging()
{
    ...
    if (PTC_PRI_ADPT_ENABLE || MPAM_MAPPING_TIMEOUT) {
        for (auto &trans : transactionQueue) {
            ...
            if (trans->addrconf)
                continue;
            if (trans->pri_adapt_th == 0 || trans->qos == PTC_QOS_MAX)
                continue;
            if (now() - trans->ptc_timeAdded >= ((trans->improve_cnt + 1) * trans->pri_adapt_th)) {
                ...
                if (trans->pri < PTC_QOS_MAX) {
                    trans->pri++;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- PRI_APAPT :: task={} pri={}", now(), trans->task, trans->pri);
                    }
                }
            }
        }
    ...
    // timeout check
    if (!PTC_TIMEOUT_ENABLE && !MPAM_MAPPING_TIMEOUT)
        return;
    ...
    if (QOS_POLICY == 2) {
        for (auto &trans : transactionQueue) {
            if (now() < trans->arb_time) continue;
            if (trans->pri > highest_pri)
                highest_pri = trans->pri;
        }
    }
    ...
    for (auto &trans : transactionQueue) {
        ...
        if ((trans->transactionType == DATA_WRITE) && (trans->data_ready_cnt < (trans->burst_length + 1)))
            continue;
        if (!trans->timeout && ((now() - trans->ptc_timeAdded >= trans->timeout_th && trans->timeout_th != 0) ||
                                   trans->qos == PTC_QOS_MAX)) {
            trans->timeout = true;
            trans->timeout_type = 1;
            trans->time_timeout = now();
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- PTC TIMEOUT :: task={} type={} qos={} pri={} cmd_rt_type={} cmd_hqos_type={}",
                    now(),
                    trans->task,
        ...
        } else if (trans->timeout && trans->pri != PTC_QOS_MAX &&
                   ((QOS_POLICY == 1) || (QOS_POLICY == 2 && trans->pri >= highest_pri))) {
            trans->pri = PTC_QOS_MAX;
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- PTC TIMEOUT_PRI :: task={} type={} qos={} pri={}",
                    now(),
                    trans->task,
                    trans->transactionType,
                    trans->qos,
        ...
        if (trans->timeout && !has_tout_cmd)
            has_tout_cmd = true;
        if (trans->timeout && trans->cmd_rt_type && !has_rt_tout_cmd)
            has_rt_tout_cmd = true;
        if (trans->timeout && trans->cmd_hqos_type && !has_hqos_tout_cmd)
            has_hqos_tout_cmd = true;
    }
    ...
    for (auto &trans : transactionQueue) {
        if (!trans->timeout)
            continue;
        if (trans->pri > tout_high_pri)
            tout_high_pri = trans->pri;
    }
    ...
    for (auto &trans : transactionQueue) {
        ...
        if (has_rt_tout_cmd && !trans->timeout)
            trans->bp_by_tout = true;
        else if (has_hqos_tout_cmd && !trans->timeout)
            trans->bp_by_tout = true;
        else if (QOS_POLICY == 1 && has_tout_cmd && !trans->timeout)
            trans->bp_by_tout = true;
        else if (QOS_POLICY == 2 && has_tout_cmd &&
                 (trans->pri < tout_high_pri || (!trans->timeout && trans->pri == tout_high_pri))) {
            trans->bp_by_tout = true;
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- PTC Backpress by tout :: task={} qos={} pri={} tout_high_pri={}",
                    now(),
                    trans->task,
                    trans->qos,
                    trans->pri,
...
void PTC::ptc_check_timeout_and_aging()
{
        ...
        for (auto &trans : transactionQueue) {
            ...
            if (now() - trans->timeAdded >= ((trans->improve_cnt + 1) * trans->pri_adapt_th)) {
                ...
                if (trans->pri < PTC_ADAPT_PRI_MAX) {  // adapt low qos to pri_max && no adapt for hqos
                    trans->pri++;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- PTC PRI_APAPT :: task={} pri={} qos={}",
                            now(),
                            trans->task,
                            trans->pri,
                            trans->qos);
    ...
    for (auto &trans : transactionQueue) {
        ...
        if ((trans->transactionType == DATA_WRITE) && (trans->data_ready_cnt < (trans->burst_length + 1)))
            continue;
        if (!trans->timeout &&
            ((now() - trans->timeAdded >= trans->timeout_th && trans->timeout_th != 0) || trans->qos == PTC_QOS_MAX)) {
            trans->timeout = true;
            trans->timeout_type = 1;
            trans->time_timeout = now();
            if (DEBUG_BUS) {
        ...
        if (has_rt_tout_cmd && !trans->timeout)
            ...
            if (PTC_TIMEOUT_MODE == 0) {
                ...
                for (size_t rank = 0; rank < NUM_RANKS; rank++) {
                    if (rank == trans->rank)
                        continue;
                    if (has_timeout_rank[rank])
                        trans->bp_by_tout = true;
                }
                // backpress by diff_type timeout
                if (has_timeout_rd && trans->transactionType == DATA_WRITE)
                    trans->bp_by_tout = true;
                else if (has_timeout_wr && trans->transactionType == DATA_READ)
                    trans->bp_by_tout = true;
            }
        } else if (QOS_POLICY == 2 && has_tout_cmd &&
                   (trans->pri < tout_high_pri || (!trans->timeout && trans->pri == tout_high_pri))) {
            trans->bp_by_tout = true;
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- PTC Backpress by tout :: task={} type={} qos={} pri={} tout_high_pri={} "
                              "has_timeout_rd={} has_timeout_wr={}",
                    now(),
                    trans->task,
                    trans->transactionType,
                    trans->qos,
...
void PTC::CalcBl(Transaction *t)
{
    ...
    if (t->transactionType == DATA_READ) {
        ...
        if (t->data_size - t->issue_size < t->trans_size)
        ...
        if ((t->data_size - t->issue_size < min_bl_data_size) && (IS_LP5 || IS_LP6 || IS_LP4)) {
            ERROR(setw(10) << now() << " -- Issue size wrong, task=" << t->task << " data_size=" << t->data_size
                           << " issue_size=" << t->issue_size << " min_bl_data_size=" << min_bl_data_size);
            assert(0);
        }
...
void PTC::need_issue(Transaction *trans)
{
    if (bankStates[trans->bankIndex].state->currentBankState == RowActive) {
        ...
        if (trans->row == openRow) {
            if (rw_exec_cnt < EXEC_NUMBER || trans->issue_size != 0 || EXEC_UNLIMIT_EN) {
                ...
                if ((trans->issue_size + trans->trans_size) < trans->data_size) {
                    ...
                    for (auto &t : transactionQueue) {
                        if (t->task == trans->task)
                            continue;
                        if (t->bankIndex != trans->bankIndex)
                            continue;
                        if (trans->row != t->row)
                            continue;
                        row_hit = true;
                    ...
                    if (bankStates[trans->bankIndex].perf_rd_rowhit ||
                        bankStates[trans->bankIndex].perf_wr_rowhit) {  // check PERF has rowhit cmd
                        row_hit = true;
                    }
                }
                ...
                if (trans->transactionType == DATA_READ) {
                    ...
                    if (WR_APRE_EN || trans->ap_cmd || page_timeout_wr[trans->bankIndex] == 0) {
                        if (row_hit)
                            ...
                            trans->nextCmd = is_mask ? WRITE_MASK_P_CMD : WRITE_P_CMD;
                    } else if (ENHAN_WR_AP_EN && !row_hit) {
                        if (bank_cnt[trans->bankIndex] <= 1)
                            trans->nextCmd = WRITE_CMD;
                        else
                            trans->nextCmd = WRITE_P_CMD;
                    } else {
                        trans->nextCmd = is_mask ? WRITE_MASK_CMD : WRITE_CMD;
                    }
                }
            ...
            ERROR(setw(10) << now() << " -- Error Send Precharge Cmd! task=" << trans->task
                           << ", issue_size=" << trans->issue_size << ", trans_size=" << trans->trans_size
        ...
        if (IS_LP4 || IS_LP5 || IS_LP6) {
            if (trans->act_executing)
                trans->nextCmd = ACTIVATE2_CMD;
            else if (bankStates[trans->bankIndex].state->act_executing)
                trans->nextCmd = INVALID;
            else
                trans->nextCmd = ACTIVATE1_CMD;
        } else {
            trans->nextCmd = ACTIVATE2_CMD;
        }
    }
    ...
    if (trans->nextCmd == READ_P_CMD || trans->nextCmd == WRITE_P_CMD || trans->nextCmd == WRITE_MASK_P_CMD) {
        if (trans->issue_size + trans->trans_size < trans->data_size) {
            ERROR(setw(10) << now() << " -- Error Send AP Cmd! task=" << trans->task
                           << ", issue_size=" << trans->issue_size << ", trans_size=" << trans->trans_size
                           << ", data_size=" << trans->data_size);
            assert(0);
        }
...
void PTC::cmd_select(Transaction *t)
{
    ...
    if (t->pre_act && bankStates[t->bankIndex].state->currentBankState == RowActive) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- LC :: PRE-ACT. pre active meet row active, task={}", now(), t->task);
        }
        return;
    }
    if (t->fast_rd && t->act_only && bankStates[t->bankIndex].state->currentBankState == RowActive) {
        t->issue_size = t->data_size;
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- LC :: FAST-RD-ACT. fast rd active meet row active, task={}", now(), t->task);
        }
        return;
    }
    ...
    if (RankState[t->rank].lp_state != IDLE) {
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- LP_BP_CMD :: task={} is BP by lowpower mode. rank={}, lp_state={}",
                now(),
                t->task,
                t->rank,
                RankState[t->rank].lp_state);
        }
        return;
    }
    ...
    for (size_t rank = 0; rank < NUM_RANKS; rank++) {
        if (force_pbr_refresh[rank][sub_channel] && SBR_REQ_MODE == 1 && t->issue_size == 0) {
            for (size_t sbr_bank = 0; sbr_bank < pbr_bg_num; sbr_bank++) {
                if ((forceRankBankIndex[rank][sub_channel] + sbr_bank * pbr_bank_num + rank * NUM_BANKS) ==
                    t->bankIndex) {
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LC :: force the same bank request in Rank {}, SC {}. task={}",
                            now(),
                            rank,
                            sub_channel,
                            t->task);
    ...
    bool rw_switch_trig = ((PreCmd.trans_type == DATA_READ) && (t->transactionType == DATA_READ) &&
                              que_read_cnt < PBR_LESS_CMD_LEVEL && que_write_cnt > 0) ||
                          ((PreCmd.trans_type == DATA_WRITE) && (t->transactionType == DATA_WRITE) &&
                              que_write_cnt < PBR_LESS_CMD_LEVEL && que_read_cnt > 0);
    bool pbr_block_en = PBR_LESS_CMD_EN && (!rw_switch_trig && PBR_LESS_CMD_MODE == 0);

    if (((refreshPerBank[t->bankIndex].refreshWaiting && (!PBR_LESS_CMD_EN || pbr_block_en)) ||
            refreshPerBank[t->bankIndex].refreshing) &&
        t->issue_size == 0 && !t->act_executing) {
        if (t->transactionType == DATA_READ)
            rd_met_pbr_cnt++;
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- LC :: this bank is refreshing. bank={} sc={} task={} postpnd={} refreshWaiting={} "
                          "refreshing={}",
                now(),
                t->bankIndex,
                sub_channel,
                t->task,
                refreshALL[t->rank][sub_channel].refresh_cnt,
                refreshPerBank[t->bankIndex].refreshWaiting,
                refreshPerBank[t->bankIndex].refreshing);
        }
        return;
    }
    ...
    if ((refreshALL[t->rank][sub_channel].refreshWaiting || refreshALL[t->rank][sub_channel].refreshing) &&
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, 
                "{:10} -- LC :: this rank is refreshing. rank={}, sc={} task={} refreshWaiting={} refreshing={}",
                now(),
                t->rank,
                sub_channel,
                t->task,
                refreshALL[t->rank][sub_channel].refreshWaiting,
                refreshALL[t->rank][sub_channel].refreshing);
        }
        return;
    }
    ...
    if (dresp_cnt >= DRESP_BP_TH && t->nextCmd >= WRITE_CMD && t->nextCmd <= READ_P_CMD && t->issue_size == 0) {
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- LC :: dresp backpress ing---. dresponse counter={}", now(), dresp_cnt);
        }
        return;
    }
    ...
    if (t->bp_by_tout && t->issue_size == 0 && !t->act_executing &&
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- LC :: Block by timeout cmd. task={}, bank={}, pri={}, tout_high_pri={}",
                now(),
                t->task,
                t->bankIndex,
                t->pri,
    ...
    if (t->transactionType != DATA_READ && t->nextCmd != ACTIVATE1_CMD && t->nextCmd != ACTIVATE2_CMD &&
        t->nextCmd != PRECHARGE_PB_CMD) {
        if (t->data_ready_cnt <= t->burst_length) {
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- LC :: write data not ready. cnt={} issue_size={} task={}",
                    now(),
                    t->data_ready_cnt,
                    t->issue_size,
                    t->task);
    ...
    if (SIMPLE_RWGRP_EN && SIMPLE_RWGRP_MODE == 0 && (!has_hqos)) {
        if (!t->timeout && t->issue_size == 0 && (t->nextCmd >= READ_CMD && t->nextCmd <= READ_P_CMD) &&
            (que_write_cnt > 0)) {
            if (PreCmd.type >= WRITE_CMD && PreCmd.type <= WRITE_MASK_P_CMD) {
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- LC :: Write group backpress a read command. task={}, nextCmd={} rank={} "
                                  "qos={} pri={} hqos_push={} rank={} que_read_cnt{}",
                        now(),
                        t->task,
                        t->nextCmd,
                        t->rank,
        ...
        if (!t->timeout && t->issue_size == 0 && (t->nextCmd >= WRITE_CMD && t->nextCmd <= WRITE_MASK_P_CMD) &&
            (que_read_cnt > 0)) {
            if (PreCmd.type >= READ_CMD && PreCmd.type <= READ_P_CMD) {
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- LC :: Read group backpress a write command. task={}, nextCmd={} rank={} "
                                  "qos={} pri={} hqos_push={} rank={} que_read_cnt{}",
                        now(),
                        t->task,
                        t->nextCmd,
    ...
    if (SIMPLE_RWGRP_EN && SIMPLE_RWGRP_MODE == 1) {
        for (auto &trans : transactionQueue) {
            if (t->task == trans->task)
                continue;
            if (t->transactionType == trans->transactionType)
                continue;
            if (t->ptc_timeAdded < trans->ptc_timeAdded)
                continue;
            if (t->transactionType == DATA_READ)
    ...
    if (SIMPLE_RWGRP_EN && SIMPLE_RWGRP_MODE == 1) {
        if (!t->timeout && t->issue_size == 0 && (t->nextCmd >= READ_CMD && t->nextCmd <= READ_P_CMD) &&
            has_early_write) {
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- LC :: Early Write backpress a read command. task={}, nextCmd={}",
                    now(),
                    t->task,
                    t->nextCmd);
            }
        ...
        if (!t->timeout && t->issue_size == 0 && (t->nextCmd >= WRITE_CMD && t->nextCmd <= WRITE_MASK_P_CMD) &&
            has_early_read) {
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- LC :: Early Read backpress a write command. task={}, nextCmd={}",
                    now(),
                    t->task,
                    t->nextCmd);
            }
        ...
        if (!ptc_no_rwgrp) {
            if (!t->timeout && t->issue_size == 0 && (PreCmd.type >= WRITE_CMD && PreCmd.type <= WRITE_MASK_P_CMD) &&
                ...
                if (t->nextCmd >= READ_CMD && t->nextCmd <= READ_P_CMD) {
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LC :: Write group backpress a read command. task={}, nextCmd={}, "
                                      "perf_grpst_cnt={}",
                            now(),
                            t->task,
                            t->nextCmd,
            ...
            if (!t->timeout && t->issue_size == 0 && (PreCmd.type >= READ_CMD && PreCmd.type <= READ_P_CMD) &&
                ...
                if (t->nextCmd >= WRITE_CMD && t->nextCmd <= WRITE_MASK_P_CMD) {
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LC :: Read group backpress a write command. task={}, nextCmd={}, "
                                      "perf_grpst_cnt={}",
                            now(),
                            t->task,
                            t->nextCmd,
            ...
            if (!t->timeout && t->issue_size == 0 && (PreCmd.type >= WRITE_CMD && PreCmd.type <= WRITE_MASK_P_CMD) &&
                ...
                if (t->nextCmd >= WRITE_CMD && t->nextCmd <= WRITE_MASK_P_CMD) {
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LC :: Write group backpress a write command. task={}, nextCmd={}, "
                                      "perf_grpst_cnt={}",
                            now(),
                            t->task,
                            t->nextCmd,
            ...
            if (!t->timeout && t->issue_size == 0 && (PreCmd.type >= READ_CMD && PreCmd.type <= READ_P_CMD) &&
                ...
                if (t->nextCmd >= READ_CMD && t->nextCmd <= READ_P_CMD) {
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- LC :: Read group backpress a read command. task={}, nextCmd={}, "
                                      "perf_grpst_cnt={}",
                            now(),
                            t->task,
                            t->nextCmd,
    ...
    if (SIMPLE_RWGRP_EN && SIMPLE_RWGRP_MODE == 3) {
        ...
        if (!ptc_nogrp) {
            ...
            bool hold_wcmd = (pfq_->wbuff_state == WBUFF_WRITE) &&
                             (PreCmd.type >= WRITE_CMD && PreCmd.type <= WRITE_MASK_P_CMD) &&
            ...
            bool hold_rcmd = (pfq_->wbuff_state == WBUFF_IDLE) &&
    ...
    bool same_cmdtype = (((t->nextCmd >= WRITE_CMD && t->nextCmd <= WRITE_MASK_P_CMD) &&
                             (PreCmd.type >= WRITE_CMD && PreCmd.type <= WRITE_MASK_P_CMD)) ||
                         ((t->nextCmd >= READ_CMD && t->nextCmd <= READ_P_CMD) &&
                             (PreCmd.type >= READ_CMD && PreCmd.type <= READ_P_CMD)));
    ...
    bool perf_hqos_rank_exclude =
        ((t->transactionType == DATA_WRITE) || (t->transactionType == DATA_READ && !perf_hqos_rank_hit)) ||
    ...
    bool ptc_hqos_rank_exclude =
        ((t->transactionType == DATA_WRITE) || (t->transactionType == DATA_READ && !ptc_hqos_rank_hit)) ||
        (!PTC_HQOS_RANK_SWITCH_EN) || (PTC_HQOS_RANK_SWITCH_MODE == 0);

    // force rank switch seperatly in rd/wr group
    if (PTC_RANK_SWITCH_EN && !t->timeout && t->issue_size == 0 && perf_hqos_rank_exclude && ptc_hqos_rank_exclude &&
        same_cmdtype && force_rank_switch && has_other_rank_cmd) {
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, 
                "{:10} -- LC :: Force Rank Switch backpress a command. task={}, nextCmd={}, rank={}, pre_cmd_rank={}, "
                "has_other_rank_cmd={}, force_rank_switch={}, perf_hqos_rank={}, ptc_hqos_rank={}",
                now(),
                t->task,
                t->nextCmd,
                t->rank,
    ...
    if (PERF_RCMD_HQOS_RANK_SWITCH_EN && !t->timeout && t->issue_size == 0 && perf_hqos_rank_miss &&
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- LC :: Perf Hqos Force Rank Switch backpress a command. task={}, nextCmd={}, "
                          "rank={}, perf_has_hqos_rank_rcmd={}, hqos_rank={}",
                now(),
                t->task,
                t->nextCmd,
                t->rank,
    ...
    if (PTC_HQOS_RANK_SWITCH_EN && !t->timeout && t->issue_size == 0 && ptc_hqos_rank_miss) {
        if ((PTC_HQOS_RANK_SWITCH_MODE == 1 && t->nextCmd >= WRITE_CMD && t->nextCmd <= READ_P_CMD) ||
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- LC :: Ptc Hqos Force Rank Switch backpress a diff rank rd/wr command. task={}, "
                              "nextCmd={}, rank={}, perf_has_hqos_rank_rcmd={}, ptc_hqos_rank={}",
                    now(),
                    t->task,
                    t->nextCmd,
                    t->rank,
    ...
    if (PTC_HQOS_RANK_SWITCH_EN && !t->timeout && t->issue_size == 0 && ptc_hqos_rank_hit) {
        if ((PTC_HQOS_RANK_SWITCH_MODE == 3 && t->nextCmd >= WRITE_CMD && t->nextCmd <= WRITE_MASK_P_CMD &&
                !t->hqos_push)) {
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, 
                    "{:10} -- LC :: Ptc Hqos Force Rank Switch backpress a same rank write command. task={}, "
                    "nextCmd={}, rank={}, perf_has_hqos_rank_rcmd={}, ptc_hqos_rank={}rank0_ptc_hqos_rcmd_cnt={}",
                    now(),
                    t->task,
                    t->nextCmd,
                    t->rank,
    ...
    if (!IS_DDR5 && t->nextCmd == PRECHARGE_PB_CMD) {  // todo: revise for e-mode
        for (size_t bank = 0; bank < NUM_BANKS; bank++) {
            ...
            if (refreshPerBank[bank_tmp].refreshWaiting && !refreshPerBank[bank_tmp].refreshWaitingPre) {
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- LC :: PBR hold PRE command. task={}", now(), t->task);
                }
                return;
            }
        }
    ...
    if (t->nextCmd == INVALID) {
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- LC :: Invalid command. task={}", now(), t->task);
        }
        return;
    }
    ...
    switch (t->nextCmd) {
        case READ_CMD: {
            ...
            if ((now() + 1) >= bankStates[t->bankIndex].state->nextRead) {
                ...
                if (t->issue_size == 0)
                    cmd_rdmet_cnt++;
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- LC :: READ timing met, bank={}, task={}", now(), t->bankIndex, t->task);
                }
            }
            break;
        }
        case READ_P_CMD: {
            ...
            if ((now() + 1) >= bankStates[t->bankIndex].state->nextReadAp) {
                ...
                if (t->issue_size == 0)
                    cmd_rdmet_cnt++;
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- LC :: READ_P timing met, bank={}, task={}", now(), t->bankIndex, t->task);
                }
            }
            break;
        }
            ...
            if (IS_DDR5) {
                if (DDR_MODE == "_x4" && t->bl == BL16) {
                    ...
                    if ((now() + 1) >= bankStates[t->bankIndex].state->nextWriteRmw) {
                        ...
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, 
                                "{:10} -- LC :: WRITE RMW timing met, bank={}, task={}", now(), t->bankIndex, t->task);
                        }
                    }
                } else if ((DDR_MODE == "_x4" && t->bl == BL32) ||
                           ((DDR_MODE == "_x8" || DDR_MODE == "_x16") && t->bl == BL16)) {
                    // next command is JW
                    if ((now() + 1) >= bankStates[t->bankIndex].state->nextWrite) {
                        timing_met = true;
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, 
                                "{:10} -- LC :: WRITE JW timing met, bank={}, task={}", now(), t->bankIndex, t->task);
                        }
                    }
                } else {
                ...
                if ((now() + 1) >= bankStates[t->bankIndex].state->nextWrite) {
                    ...
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, 
                            "{:10} -- LC :: WRITE timing met, bank={}, task={}", now(), t->bankIndex, t->task);
                    }
                }
            }
            break;
            ...
            if (IS_DDR5) {
                if (DDR_MODE == "_x4" && t->bl == BL16) {
                    ...
                    if ((now() + 1) >= bankStates[t->bankIndex].state->nextWriteApRmw) {
                        timing_met = true;
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, "{:10} -- LC :: WRITE_P RMW timing met, bank={}, task={}",
                                now(),
                                t->bankIndex,
                                t->task);
                        }
                    ...
                    if ((now() + 1) >= bankStates[t->bankIndex].state->nextWriteAp) {
                        ...
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, 
                                "{:10} -- LC :: WRITE_P JW timing met, bank={}, task={}", now(), t->bankIndex, t->task);
                        }
                    }
                } else {
                    ERROR(setw(10) << now() << " -- Error DDR Mode: " << DDR_MODE << ", BL=" << t->bl);
                ...
                if ((now() + 1) >= bankStates[t->bankIndex].state->nextWriteAp) {
                    ...
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, 
                            "{:10} -- LC :: WRITE_P timing met, bank={}, task={}", now(), t->bankIndex, t->task);
                    }
                }
            }
            break;
        ...
        case WRITE_MASK_CMD: {
            ...
            if ((now() + 1) >= bankStates[t->bankIndex].state->nextWriteMask) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- LC :: WRITE_MASK timing met, bank={}, task={}", now(), t->bankIndex, t->task);
                }
            }
            break;
        }
        case WRITE_MASK_P_CMD: {
            ...
            if ((now() + 1) >= bankStates[t->bankIndex].state->nextWriteMaskAp) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- LC :: WRITE_MASK_P timing met, bank={}, task={}", now(), t->bankIndex, t->task);
                }
            }
            break;
        }
        case PRECHARGE_PB_CMD: {
            if ((now() + 1) >= bankStates[t->bankIndex].state->nextPrecharge && tFPWCountdown[t->rank].size() < 4) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- LC :: PRECHARGE_PB timing met, bank={}, task={}", now(), t->bankIndex, t->task);
                }
            }
            break;
        }
        case PRECHARGE_SB_CMD: {
            ...
            for (size_t i = 0; i < pbr_bg_num; i++) {
                ...
                if ((now() + 1) >= bankStates[bank].state->nextPrecharge)
                    met_cnt++;
            }
            if (met_cnt == pbr_bg_num && tFPWCountdown[t->rank].size() < 4) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- LC :: PRECHARGE_SB timing met, bank={}, task={}", now(), t->bankIndex, t->task);
                }
            }
            break;
        }
        case PRECHARGE_AB_CMD: {  // todo: revise for e-mode
            ...
            if ((met_cnt == NUM_BANKS / sc_num) && tFPWCountdown[t->rank].size() < 4) {
                timing_met = true;
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- LC :: PRECHARGE_AB timing met, bank={}, sc={}, task={}",
                        now(),
                        t->bankIndex,
                        sub_channel,
                        t->task);
        ...
        case ACTIVATE1_CMD: {
            if ((now() + 1) >= bankStates[t->bankIndex].state->nextActivate1) {
                timing_met = true;
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- LC :: ACTIVE1 timing met, bank={}, task={}", now(), t->bankIndex, t->task);
                }
            }
            break;
        }
        case ACTIVATE2_CMD: {
            if ((now() + 1) >= bankStates[t->bankIndex].state->nextActivate2 &&
                ((tFAWCountdown[t->rank].size() < 4 && sub_channel == 0) ||
                    (tFAWCountdown_sc1[t->rank].size() < 4 && sub_channel == 1))) {
                timing_met = true;
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- LC :: ACTIVE2 timing met, bank={}, task={}", now(), t->bankIndex, t->task);
                }
            }
            break;
        }
    ...
    if (timing_met) {
        ...
        if (c->cmd_type >= WRITE_CMD && c->cmd_type <= READ_P_CMD) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- rwcmd in rw_cmdqueue, task={}, cmd_type={}", now(), c->task, c->cmd_type);
            }
        } else
            act_cmdqueue.push_back(c);
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- act_cmdqueue_empty={}", now(), act_cmdqueue.empty());
        }
        // CmdQueue.push_back(c);
    }
}
    ...
    for (auto &t : transactionQueue) {
        if (((trans->address & ~ALIGNED_SIZE) == (t->address & ~ALIGNED_SIZE)) &&
            ...
            if (trans->pri < t->pri && PRIORITY_PASS_ENABLE) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- ADD_CONF_PUSH :: push add conflict pri, src task={} dst task={} qos={} pri={}",
                        now(),
                        trans->task,
                        t->task,
                        trans->qos,
...
void PTC::page_adapt_policy(Transaction *trans)
{
    ...
    if (now() % 800 == 0 && now() != 0 && 0) {
        ...
        if (page_rw_cnt != 0)
            rowhit_ratio = 1000 * page_act_cnt / page_rw_cnt;
    ...
    for (uint32_t i = 0; i < sizeof(page_timeout_window) / sizeof(page_timeout_window[0]); i++) {
        if (row == bankStates[bank].last_activerow) {
            if (bank_cas_delay[bank] < page_timeout_window[i])
                page_row_hit[bank][i]++;
            else
                page_row_miss[bank][i]++;
        } else {
            uint32_t gap;
            gap = (bank_cas_delay[bank] < tRPpb) ? 0 : bank_cas_delay[bank] - tRPpb;
            if (gap < page_timeout_window[i])
                page_row_conflict[bank][i]++;
            else
                page_row_miss[bank][i]++;
        }
    }
    ...
    if ((trans->transactionType == DATA_READ && page_cmd_cnt[bank] >= OPENPAGE_TIME_RD) ||
        ...
        if (trans->transactionType == DATA_READ)
            page_timeout_rd[bank] = page_timeout_window[index];
        else
            page_timeout_wr[bank] = page_timeout_window[index];
...
void PTC::page_adpt_policy(Transaction *trans)
{
    ...
    if (bank_cnt[trans->bankIndex] == 0 && state->currentBankState == RowActive &&
        ...
        if (PAGE_WIN_MODE == 0)
            page_adpt_win_cnt++;
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- OPC_ADD, opc_cnt={}, ppc_cnt={}, task={}, bank={}, adpt_openpage_time={}",
                now(),
                opc_cnt,
                ppc_cnt,
                trans->task,
    ...
    if (bank_cnt[trans->bankIndex] == 0 && state->currentBankState != RowActive && state->lastRow == trans->row &&
        ...
        if (PAGE_WIN_MODE == 0)
            page_adpt_win_cnt++;
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- PPC_ADD, opc_cnt={}, ppc_cnt={}, task={}, bank={}, adpt_openpage_time={}",
                now(),
                opc_cnt,
                ppc_cnt,
                trans->task,
    ...
    if (page_adpt_win_cnt == PAGE_WIN_SIZE) {
        if (ppc_cnt >= PAGE_PPC_TH && opc_cnt < PAGE_OPC_TH) {
            if (adpt_openpage_time >= PAGE_TIME_MAX)
                adpt_openpage_time = PAGE_TIME_MAX;
            else
                adpt_openpage_time += PAGE_ADPT_STEP;
        } else if (ppc_cnt < PAGE_PPC_TH && opc_cnt >= PAGE_OPC_TH) {
            if (adpt_openpage_time < PAGE_ADPT_STEP)
                adpt_openpage_time = 0;
            else
                adpt_openpage_time -= PAGE_ADPT_STEP;
        }
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- WIN_EXCEED, page_adpt_win_cnt={}, opc_cnt={}, ppc_cnt={}, adpt_openpage_time={}",
                now(),
                page_adpt_win_cnt,
                opc_cnt,
                ppc_cnt,
...
void PTC::pushQosForSameMpamTrans(Transaction *trans)
{
    for (auto &trans_t : transactionQueue) {
        if (trans_t->mpam_id == trans->mpam_id) {
            if (trans->pri < trans_t->pri && trans_t->transactionType == DATA_READ)
                trans_t->pri = trans->pri;
        }
    }
}
...
void PTC::noc_read_inform(bool fast_wakeup_rank0, bool fast_wakeup_rank1, bool bus_rempty)
{
    ...
    for (size_t rank = 0; rank < NUM_RANKS; rank++) {
        ...
        if (FASTWAKEUP_EN && !PREDICT_FASTWAKEUP)
            fast_wakeup_cnt[rank] += int(wakeup[rank]);
    }
    if (DEBUG_BUS) {
        if (fast_wakeup_rank0 || fast_wakeup_rank1) {
            PRINT_SPD_PTC(ch, "{:10} -- Fast Wakeup, rank0={}, rank1={}", now(), fast_wakeup_rank0, fast_wakeup_rank1);
        }
    }
}
...
bool PTC::addTransaction(Transaction *trans, bool fastread)
{
    ...
    bool fast_read_req =
        ((!get_bs_full(trans)) &&
            (state.state->currentBankState == Idle && now() >= state.state->nextActivate1)  // bank closed
            && (!refreshPerBank[trans->bankIndex].refreshWaiting &&
                   !refreshALL[trans->rank][sub_channel].refreshWaiting)  // not refresh bank pairs
            && (!refreshPerBank[trans->bankIndex].refreshing &&
                   !refreshALL[trans->rank][sub_channel].refreshing)  // not refresh bank pairs
            && ((!state.perf_bankrd_conflict && state.ptc_samebank_rd == 0 && state.ptc_samebank_wr == 0 &&
                    FAST_RD_CANCEL_MODE == 0)  // not same bank cmd in perf/ptc mode 0
                   || (!state.perf_bankrd_conflict && state.ptc_samebank_rd == 0 && !state.perf_bankwr_conflict &&
                          state.ptc_samebank_wr == 0 &&
                          FAST_RD_CANCEL_MODE == 1)));  // not same bank cmd in perf/ptc mode 1
    ...
    if (FAST_READ_EN && trans->transactionType == DATA_READ && fast_read_req && fastread) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, 
                "{:10} -- PTC : Fast Read Generate, task={}, rank={}, bankIndex={}, fast_rd={} Bank slot idle={}",
                now(),
                fast_rtrans->task,
                fast_rtrans->rank,
                fast_rtrans->bankIndex,
                fast_rtrans->fast_rd,
                get_bs_full(trans));
        }
    }
    ...
    if (FAST_HQOS_PUSH_EN && !trans->pre_act && !trans->fast_rd && trans->transactionType == DATA_READ &&
        trans->qos >= PERF_SWITCH_HQOS_LEVEL && fastread) {
        for (auto &t : transactionQueue) {
            if (t->bankIndex != trans->bankIndex)
                continue;
            if (t->issue_size > 0)
            ...
            if (t->timeout)
                continue;
            if (t->transactionType == DATA_READ) {
                if (FAST_HQOS_PUSH_MODE == 0)
                    t->pri = PTC_QOS_MAX - 1;
                else if (FAST_HQOS_PUSH_MODE == 1) {
                    t->pri = PTC_QOS_MAX;
                    t->timeout = true;
                } else if (FAST_HQOS_PUSH_MODE == 2) {
                    t->pri = PTC_QOS_MAX - 1;
                    t->hqos_push = true;
                } else if (FAST_HQOS_PUSH_MODE == 3) {
                    t->pri = PTC_QOS_MAX;
                    t->hqos_push = true;
                }

            } else {
                if (FAST_HQOS_PUSH_MODE == 3) {
                    t->pri = PTC_QOS_MAX;
                    t->hqos_push = true;
                } else {
                    t->pri = PTC_QOS_MAX;
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- PTC : Fast Hqos Push, task={}, type={} pri={}, rank={}, bankIndex={}, "
                              "hqos_task={}, rank={}, bankIndex={}",
                    now(),
                    t->task,
                    t->transactionType,
                    t->pri,
    ...
    if (!trans->pre_act && fastread) {
        if ((trans->transactionType == DATA_READ && pfq_->wbuff_state == WBUFF_IDLE) ||
            (trans->transactionType == DATA_WRITE && pfq_->wbuff_state == WBUFF_WRITE) || PTC_BYP_UPDATE_EN) {
            if (state.state->openRowAddress == trans->row) {
                if (trans->transactionType == DATA_READ)
                    state.perf_rd_rowhit = true;
                else
                    state.perf_wr_rowhit = true;
            } else {
                if (trans->transactionType == DATA_READ)
                    state.perf_rd_rowmiss = true;
                else
            ...
            if (trans->transactionType == DATA_READ) {
                ...
                state.samebank_wcnt++;
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- same_bank_wcnt++ task={}", now(), trans->task);
                }
            }
        }
    ...
    if (fastread) {
        ...
        if (trans->transactionType == DATA_READ)
            DmcTotalReadBytes += trans->data_size;
        else
            DmcTotalWriteBytes += trans->data_size;
    ...
    if (pre_req_time == now() && !fastread) {
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- One Cycle One Cmd, pre_req_time={}", now(), pre_req_time);
        }
        return false;
    }
    ...
    if (((trans->timeout && trans->dummy_timeout) || trans->dummy_hqos) && !fastread) {
        ...
        if (DEBUG_BUS) {
            if (trans->dummy_timeout) {  // dummy timeout
                PRINT_SPD_PTC(ch, "{:10} -- ADD_DUMMY_TOUT :: type={} addr={:x} task={} bank={} row={} timeout={} "
                              "dummy_tout={} dummy_hqos={}",
                    now(),
                    trans->transactionType,
                    trans->address,
                    trans->task,
                ...
                PRINT_SPD_PTC(ch, "{:10} -- ADD_DUMMY_HQOS :: type={} addr={:x} task={} bank={} row={} timeout={} "
                              "dummy_tout={} dummy_hqos={}",
                    now(),
                    trans->transactionType,
                    trans->address,
        ...
        for (auto &t : transactionQueue) {
            if (trans->rank == t->rank && trans->bankIndex == t->bankIndex) {
                if (trans->dummy_timeout) {  // dummy timeout
                    if (DEBUG_BUS) {
                        PRINT_SPD_PTC(ch, "{:10} -- DUMMY_TOUT_PUSH :: task={} address={:x} rank={} bank={} row={} qos={} "
                                      "pri={} dummy_task={} dummy_bank={} dummy_row={} dummy_qos={} dummy_pri={}",
                            now(),
                            t->task,
                            t->address,
                            t->rank,
                    ...
                    if (!t->timeout) {
                        if (DEBUG_BUS) {
                            PRINT_SPD_PTC(ch, 
                                "{:10} -- DUMMY_HQOS_PUSH :: task={} address={:x} type={} rank={} bank={} row={} "
                                "qos={} pri={} dummy_task={} dummy_bank={} dummy_row={} dummy_qos={} dummy_pri={}",
                                now(),
                                t->task,
                                t->address,
                                t->transactionType,
    ...
    if (!full() && !fastread) {
        if (BSC_EN) {
            ...
            if (get_bs_full(trans)) {
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- BANK SLOT already reached the BS_BP_LVL::  bankIndex={} qos={} pri={} "
                                  "row={} type={} rank={} task={}\n",
                        now(),
                        trans->bankIndex,
                        trans->qos,
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- BANK SLOT after addTransaction:: ", now());
                for (unsigned i = 0; i < bank_slot.size(); i++) {
                    if (i != 0 && i % 4 == 0) {
                        PRINT_SPD_PTC(ch, "    ");
                    }
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, 
                "{:10} -- perf_qos={} pri={} arb_time={}", now(), trans->perf_qos, trans->pri, trans->arb_time);
        }
        ...
        if (trans->pre_act) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- PRE_ACT :: type{} addr={:x} task={} bank={} row={}",
                    now(),
                    trans->transactionType,
                    trans->address,
                    trans->task,
        ...
        if (BYP_ACT_EN && trans->transactionType == DATA_READ && (GetDmcQsize() == 0 || BYP_ACT_PIPE3_EN == true) &&
            ...
            (bank_cnt[trans->bankIndex] == 0 || BYP_ACT_PIPE3_EN == false)) {
            bool act_met = false;
            ...
            if (act_met) {
                ...
                bypass_active_cnt++;
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- BYP_ACT :: addr=0x{:x} task={} bank={}",
                        now(),
                        trans->address,
                        trans->task,
                        trans->bankIndex);
        ...
        if (trans->fast_rd) {
            ...
            if (trans->transactionType != DATA_READ || state.state->currentBankState != Idle) {
                ERROR(setw(10) << now() << " Wrong Fast Read Command, task=" << trans->task
                               << ", type=" << trans->transactionType
                               << ", state=" << bank_state_opcode(state.state->currentBankState)
                               << ", rank=" << trans->rank << " bank=" << trans->bankIndex);
                assert(0);
            }
            ...
            if (FAST_READ_MODE == 0) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- FAST_READ_ACT :: type={} addr={:x} task={} bank={} row={} act_only={} byp_act={}",
                        now(),
                        trans->transactionType,
                        trans->address,
                        trans->task,
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, 
                        "{:10} -- FAST_READ_RD :: type={} addr={:x} task={} bank={} row={} act_only={} byp_act={}",
                        now(),
                        trans->transactionType,
                        trans->address,
                        trans->task,
        ...
        if (MPAM_PUSH_EN)
            pushQosForSameMpamTrans(trans);
        if (MID_PUSH_EN)
            pushQosForSameMidTrans(trans);
        ...
        if (trans->transactionType == DATA_READ) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- ADD_ENT :: [R]B[{}]QOS[{}]MID[{}] addr=0x{:x} task={} rank={} group={} bank={} "
                              "bankIndex={} row={} col={} addr_col={} data_size={} Q={} QR={} QW={} ptc_timeAdded={} "
                              "timeout_th={} timeAdded={} mask_wcmd={} perf_rd_rowhit={} perf_rd_rowmiss={} "
                              "perf_wr_rowhit={} perf_wr_rowmiss={} perf_maxbg_hit={} perf_qos={} pri={} fast_rd={}",
                    now(),
                    trans->burst_length,
                    trans->qos,
                    trans->mid,
                    ...
                    trans->timeAdded,
                    trans->mask_wcmd,
                    trans->perf_rd_rowhit,
                    trans->perf_rd_rowmiss,
                    trans->perf_wr_rowhit,
                    trans->perf_wr_rowmiss,
                    trans->perf_maxbg_hit,
                    trans->perf_qos,
                    trans->pri,
                    trans->fast_rd);
            }
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- ADD_ENT :: [W]B[{}]QOS[{}]MID[{}] addr=0x{:x} task={} rank={} group={} bank={} "
                              "bankIndex={} row={} col={} addr_col={} data_size={} Q={} QR={} QW={} ptc_timeAdded={} "
                              "timeout_th={} timeAdded={} mask_wcmd={} perf_rd_rowhit={} perf_rd_rowmiss={} "
                              "perf_wr_rowhit={} perf_wr_rowmiss={} perf_maxbg_hit={} perf_qos={} pri={}",
                    now(),
                    trans->burst_length,
                    trans->qos,
                    trans->mid,
        ...
        if (trans->transactionType == DATA_READ) {
            ...
            state.ptc_samebank_wr++;
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- ptc_same_bank_wcnt++ task={}", now(), trans->task);
            }
        }
        ...
        if (state.perf_rd_rowmiss || state.perf_wr_rowmiss) {
            ...
            if (PTC_RHIT_BREAK_EN && bankStates[trans->bankIndex].ser_rhit_cnt >= PTC_RHIT_BREAK_LEVEL &&
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PTC(ch, "{:10} -- Rhit Break Trig :: addr=0x{:x} task={} rank={} group={} bank={}",
                        now(),
                        trans->address,
                        trans->task,
                        trans->rank,
        ...
        if (!SLOT_FIFO) {
            ...
            if (trans->arb_group >= ARB_GROUP_NUM) {
                ERROR(setw(10) << now() << " Wrong Arb Group, task=" << trans->task
                               << ", arb_group=" << trans->arb_group);
                assert(0);
            }
            if (DEBUG_BUS) {
                PRINT_SPD_PTC(ch, "{:10} -- Label New Cmd :: addr=0x{:x} task={} arb_group={} arb_group_cnt={} bank={}",
                    now(),
                    trans->address,
                    trans->task,
                    trans->arb_group,
        ...
        if (trans->fast_rd || trans->byp_act)
            transactionQueue.push_back(trans);
        else
            ent_delay_buf.push_back(trans);
        ...
        if (FASTWAKEUP_EN && !PREDICT_FASTWAKEUP && trans->transactionType == DATA_READ) {
            if (fast_wakeup_cnt[trans->rank] == 0 && now() >= 100) {
                ERROR(setw(10) << now() << " -- DMC[" << channel << "] rank:" << trans->rank
                               << ", Error fast wakeup count!");
                assert(0);
            }
        ...
        if (state.state->currentBankState == RowActive && trans->row == state.state->openRowAddress)
            state.row_hit_cnt++;
        else
            state.row_miss_cnt++;
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- CMD_BP :: addr=0x{:x} task={} bank={} QR={} QW={}",
                now(),
                trans->address,
                trans->task,
                trans->bankIndex,
...
{
    if (!IS_HBM3 || !trans->pre_act) {
        return false;
    }
    bool drop_preact = false;
    if (PREACT_FLOW_CTRL_EN && PREACT_FLOW_CTRL_TYPE == 0 && !SIMPLE_BANKTABLE_ENABLE) {
        drop_preact = table_use_cnt > PREACT_TBL_THD;
    } else if (PREACT_FLOW_CTRL_EN && PREACT_FLOW_CTRL_TYPE == 1) {
        drop_preact = availability > PREACT_BW_THD;
    }
    unsigned sub_channel = (trans->bankIndex % NUM_BANKS) / sc_bank_num;
    if (RankState[trans->rank].lp_state != IDLE || refreshALL[trans->rank][sub_channel].refreshWaiting ||
        refreshALL[trans->rank][sub_channel].refreshing || refreshPerBank[trans->bankIndex].refreshing ||
        refreshPerBank[trans->bankIndex].refreshWaiting) {
        drop_preact = true;
    }
    if (drop_preact) {
...
void PTC::trans_state_init(Transaction *trans)
{
    ...
    if (DEBUG_BUS) {
        PRINT_SPD_PTC(ch, "{:10} -- perf_qos={}, perf_pri={}, task={}", now(), perf_qos, perf_pri, trans->task);
    }
    if (!PTC_QOS_HOLD_EN) {
    ...
    if (MPAM_MAPPING_TIMEOUT && trans->mpam_id != 0) {  // mpam_id mapping timeout
        ...
        if (PTC_TIMEOUT_ENABLE)
            timeout = MAP_CONFIG[mpam_timeout][trans->mpam_id];
        if (PTC_PRI_ADPT_ENABLE)
            pri_adapt = MAP_CONFIG[mpam_adapt][trans->mpam_id];
        ...
        if (PTC_TIMEOUT_ENABLE)
            timeout = MAP_CONFIG[mpam_timeout][trans->qos];
        if (PTC_PRI_ADPT_ENABLE)
            pri_adapt = MAP_CONFIG[mpam_adapt][trans->qos];
    }
    if (timeout == 0)
        trans->timeout_th = 0;
    else
...
void PTC::dfs_backpress(bool backpress)
{
    ...
    if (DEBUG_BUS) {
        PRINT_SPD_PTC(ch, "{:10} -- DFS :: set dfs_backpress_en={}", now(), dfs_backpress_en);
    }
}
...
void PTC::push_pending_TransactionQue(Transaction *trans)
{
    TRANS_MSG msg;
    if (LAT_INC_BP)
        msg.time = uint64_t(trans->reqEnterDmcBufTime);
    else
        msg.time = now();
...
{
    if (sel == 0)
        return ptc_lru_matrix[index1][index2];
    else if (sel == 1)
        return bank_lru_matrix[index1][index2];
    else
        return -1;
}
void PTC::update_lru(uint64_t index, uint64_t sel, Cmd *cmd)
{
    if (sel == 0) {
        if (IS_LP5) {
            if (cmd->issue_size + cmd->bl * JEDEC_DATA_BUS_BITS / 8 != cmd->data_size) {
                return;
            }
        } else if (IS_LP6) {
            if (cmd->issue_size + cmd->bl * JEDEC_DATA_BUS_BITS / 9 != cmd->data_size) {
                return;
            }
        }
        for (size_t i = 0; i < TRANS_QUEUE_DEPTH; i++) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- LRU :: update lru matrix={}, index={}, task={}", now(), sel, index, cmd->task);
        }
    }
    if (sel == 1) {
        for (size_t i = 0; i < (NUM_BANKS * NUM_RANKS); i++) {
...
void PTC::ent_pop()
{
    if (!ent_delay_buf.empty()) {
        if (DEBUG_BUS) {
            PRINT_SPD_PTC(ch, "{:10} -- ADD_PTC :: [{}] task={}",
                now(),
                trans_type_opcode(ent_delay_buf[0]->transactionType),
                ent_delay_buf[0]->task);
        }
...
void PTC::ooo_statis(uint64_t id)
{
    // cout<<"begin to statis ooo"<<endl;
    if (ooo_queue.find(id) == ooo_queue.end()) {
        cout << "ERROR: id not found\n";
        return;
    }

    uint64_t arr = ooo_queue[id];
    uint64_t comp = completion_counter++;

    uint64_t depth = std::llabs((long long)comp - (long long)arr);
