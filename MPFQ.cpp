...
SvZMzBtMrQ+w1lxbG5+EFWux51V2bvtZUTAAA+en/pM7ZB5Cy3u0JTs1VqxXwkl+8sQkIumgSmWx
HkZzKwKLrHyw6tVp9O5tjxe1IGMO5KVCJnM9WK7kAiYNwEafpAIRotDJ7cMCXi851XlHkGXOWTi3
uNHFsPx4yyLVhINX# */
...
MPFQ:: MPFQ(MemorySystemTop *top, vector<ofstream> &DDRSim_log_, vector<ofstream> &trace_log_)
     :  memorySystemTop_(top), DDRSim_log(DDRSim_log_), trace_log(trace_log_), ReturnReadData(NULL), WriteResp(NULL),
      ReadResp(NULL), CmdResp(NULL)
{
...
void MPFQ:: addFastRead(unsigned pfq_idx)
{
    ...
    if (!mptc_->getPTC(chl)->fastrd_fifo.empty()) {
        bool fastrd_del = (pfqs_[pfq_idx]->wbuff_state == WBUFF_WRITE) ||
                          (mptc_->getPTC(chl)->rank_cnt[fastrd_trans->rank] >= PTC_MAX_CNT_PER_RANK ||
                              refresh_waiting || refresh_all_waiting || refreshing || refreshing_all) ||
                          perf_rnkgrp || ptc_full || ptc_bs_full;
        if (fastrd_del) {
            if (DEBUG_BUS) {
                PRINTN_M(pfq_idx,
                    setw(10) << now() << " -- Fast Read Cmd Unvalid :: task=" << fastrd_trans->task
                             << ", rank=" << fastrd_trans->rank << ", bank=" << fastrd_trans->bankIndex
                             << ", perf_state=" << pfqs_[pfq_idx]->wbuff_state
    ...
    if (!mptc_->getPTC(chl)->fastrd_fifo.empty() &&
        ((((now() % CLKH2CLKL == 1) || (now() % CLKH2CLKL == 0 && !pfqs_[pfq_idx]->perf_cmd_vld)) &&
             PERF_CLK_MODE == 1) ||
            PERF_CLK_MODE == 0)) {
        if (DEBUG_BUS) {
            PRINTN_M(pfq_idx,
                setw(10) << now() << " -- pfq[" << pfq_idx << "]->perf_cmd_vld=" << pfqs_[pfq_idx]->perf_cmd_vld
                         << ", now()%CLKH2CLKL=" << now() % CLKH2CLKL << endl);
        }
        ...
        if (state.ptc_samebank_rd > 0 || mptc_->getPTC(chl)->r_bank_cnt[fastrd_trans->bankIndex] > 0 ||
            ...
            ERROR(setw(10) << now() << " Fast Read Conflict with Ptc Cmd, task=" << fastrd_trans->task
                           << ", ptc_samebank_rd=" << state.ptc_samebank_rd << ", r_bank_cnt"
                           << mptc_->getPTC(chl)->r_bank_cnt[fastrd_trans->bankIndex] << ", perf_rd_conf="
                           << state.perf_bankrd_conflict << ", ptc_samebank_rd=" << state.ptc_samebank_wr
                           << ", w_bank_cnt" << mptc_->getPTC(chl)->w_bank_cnt[fastrd_trans->bankIndex]
        ...
        if (fastrd_pos) {
            if (DEBUG_BUS) {
                PRINTN_M(pfq_idx,
                    setw(10) << now() << " -- ADD_DMC (FAST_RD) :: [R]B[" << fastrd_trans->burst_length << "]QOS["
                             << fastrd_trans->qos << "]MID[" << fastrd_trans->mid << "] addr=0x" << hex
                             << fastrd_trans->address << dec << " task=" << fastrd_trans->task << " rank="
                             << fastrd_trans->rank << " group=" << fastrd_trans->group << " bank=" << fastrd_trans->bank
                             << " bankIndex=" << fastrd_trans->bankIndex << " row=" << fastrd_trans->row
                             << " col=" << fastrd_trans->col << " addr_col=" << fastrd_trans->addr_col
                             << " data_size=" << fastrd_trans->data_size << " Q=" << mptc_->getPTC(chl)->GetDmcQsize()
                             << " ptc_timeAdded=" << fastrd_trans->ptc_timeAdded
                             << " timeout_th=" << fastrd_trans->timeout_th << " timeAdded=" << fastrd_trans->timeAdded
                             << " mask_wcmd=" << fastrd_trans->mask_wcmd << " perf_rd_rowhit="
                             << fastrd_trans->perf_rd_rowhit << " perf_rd_rowmiss=" << fastrd_trans->perf_rd_rowmiss
                             << " perf_maxbg_hit=" << fastrd_trans->perf_maxbg_hit
                             << " perf_qos=" << fastrd_trans->perf_qos << " pri=" << fastrd_trans->pri
                             << " byp_act=" << fastrd_trans->byp_act << " fast_rd=" << fastrd_trans->fast_rd
                             << " act_only=" << fastrd_trans->act_only << endl);
            }
            ...
            for (auto &rtrans : pfqs_[pfq_idx]->PerfQue) {
                if (rtrans == nullptr)
                    continue;
                if (rtrans->transactionType != DATA_READ)
                    continue;
                if (rtrans->task != fastrd_trans->task)
                    continue;
                exist_fastrd = true;
                ...
                if (DEBUG_BUS) {
                    PRINTN_M(pfq_idx,
                        setw(10) << now() << " -- SET FAST_RD IN PERF, task=" << rtrans->task
                                 << " fast_rd=" << rtrans->fast_rd << " act_only=" << rtrans->act_only << endl);
                }
            }

            if (!exist_fastrd) {
                ERROR(setw(10) << now() << " Fast Rd Not Found in Perf :: task=" << fastrd_trans->task
                               << " fast_rd=" << fastrd_trans->fast_rd << " act_only=" << fastrd_trans->act_only);
                assert(0);
            }

            mptc_->getPTC(chl)->fastrd_fifo.erase(mptc_->getPTC(chl)->fastrd_fifo.begin());
        } else {
            ERROR(setw(10) << now() << " Fast Read Cmd Met, but PTC reject :: task=" << fastrd_trans->task
                           << ", ptc_size=" << mptc_->getPTC(chl)->transactionQueue.size() << ", channel=" << chl
                           << ", ptc_pre_req_time=" << mptc_->getPTC(chl)->pre_req_time);
            assert(0);
        }
    }
}
...
// ============================
void MPFQ:: RegisterCallbacks(Callback_t *readData, Callback_t *writeCB, Callback_t *readCB, Callback_t *cmdCB)
{
