...
SvZMzBtMrQ+w1lxbG5+EFWux51V2bvtZUTAAA+en/pM7ZB5Cy3u0JTs1VqxXwvOu7YEhTo2mUyZU
1u8AHTgAUiU/AuWZTxD5oz6U+anP++obRVqRVvWIfX5Ug2wDpSi9SrcmWMdLSpOmST5gvIGNPl7b
meAIR7tCQT2ZFf9t# */
...
{
    if (IS_LP6) {
        return (float(100 * total_bytes * 8) / cnt / 2 / JEDEC_DATA_BUS_BITS / WCK2DFI_RATIO / PAM_RATIO * 9 / 8);
    } else {
        return (float(100 * total_bytes * 8) / cnt / 2 / JEDEC_DATA_BUS_BITS / WCK2DFI_RATIO / PAM_RATIO);
    }
}

/* callback functors */
bool some_object::read_data(
    unsigned channel, uint64_t task, double readDataEnterDmcTime, double reqAddToDmcTime, double reqEnterDmcBufTime)
{
        ...
        if (task >= 0xF000000000000000) {
            ...
            if (it != OutstandingQueueTest.end()) {
                ...
                if (loadlatency_cnt == T_NUM) {
                    ...
                    DEBUG("Core 0 exit after " << loadlatency_cycle << " cycles, " << loadlatency_cnt
                                               << " instructions, 16921 uops, 0.0011 ipc");
...
bool some_object::write_response(
    unsigned channel, uint64_t task, double readDataEnterDmcTime_, double reqAddToDmcTime_, double reqEnterDmcBufTime_)
{
...
bool some_object::read_response(
    unsigned channel, uint64_t task, double readDataEnterDmcTime, double reqAddToDmcTime, double reqEnterDmcBufTime)
{
    return true;
}

bool some_object::cmd_response(
    unsigned channel, uint64_t task, double readDataEnterDmcTime, double reqAddToDmcTime, double reqEnterDmcBufTime)
{
    return true;
}
...
void get_line()
{
    ...
    while (!file.eof()) {
        if (TRACE_CMD_NUM != 0 && line_number > TRACE_CMD_NUM)
            break;
        getline(file, line);
        if (line_number != 0 && line.size() != 0) {
            ...
            // f_time=0;
            //f_time = f_time * (9600.0f / DMC_RATE);
            //if(NUM_CHANS > 1) f_time = f_time * float(((f_data_length==128)?0.7 : 0.6));
                ...
                if (CH_XOR_EN) {
                    if (CHINTLV_BIT == 2) {  // addr7/8
                        ...
                        intlv_ch_num |= (bitset<64>(f_address).test(8) ^ bitset<64>(f_address).test(10) ^
                                            bitset<64>(f_address).test(12) ^ bitset<64>(f_address).test(14))
                                        << 1;
            ...
            if (valid_command) {
                ...
                if (TRACE_EN) {
                    ...
                    if (CH_XOR_EN)
                        ...
                        start_bit = CHINTLV_START;
                    unsigned addr_mask_low = 0;
                    for (size_t i = 0; i < start_bit; i++) {
                        addr_mask_low = (addr_mask_low << 1) | 1;
                    }
                    transaction.address =
                        (f_address & addr_mask_low) | ((f_address >> (start_bit + CHINTLV_BIT)) << start_bit);
                }
                ...
                // transaction.channel = task_cnt % 2;
                if (NUM_CHANS > 1) {
                    if(TRACE_EN){
...
void rand_command(MemorySystemTop *ddrc, bool is_test_cmd)
{
    ...
    if (SEQ_NUM == 0 && !is_test_cmd) {
        ...
        if (is_read)
            transaction.address = r_address + data_size;
        else
            transaction.address = r_address + data_size;
        if (RK_SW_RATIO > 0) {
            if (is_rank0) {
        ...
        if (seq_cnt == 0) {
            ...
            if (is_read)
                ...
                transaction.address = w_address + DATA_SIZE;
        }
    }
    if (SAME_ADDR_MODE)
...
void send_command(MemorySystemTop *ddrc)
{
    ...
    for (size_t i = 0; i < size; i++) {
        if (CommandQueue[i].size() == 0)
            continue;
        if (test_cmd_time_met && !CommandQueue[i].empty()) {
            if (WRITE_BUFFER_ENABLE) {
                bool bus_rempty = true;
                for (auto cmd : CommandQueue[i]) {
                    if ((cnt + 40) < uint64_t(cmd.reqEnterDmcBufTime))
                        break;
                    if (cmd.type == DATA_WRITE)
                        continue;
                    bus_rempty = false;
                    break;
                }
                if (FASTWAKEUP_CYCLE <= 0)
                    mem->noc_read_inform(0, false, false, bus_rempty);
            }
...
{
    SPD_LOGGER::esl_log::parse_log_config("../DDRC_MODEL/parameter/log_config.toml");
    ...
    rdata_cb = new DRAMSim::Callback<some_object, bool, unsigned, uint64_t, double, double, double>(
        &obj, &some_object::read_data);
    write_cb = new DRAMSim::Callback<some_object, bool, unsigned, uint64_t, double, double, double>(
        &obj, &some_object::write_response);
    read_cb = new DRAMSim::Callback<some_object, bool, unsigned, uint64_t, double, double, double>(
        &obj, &some_object::read_response);
    cmd_cb = new DRAMSim::Callback<some_object, bool, unsigned, uint64_t, double, double, double>(
        &obj, &some_object::cmd_response);

    mem = new MemorySystemTop(0, PARAM_PATH, "./", argc, argv);
    ...
    while (1) {
        g_soc_time = cnt / tDFI;  // add for UT
        ...
        if (TRACE_EN || DOU_TRACE_EN) {
            ...
            if (trace_send_cnt >= task_cnt) {
                float efficiency = 0;
                efficiency = calc_effi();
                DEBUG("Done, time: " << cnt << ", total command cnt: " << task_cnt << ", efficiency: " << fixed
            ...
            if (LATENCY_MODE)
                rand_command(mem, true);
            if (PRINT_IDLE_LAT && cnt >= 1000)
                exit(0);
            if (!PRINT_IDLE_LAT || (cnt % 1000 == 0))
                rand_command(mem, false);
        }
        send_command(mem);
        send_wdata(mem);
        if (MATCH_MODE) {
