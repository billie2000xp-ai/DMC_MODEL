...
PFQ:: PFQ(unsigned index, MemorySystemTop *top, MPFQ *mpfq, ofstream &DDRSim_log_, ofstream &trace_log_)
    :  id_(index), memorySystemTop_(top), mpfq_(mpfq), DDRSim_log(DDRSim_log_), trace_log(trace_log_)
{
    ...
    for (size_t i = 0; i < PERF_SUBQ_NUM; i++) {
        for (size_t j = 0; j < PERF_SUBQ_NUM; j++) {
            if (i < j)
                subq_lru_matrix[i][j] = 1;
            else
                subq_lru_matrix[i][j] = 0;
        }
    }
    ...
    //         if (i < j)
    //             cmd1_lru_matrix[i][j] = 1;
    //         else
    //             cmd1_lru_matrix[i][j] = 0;
    //     }
    // }
    ...
    for (size_t i = 0; i < PERF_SUBQ_NUM; i++) {
        for (size_t j = 0; j < PERF_SUBQ_NUM; j++) {
            if (i < j)
                cmd2_lru_matrix[i][j] = 1;
            else
                cmd2_lru_matrix[i][j] = 0;
        }
    }
    ...
    for (auto &wrel_bank : wrel_bank_cnt)
        wrel_bank.resize(NUM_BANKS);
    for (auto &rrel_bank : rrel_bank_cnt)
        rrel_bank.resize(NUM_BANKS);
    ...
    for (size_t i = 0; i < 4; i++) {
        wr_level[i].clear();
        wr_level[i].resize(5);
        wr_most_level[i].clear();
        wr_most_level[i].resize(5);
        rd_level[i].clear();
        rd_level[i].resize(5);
        grp_mode[i].clear();
        grp_mode[i].resize(5);
        ...
        for (size_t j = 0; j < 5; j++) {
            wr_level[i].push_back(0);
            wr_most_level[i].push_back(0);
            rd_level[i].push_back(0);
            grp_mode[i].push_back(0);
        }
    }
    ...
    sid_group_target.resize(NUM_RANKS, NUM_SIDS);
    sid_group_next_target.resize(NUM_RANKS, NUM_SIDS);
    sid_group_switch_count.resize(NUM_RANKS, 0);
    sid_group_serial_count.resize(NUM_RANKS, 0);
    sid_group_timeout.resize(NUM_RANKS, std:: vector<unsigned>(NUM_SIDS, 0));
...
bool PFQ:: addTransaction(Transaction *trans)
{
    if (WSRAM_MAP_EN) {
        ...
        bool wsram_ready = 
            (trans->transactionType == DATA_WRITE) && (trans->pre_act || enoughWSRAM(trans, trans->burst_length));
        unsigned need_size = wdataQueNeedSize(trans, trans->data_size);
        if (need_size == 0) need_size = 4; // 保底防护，防止 pre_act 导致除 0 或死循环
        for (unsigned i = 0; i <= (WSRAM_QUEUE_DEPTH - need_size); i += need_size) {
            bool current_chunk_idle = true;
            for (unsigned j = 0; j < need_size; j++) {
                if (wsram_slt[i+j].state != SLT_IDLE) {
                    current_chunk_idle = false;
                    break;
                }
            }
            if (current_chunk_idle) {
                slot_idle = true;
                break;
            }
        }
        if ((!wsram_ready) && trans->transactionType == DATA_WRITE) {
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- WSRAM_FULL_BP :: task={} type={} qos={} "
                              "burst_length:{} address={:x} rank={} bank={} row={} "
                              "(R:{}|W:{}|RR:{}|RW:{})",
                    now(),
                    trans->task,
        ...
        if ((!slot_idle) && trans->transactionType == DATA_WRITE) {
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- CAN'T ACCEPT 128B BP :: task={} type={} qos={} burst_length:{} address={:x} "
                              "rank={} bank={} row={} (R:{}|W:{}|RR:{}|RW:{})",
                    now(),
                    trans->task,
                    trans->transactionType,
                    trans->qos,
    ...
    if (perf_full()) {
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, "{:10} -- PERF_FULL_DROP :: task={} type={} qos={} burst_length:{} address={:x} rank={} "
                          "bank={} row={} (R:{}|W:{}|RR:{}|RW:{})",
                now(),
                trans->task,
                trans->transactionType,
    ...
    linked_ptcs_[ptcID(trans->channel)]->ooo_queue[trans->task] = 
        linked_ptcs_[ptcID(trans->channel)]->arrival_counter++;
    trans->timeAdded = now();
    if (!curClkCanAddTrans()) {
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, "{:10} -- CLK1X_DROP :: task={} type={} qos={} burst_length:{} address={:x} rank={} bank={} "
                          "row={} (R:{}|W:{}|RR:{}|RW:{})",
                now(),
                trans->task,
                trans->transactionType,
    ...
    if (trans->transactionType == DATA_READ) {
        ...
        if (FORWARD_ENABLE && trans->data_size == 64 && read_forward(trans))
            return true;
        if (RQ_ADCONF_PUSH_EN)
            rcmd_push_wcmd(trans);
        ...
        if (PerfQue_pushCmd(trans)) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, 
                    "{:10} -- PERF_ADD :: [R]B[{}]QOS[{}] addr={:x} task={} rank={} sid={} group={} bank={} row={} col={} "
                    "addr_col={} data_size={} timeAdded={} timeout_th={} inject_time={} (R:{}|W:{}|RR:{}|RW:{})",
                    now(),
                    trans->burst_length,
                    trans->qos,
                    trans->address,
        ...
        if (MERGE_ENABLE && trans->data_size == 64 && write_merge(trans))
            return true;
        ...
        if (WSRAM_MAP_EN) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, 
                    "{:10} -- WSRAM :: before success={} need_size={} wsram_idx={} need_size={} used_wdata_que_cnt={}",
                    now(),
                    success,
                    need_size,
                    trans->wsram_idx,
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, 
                    "{:10} -- WSRAM :: after success={} need_size={} wsram_idx={} need_size={} used_wdata_que_cnt={}",
                    now(),
                    success,
                    need_size,
                    trans->wsram_idx,
            ...
            if (DEBUG_BUS) {
                    ...
                    for (unsigned i = 0; i < msize_bmp.size(); i++) {
                        ...
                        line += fmt::format("{} ", msize_bmp[i]);
                    }
                    PRINT_SPD_PFQ(ch, "{:10} -- WSRAM :: msize_bmp={}", now(), line);
            ...
            if (DEBUG_BUS) {
                ...
                for (unsigned i = 0; i < msize_single.size(); i++) {
                    ...
                    line += fmt::format("{} ", msize_single[i]);
                }
                PRINT_SPD_PFQ(ch, "{:10} -- WSRAM :: msize_single={}", now(), line);
            }
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- WSRAM :: OCCUPY, TRAN ACCEPT task={} data_size={} occupy_size={} wsram_idx={} "
                              "used_size_after_occupy={}",
                    now(),
                    trans->task,
                    trans->data_size,
                    wdataQueNeedSize(trans, trans->data_size),
                    trans->wsram_idx,
                    used_wdata_que_cnt);
                std::string line;
                for (unsigned i = 0; i < wsram_slt.size(); i++) {
                    ...
                    line += fmt::format("{} ", wsram_slt[i].state);
                }
                PRINT_SPD_PFQ(ch, "{:10} -- WSRAM :: {}", now(), line);
        ...
        if (PerfQue_pushCmd(trans)) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- PERF_ADD :: [W]B[{}]QOS[{}] addr={:x} task={} rank={} sid={} group={} bank={} row={} "
                              "col={} addr_col={} data_size={} timeAdded={} timeout_th={} wsram_idx={} channels={} "
                              "fast_rd={} inject_time={} (R:{}|W:{}|RR:{}|RW:{})",
                    now(),
                    trans->burst_length,
                    trans->qos,
                    trans->address,
...
bool PFQ:: findFreeWsram(Transaction *trans, unsigned need_size)
{
    ...
    if (need_size == 2 || need_size == 1) {
        ...
        for (unsigned k = 0; k <= msize_single.size() - 1; k++) {
            if (msize_single[k] == 1) {
                if (msize_bmp[2 * k] == 1 && msize_bmp[2 * k + 1] == 0) {
                    ...
                    if (wsram_slt[(2 * k + 1) * 2].state == SLT_IDLE &&
                        wsram_slt[(2 * k + 1) * 2 + 1].state == SLT_IDLE) {
        ...
        if (DEBUG_BUS) {
            std::string msize_bmp_line;
            for (unsigned i = 0; i < msize_bmp.size(); i++) {
                if (i != 0 && i % 4 == 0) {
                    msize_bmp_line += "    ";
                }
                msize_bmp_line += fmt::format("{} ", msize_bmp[i]);
            }
            PRINT_SPD_PFQ(ch, "{:10} -- WSRAM :: msize_bmp={}", now(), msize_bmp_line);
            
        }
        if (DEBUG_BUS) {
            std::string msize_single_line;
            for (unsigned i = 0; i < msize_single.size(); i++) {
                if (i != 0 && i % 4 == 0) {
                    msize_single_line += "    ";
                }
                msize_single_line += fmt::format("{} ", msize_single[i]);
            }
            PRINT_SPD_PFQ(ch, "{:10} -- WSRAM :: msize_single={}", now(), msize_single_line);
    ...
    } else if (need_size == 8 || need_size == 7) { 
        for (unsigned loc = 0; loc <= (WSRAM_QUEUE_DEPTH - 1); loc += 8) {
            // 防止越界
            if (loc + 7 >= WSRAM_QUEUE_DEPTH) break; 
            
            bool find_loc = true;
            for (unsigned i = 0; i < 8; i++) {
                if (wsram_slt[loc+i].state != SLT_IDLE) {
                    find_loc = false; break;
                }
            }
            if (find_loc) {
                trans->wsram_idx = loc;
...
bool PFQ:: enoughWSRAM(Transaction *trans, uint32_t burst_length)
{
    ...
    if (DEBUG_BUS) {
        PRINT_SPD_PFQ(ch, "{:10} -- WSRAM :: enough func need_wdata_que={} used_wdata_que_cnt={} WSRAM_QUEUE_DEPTH={} "
                      "(R:{}|W:{}|RR:{}|RW:{})",
            now(),
            need_wdata_que,
            used_wdata_que_cnt,
            ...
            rel_rcmd_cnt,
            rel_wcmd_cnt);
            PRINT_SPD_PFQ(ch, "{:10} -- WSRAM :: enough= {}", now(), ret);
            std::string wsram_slt_line;
            for (unsigned i = 0; i < wsram_slt.size(); i++) {
                if (i != 0 && i % 4 == 0) {
                    wsram_slt_line += "    ";
                }
                wsram_slt_line += fmt::format("{} ", wsram_slt[i].state);
            }
            PRINT_SPD_PFQ(ch, "{:10} -- WSRAM :: wsram_slt={}", now(), wsram_slt_line);
// 
        // 
    }
    return ret;
...
void PFQ:: genCresp(Transaction *trans)
{
    ...
    if (((trans->transactionType == DATA_WRITE && !trans->mask_wcmd) ||
            (trans->transactionType == DATA_READ && trans->mask_wcmd)) &&
...
bool PFQ:: read_forward(Transaction *trans)
{
    for (auto &w : PerfQue) {
        ...
        if (w->in_ptc)
            continue;
        if (trans->rank != w->rank)
            continue;
        if (trans->bankIndex != w->bankIndex)
            continue;
        if (trans->row != w->row)
            continue;
        if (w->data_ready_cnt <= w->burst_length)
            continue;
        if (trans->col >= w->col && (trans->col + 64 / bytes_per_col) <= (w->col + 64 / bytes_per_col)) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- GBUF_FORWARD :: task={} address={:x} (R:{}|W:{})",
                    now(),
                    trans->task,
                    trans->address,
                    rcmd_cnt,
...
void PFQ:: rcmd_push_wcmd(Transaction *t)
{
    for (auto &w : PerfQue) {
        ...
        if (w->in_ptc)
            continue;
        if (w->transactionType == DATA_READ)
            continue;
        if (t->bankIndex == w->bankIndex && t->row == w->row && (t->addr_col < (w->addr_col + w->data_size)) &&
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- ADD_PUSH :: task={} address={:x} rank={} bank={} row={} w_task={} "
                              "w_adtimeout={} w_tout_cnt={}",
                    now(),
                    t->task,
                    t->address,
                    t->rank,
...
void PFQ:: rcmd_set_conflict(Transaction *t)
{
    ...
    for (auto &w : PerfQue) {
        ...
        if (t->bankIndex == w->bankIndex && t->row == w->row && (t->addr_col < (w->addr_col + w->data_size)) &&
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- RCMD_SET_CONF :: task={} address={:x} rank={} bank={} row={} col={} g_bg_cc={} "
                              "g_ba_cc={} g_ad_cc={} w_task={} rank={} bank={} row={} col={}",
                    now(),
                    t->task,
                    t->address,
                    t->rank,
...
void PFQ:: wcmd_set_conflict(Transaction *t)
{
    ...
    for (auto &cmd : PerfQue) {
        if (cmd == nullptr)
            continue;
        if (t->group == cmd->group)
            c->perf.bg_conf_cnt++;
        if (t->bankIndex == cmd->bankIndex)
            c->perf.ba_conf_cnt++;
        if (t->bankIndex == cmd->bankIndex && t->row == cmd->row && (t->addr_col < (cmd->addr_col + cmd->data_size)) &&
            ...
            if (DEBUG_BUS) {
                if (cmd->transactionType == DATA_READ) {
                    PRINT_SPD_PFQ(ch, "{:10} -- WCMD_SET_CONF_R :: task={} address={:x} rank={} bank={} row={} col={} "
                                  "g_bg_cc={} g_ba_cc={} g_ad_cc={} r_task={} rank={} bank={} row={} col={}",
                        now(),
                        t->task,
                        t->address,
                        t->rank,
                    ...
                    PRINT_SPD_PFQ(ch, "{:10} -- WCMD_SET_CONF_W :: task={} address={:x} rank={} bank={} row={} col={} "
                                  "g_bg_cc={} g_ba_cc={} g_ad_cc={} w_task={} rank={} bank={} row={} col={}",
                        now(),
                        t->task,
                        t->address,
                        t->rank,
...
{
    if (sel == 0)
        return subq_lru_matrix[index1][index2];
    else if (sel == 1) {
        // 命令级 LRU，每个子队列独立
        ...
        return cmd2_lru_matrix[index1][index2];
    else
        return -1;
}
...
void PFQ:: update_lru(uint64_t index, uint64_t sel, arb_cmd *cmd)
{
    if (sel == 0) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, "{:10} -- LRU :: update subq lru matrix={}, index={}, task={}", now(), sel, index, cmd->task);
        }
    }
    if (sel == 1) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, 
                "{:10} -- LRU :: update cmd lv1 sel lru matrix={}, index={}, task={}", now(), sel, index, cmd->task);
        }
    }
    if (sel == 2) {
        for (size_t i = 0; i < PERF_SUBQ_NUM; i++) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, 
                "{:10} -- LRU :: update cmd lv2 sel lru matrix={}, index={}, task={}", now(), sel, index, cmd->task);
        }
    }
}
    ...
    for (size_t subq_idx = 0; subq_idx < PERF_SUBQ_NUM; subq_idx++) {
        ...
        if (findEmptySlotInSubq(subq_idx) != static_cast<size_t>(-1)) {
            if (best_subq == static_cast<size_t>(-1)) {
                ...
                if (lru_arb(subq_idx, best_subq, 0))
                    best_subq = subq_idx;
            }
        }
    }
...
bool PFQ:: PerfQue_pushCmd(Transaction *trans)
{
    ...
    if (insert_pos != static_cast<size_t>(-1)) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, "{:10} -- CMD slot_index ={}, subq={}, task={}", now(), insert_pos, target_subq, cmd->task);
        }
        return true;
    } else {
        return false;
...
bool PFQ:: write_merge(Transaction *trans)
{
    for (auto &w : PerfQue) {
        ...
        if ((trans->address & ALIGNED_DATA_64B) == (w->address & ALIGNED_DATA_64B) &&
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- PERF_MERGE :: Send task={} address={:x}, Merge task={} address={:x}",
                    now(),
                    trans->task,
                    trans->address,
                    w->task,
...
/*==================================================================================================
Descriptor: ADD DATA FIELD
================================================================================================= == */

/***************************************************************************************************
Descriptor: main addData
****************************************************************************************************/
bool PFQ:: addData(uint32_t *data, uint64_t task)
{
    ...
    if (!curClkCanAddData() && PERF_CLK_MODE == 1) {
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, "{:10} -- ADD DATA BP :: task={}", now(), task);
        }
        return false;
    }
    ...
    for (auto &trans : PerfQue) {
        ...
        if (trans->data_ready_cnt >= (trans->burst_length + 1)) {
            ERROR(
                setw(10) << now() << "Too Much Data, task=" << trans->task << ", data_ready_cnt=" << trans->data_size);
            assert(0);
        }
        if (WSRAM_MAP_EN) {
            unsigned slot_idx = trans->wsram_idx + trans->data_ready_cnt;
            if (wsram_slt[slot_idx].state != PENDING) {
                ERROR(setw(10) << now() << "Invalid wsram state transition at slot" << slot_idx
                               << ", expected PENDING but got " << wsram_slt[slot_idx].state
                               << " for task=" << trans->task);
                assert(0);
            }
            if (wsram_slt[slot_idx].task != task) {
                ERROR(setw(10) << now() << "Invalid wdata task at slot" << slot_idx << ", expected task=" << task
                               << "but got " << wsram_slt[slot_idx].task);
                assert(0);
            }
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, "{:10} -- PERF_MATCH :: data_ready_cnt:{}, {}, task={}",
                now(),
                trans->data_ready_cnt,
                trans->data_size,
                trans->task);
        ...
        if ((trans->transactionType == DATA_WRITE && trans->mask_wcmd && RMW_ENABLE_PERF && IS_LP6) &&
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- RMW_WDATA_COLLECTED :: task={} type={} qos={} burst_length:{} address={:x} "
                              "rank={} bank={} row={} (R:{}|W:{}|RR:{}|RW:{}) timeadded={}",
                    now(),
                    trans->task,
                    trans->transactionType,
                    trans->qos,
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- RMW_CMD W2R_conversion completed for task{}", now(), trans->task);
            }
        }
        wbuf_exist = true;
        break;
...
void PFQ:: rcmd_release_conflict(Transaction *trans)
{
    for (auto &w : PerfQue) {
        ...
        if (w->transactionType == DATA_READ)
            continue;
        if (w->conflict_state == nullptr)
            continue;
        if (trans->group == w->group)
            PROTECT_SUB(w->conflict_state->perf.bg_conf_cnt);
        if (trans->bankIndex == w->bankIndex)
            PROTECT_SUB(w->conflict_state->perf.ba_conf_cnt);
        if (trans->bankIndex == w->bankIndex && trans->row == w->row &&
            (trans->addr_col < (w->addr_col + w->data_size)) && ((trans->addr_col + trans->data_size) > w->addr_col)) {
            PROTECT_SUB(w->conflict_state->perf.ad_conf_cnt);
            if (w->conflict_state->perf.ad_conf_cnt == 0)
                w->perf_addrconf = false;
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- RCMD_REL_CONF :: task={} address={:x} rank={} bank={} row={} g_bg_cc={} "
                              "g_ba_cc={} g_ad_cc={} r_task={}",
                    now(),
                    w->task,
                    w->address,
                    w->rank,
                    w->bankIndex,
                    w->row,
                    w->conflict_state->perf.bg_conf_cnt,
                    w->conflict_state->perf.ba_conf_cnt,
                    w->conflict_state->perf.ad_conf_cnt,
                    trans->task);
            }
        }
    }
...
void PFQ:: wcmd_release_conflict(Transaction *trans)
{
    for (auto &cmd : PerfQue) {
        ...
        if (cmd->in_ptc)
            continue;
        if (cmd->transactionType == DATA_WRITE && trans->task == cmd->task)
            continue;
        if (cmd->transactionType == DATA_READ && trans->task == cmd->task && trans->mask_wcmd)
            continue;
        if (cmd->conflict_state == nullptr)
            continue;
        if (trans->group == cmd->group)
            PROTECT_SUB(cmd->conflict_state->perf.bg_conf_cnt);
        if (trans->bankIndex == cmd->bankIndex)
            PROTECT_SUB(cmd->conflict_state->perf.ba_conf_cnt);
        if (trans->bankIndex == cmd->bankIndex && trans->row == cmd->row &&
            ...
            if (cmd->conflict_state->perf.ad_conf_cnt == 0)
                cmd->perf_addrconf = false;
            if (DEBUG_BUS) {
                if (cmd->transactionType == DATA_READ) {
                    PRINT_SPD_PFQ(ch, "{:10} -- WCMD_REL_CONF_R :: task={} address={:x} rank={} bank={} row={} g_bg_cc={} "
                                  "g_ba_cc={} g_ad_cc={} w_task={}",
                        now(),
                        cmd->task,
                        cmd->address,
                        cmd->rank,
                        cmd->bankIndex,
                        cmd->row,
                        cmd->conflict_state->perf.bg_conf_cnt,
                        cmd->conflict_state->perf.ba_conf_cnt,
                        cmd->conflict_state->perf.ad_conf_cnt,
                        trans->task);
                } else if (cmd->transactionType == DATA_WRITE) {
                    PRINT_SPD_PFQ(ch, "{:10} -- WCMD_REL_CONF_W :: task={} address={:x} rank={} bank={} row={} g_bg_cc={} "
                                  "g_ba_cc={} g_ad_cc={} w_task={}",
                        now(),
                        cmd->task,
                        cmd->address,
...
void PFQ:: dmc_release_conflict(Transaction *trans)
{
    for (auto &cmd : PerfQue) {
        ...
        if (dmc_conf_state == nullptr)
            continue;
        if (dmc_conf_state->dmc.bg_conf_cnt > 0 || dmc_conf_state->dmc.ba_conf_cnt > 0 ||
            dmc_conf_state->dmc.ad_conf_cnt > 0) {
            ERROR("PTC Conf Value Wrong, task=" << cmd->task << ", bankIndex=" << cmd->bankIndex
                                                << ", dmc_bgcc=" << dmc_conf_state->dmc.bg_conf_cnt
                                                << ", dmc_bacc=" << dmc_conf_state->dmc.ba_conf_cnt
                                                << ", dmc_adcc=" << dmc_conf_state->dmc.ad_conf_cnt);
            assert(0);
        }
        ...
        if (cmd->in_ptc)
            continue;
        if (trans->group == cmd->group)
            PROTECT_SUB(dmc_conf_state->dmc.bg_conf_cnt);
        if (trans->bankIndex == cmd->bankIndex)
            PROTECT_SUB(dmc_conf_state->dmc.ba_conf_cnt);
    ...
    if (!writeResp.empty()) {
        if (pre_wresp_time != now()) {
            if ((*mpfq_->WriteResp)(writeResp[0].channel, writeResp[0].task, 0, 0, 0)) {
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- Wresp Received :: task={}", now(), writeResp[0].task);
                }
                pre_wresp_time = now();
                writeResp.erase(writeResp.begin());
                linked_ptcs_[ptcID(writeResp[0].channel)]->dresp_cnt--;
            } else {
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- Wresp Back Pressure :: task={}", now(), writeResp[0].task);
                }
            }
        }
    }
    ...
    if (!readResp.empty()) {
        if (pre_rresp_time != now()) {
            if ((*mpfq_->ReadResp)(readResp[0].channel, readResp[0].task, 0, 0, 0)) {
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- Rresp Received :: task={}", now(), readResp[0].task);
                }
                pre_rresp_time = now();
                readResp.erase(readResp.begin());
                linked_ptcs_[ptcID(readResp[0].channel)]->dresp_cnt--;
            } else {
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- Rresp Back Pressure :: task={}", now(), readResp[0].task);
                }
            }
        }
    }
    ...
    if (!cmdResp.empty()) {
        if (pre_cresp_time != now()) {
            ...
            if ((*mpfq_->CmdResp)(cmdResp[0].channel, cmdResp[0].task, 0, 0, 0)) {
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- Cresp Received :: task={}", now(), cmdResp[0].task);
                }
                pre_cresp_time = now();
                cmdResp.erase(cmdResp.begin());
            } else {
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- Cresp Back Pressure :: task={}", now(), cmdResp[0].task);
                }
            }
        }
    }
...
/***************************************************************************************************
Descriptor: update state: PRINT_SPD_PFQ & DEBUG
****************************************************************************************************/
void PFQ:: update_state()
{
    // // =========================================================================
    // // 宏观离散度、拥塞与历史平均分析 (Master Bank Entropy & Average Report) 
    // // =========================================================================
    // if (DEBUG_GBUF_STATE) {
    //     // --- 瞬时状态统计 ---
    //     std::set<unsigned> unique_banks_pending_r;
    //     std::set<unsigned> unique_banks_pending_w;
    //     std::set<unsigned> unique_banks_total_r;
    //     std::set<unsigned> unique_banks_total_w;
        
    //     unsigned cnt_pending_r = 0, cnt_pending_w = 0;
    //     unsigned cnt_total_r = 0, cnt_total_w = 0;
    ...
    //             cnt_total_r++;
    //             unique_banks_total_r.insert(t->bankIndex);
    //             if (!t->in_ptc) {
    //                 cnt_pending_r++;
    //                 unique_banks_pending_r.insert(t->bankIndex);
    //             }
    //         } else if (t->transactionType == DATA_WRITE) {
    //             cnt_total_w++;
    //             unique_banks_total_w.insert(t->bankIndex);
    //             if (!t->in_ptc) {
    //                 cnt_pending_w++;
    //                 unique_banks_pending_w.insert(t->bankIndex);
    //             }
    //         }
    //     }

    //     // --- 历史平均值累加器 (利用 static 保持函数调用间的状态) ---
    //     static unsigned long long sample_count = 0;
        
    //     static unsigned long long sum_cnt_total_w = 0, sum_cnt_pending_w = 0;
    //     static unsigned long long sum_banks_total_w = 0, sum_banks_pending_w = 0;
        
    //     static unsigned long long sum_cnt_total_r = 0, sum_cnt_pending_r = 0;
    //     static unsigned long long sum_banks_total_r = 0, sum_banks_pending_r = 0;

    //     sample_count++;
        
    //     // 写累加
    //     sum_cnt_total_w += cnt_total_w;
    //     sum_cnt_pending_w += cnt_pending_w;
    //     sum_banks_total_w += unique_banks_total_w.size();
    //     sum_banks_pending_w += unique_banks_pending_w.size();
        
    //     // 读累加
    //     sum_cnt_total_r += cnt_total_r;
    //     sum_cnt_pending_r += cnt_pending_r;
    //     sum_banks_total_r += unique_banks_total_r.size();
    //     sum_banks_pending_r += unique_banks_pending_r.size();

    //     // 写平均
    //     double avg_cnt_total_w = (double)sum_cnt_total_w / sample_count;
    //     double avg_cnt_pending_w = (double)sum_cnt_pending_w / sample_count;
    //     double avg_banks_total_w = (double)sum_banks_total_w / sample_count;
    //     double avg_banks_pending_w = (double)sum_banks_pending_w / sample_count;
        
    //     // 读平均
    //     double avg_cnt_total_r = (double)sum_cnt_total_r / sample_count;
    //     double avg_cnt_pending_r = (double)sum_cnt_pending_r / sample_count;
    //     double avg_banks_total_r = (double)sum_banks_total_r / sample_count;
    //     double avg_banks_pending_r = (double)sum_banks_pending_r / sample_count;

    //     // --- 打印输出 ---
    //     PRINT_SPD_PFQ(ch, "==================================================================================================");
    //     PRINT_SPD_PFQ(ch, " MASTER BANK ENTROPY & QUEUING REPORT (Samples: {}) ", sample_count);
        
    //     PRINT_SPD_PFQ(ch, "  [READ  INSTANT] Total in Queue: {:>2} (Unique Banks: {:>2}) | Pending Only: {:>2} (Unique Banks: {:>2})", 
    //                   cnt_total_r, unique_banks_total_r.size(), cnt_pending_r, unique_banks_pending_r.size());
                      
    //     PRINT_SPD_PFQ(ch, "  [WRITE INSTANT] Total in Queue: {:>2} (Unique Banks: {:>2}) | Pending Only: {:>2} (Unique Banks: {:>2})", 
    //                   cnt_total_w, unique_banks_total_w.size(), cnt_pending_w, unique_banks_pending_w.size());
                      
    //     PRINT_SPD_PFQ(ch, "  ----------------------------------------------------------------------------------------------");
        
    //     PRINT_SPD_PFQ(ch, "  [READ  AVERAGE] Total in Queue: {:>5.1f} (Unique Banks: {:>5.1f}) | Pending Only: {:>5.1f} (Unique Banks: {:>5.1f})", 
    //                   avg_cnt_total_r, avg_banks_total_r, avg_cnt_pending_r, avg_banks_pending_r);
                      
    //     PRINT_SPD_PFQ(ch, "  [WRITE AVERAGE] Total in Queue: {:>5.1f} (Unique Banks: {:>5.1f}) | Pending Only: {:>5.1f} (Unique Banks: {:>5.1f})", 
    //                   avg_cnt_total_w, avg_banks_total_w, avg_cnt_pending_w, avg_banks_pending_w);

    //     // 自动诊断逻辑 (基于平均值判断，比瞬时值更准)
    //     if (sample_count > 1000) { // 跑过一段预热期后再开始诊断
    //         if (avg_cnt_total_w >= 40.0 && avg_banks_total_w < 34.0) {
    //             PRINT_SPD_PFQ(ch, "  !!! WARNING: Average Bank Entropy for Writes is severely low (<34)! Check Address Mapping Matrix !!!");
    //         }
    //         if (avg_cnt_total_w > 0 && (avg_cnt_total_w - avg_cnt_pending_w) > avg_banks_total_w / 2.0) {
    //             PRINT_SPD_PFQ(ch, "  !!! NOTICE: Physical Layer (PTC) is chronically congested on average.");
    //         }
    //     }
    ...
    if (!DEBUG_GBUF_STATE)
        return;
    PRINT_SPD_PFQ(ch, "--------------------------------------------------------------------------------------------------")
    PRINT_SPD_PFQ(ch, "Perf Total Status: R:{} W:{} wbuff_state{} occ={} state_trig={}",
        rcmd_cnt,
        wcmd_cnt,
        wbuff_state,
        linked_ptcs_[0]->occ,
    ...
    for (auto &t : PerfQue) {
        ...
        auto rd = r->dmc;
        PRINT_SPD_PFQ(ch, 
            "Rbuf time: {} | task={} | mask={} | bank={} | rank={} | row={} | address={:x} | len={} | data_size={} | "
            "data_ready_cnt={} | timeout={} | qos={} | pri={} rd_byp={} | rhit={} | g_bg_cc={} | g_ba_cc={} | "
            "g_ad_cc={} | d_bg_cc={} | d_ba_cc={} | d_ad_cc={} | r_bank_cnt={} | w_bank_cnt={} | in_ptc={}",
            now(),
            t->task,
            t->mask_wcmd,
            t->bankIndex,
            ...
            rd.ba_conf_cnt,
            rd.ad_conf_cnt,
            linked_ptcs_[0]->r_bank_cnt[t->bankIndex],
            linked_ptcs_[0]->w_bank_cnt[t->bankIndex],
            t->in_ptc);
    }
    PRINT_SPD_PFQ(ch, "--------------------------------------------------------------------------------------------------")
    for (auto &t : PerfQue) {
        ...
        auto wd = w->dmc;
        PRINT_SPD_PFQ(ch, 
            "Wbuf time: {} | task={} | mask={} | bank={} | rank={} | row={} | address={:x} | len={} | data_size={} | "
            "data_ready_cnt={} | timeout={} | qos={} | pri={} rd_byp={} | rhit={} | g_bg_cc={} | g_ba_cc={} | "
            "g_ad_cc={} | d_bg_cc={} | d_ba_cc={} | d_ad_cc={} | r_bank_cnt={} | w_bank_cnt={} | in_ptc{}",
            now(),
            t->task,
    ...
    PRINT_SPD_PFQ(ch, "--------------------------------------------------------------------------------------------------")
    PRINT_SPD_PFQ(ch, "Perf Total Status(in Ptc): RR:{} RW:{}", rel_rcmd_cnt, rel_wcmd_cnt);
    for (auto &t : PerfQue) {
        if (t == nullptr)
            continue;
        if (t->transactionType == DATA_WRITE)
...
void PFQ:: update_state_trig()
{
    ...
    if (wbuff_state == WBUFF_IDLE) {
        ...
        if (state_trig == 0 && (rel_wcmd_cnt + wcmd_cnt) > 0 && (rel_rcmd_cnt + rcmd_cnt) == 0) {
            if (idle_trig_time == IDLE_TRIG_TIME)
                state_trig = 1;  // idle trig
            else
                idle_trig_time++;
        } else {
            idle_trig_time = 0;
    ...
    if (wbuff_state_gap > 0)
        wbuff_state_gap--;

    if (DEBUG_BUS) {
        PRINT_SPD_PFQ(ch, "{:10} -- State Trig Update: update_state_trig={}, availability={}, update_wcmd_cnt={}, update_rcmd_cnt={}, "
                      "dmc_rd_cnt={}, rel_wcmd_cnt={}, rel_rcmd_cnt={}, idle_trig_time={}, wbuff_state_gap={}",
            now(),
            state_trig,
            linked_ptcs_[0]->occ,
            wcmd_cnt,
...
unsigned PFQ:: check_wr_level()
{
    unsigned rd_cnt = 0;
    unsigned wr_cnt = 0;
    if (TOTAL_RCMD_MODE == 0)
        // rd_cnt = rel_rcmd_cnt + rcmd_cnt;
        rd_cnt =  rcmd_cnt;
    else if (TOTAL_RCMD_MODE == 1)
        rd_cnt = rcmd_cnt;
    else {
...
void PFQ:: perf_check_timeout_and_aging()
{
    ...
    if (PERF_PRI_ADAPT_ENABLE) {
        for (auto &trans : PerfQue) {
            ...
            if (now() < trans->arb_time)
                continue;
            if (trans->pri_adapt_th == 0 || trans->qos == QOS_MAX)
                continue;
            if (now() - trans->timeAdded >= ((trans->improve_cnt + 1) * trans->pri_adapt_th)) {
                ...
                if (trans->pri < PERF_ADAPT_PRI_MAX) {  // adapt low qos to pri_max && noadapt for hqos
                    trans->pri++;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PFQ(ch, "{:10} -- PERF PRI_ADAPT :: task={} pri={} qos={}",
                            now(),
                            trans->task,
                            trans->pri,
                            trans->qos);
    ...
    for (auto &trans : PerfQue) {
        ...
        if (!trans->timeout && ((now() - trans->timeAdded >= trans->timeout_th && trans->timeout_th != 0))) {
            if (trans->qos == QOS_MAX || (PERF_QOS_POLICY == 1) ||
                (PERF_QOS_POLICY == 2 && trans->pri >= highest_pri)) {
                trans->timeout      = true;
                trans->timeout_type = 0;
                trans->time_timeout = now();
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, 
                        "{:10} -- PERF TIMEOUT :: task={} type={} qos={} pri={} cmd_rt_type={} cmd_hqos_type={}",
                        now(),
                        trans->task,
                        trans->transactionType,
                        trans->qos,
        ...
        if (trans->timeout && !has_rd_tout && trans->transactionType == DATA_READ)
            has_rd_tout = true;
        if (trans->timeout && !has_wr_tout && trans->transactionType == DATA_WRITE)
            has_wr_tout = true;
    }
    ...
    if (PERF_DUMMY_TOUT_EN) {
        for (auto &trans : PerfQue) {
            ...
            if (trans->dummy_ever)
                continue;
            if (rowhit)
                continue;  // rowhit -> not dummy
            if (bankConfPolicyPass(trans) && cur_bank_state != CurrentBankState::RowActive)
                continue;  // rowmiss && bank credit enough && not RowActive -> not dummy
            trans->dummy_timeout = true;
            if (trans->transactionType == DATA_READ)
                has_rd_dummy_tout = true;
            else
                has_wr_dummy_tout = true;
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- PERF DUMMY TIMEOUT :: task={} type={} qos={} pri={} rank={} bank={}",
                    now(),
                    trans->task,
                    trans->transactionType,
                    trans->qos,
...
void PFQ:: sch_subque()
{
    ...
    for (size_t subq_idx = 0; subq_idx < PERF_SUBQ_NUM; subq_idx++) {
        ...
        for (size_t i = start_idx; i <= end_idx; i++) {
            ...
            if (!schedule_read && trans->transactionType == DATA_READ && !trans->timeout) {
                forbidden_cnt["read_disabled"]++;
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- PERF rcmd blocked by WRITE_GROUP :: task={}", now(), trans->task);
                }
                continue;
            }
            if (!schedule_write && trans->transactionType == DATA_WRITE && !trans->timeout) {
                forbidden_cnt["write_disabled"]++;
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- PERF wcmd blocked by READ_GROUP :: task={}", now(), trans->task);
                }
                continue;
            }
            // check for high qos label
            if (trans->hqos_timeout && trans->dummy_hqos && trans->transactionType == DATA_READ) {
                ERROR(setw(10) << now() << " Hqos Timeout and Dummy Not Coexist, task=" << trans->task
                               << " hqos_timeout=" << trans->hqos_timeout << " dummy_hqos=" << trans->dummy_hqos
                               << " dummy_hqos_ever=" << trans->dummy_hqos_ever);
                assert(0);
            }

            if ((trans->hqos_timeout || trans->dummy_hqos || trans->dummy_hqos_ever) &&
                trans->transactionType == DATA_WRITE) {
                ERROR(setw(10) << now() << " High Qos Not for Wcmd, task=" << trans->task
                               << " hqos_timeout=" << trans->hqos_timeout << " dummy_hqos=" << trans->dummy_hqos
                               << " dummy_hqos_ever=" << trans->dummy_hqos_ever);
                assert(0);
            }
            ...
            if (now() < trans->arb_time) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd blocked by arb_time :: task={} TimeAdded={} ArbTime={}",
                        now(),
                        trans->task,
                        trans->timeAdded,
                        trans->arb_time);
            ...
            if (trans->transactionType == DATA_WRITE && trans->data_ready_cnt < (trans->burst_length + 1)) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd blocked by data_ready :: task={} DataReadyCnt={} NeedSize={}",
                        now(),
                        trans->task,
                        trans->data_ready_cnt,
            ...
            if ((conf->dmc.ad_conf_cnt + conf->perf.ad_conf_cnt) > 0) {
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd blocked by addr_conf :: task={} Addr={} perf_ad_cc={} -- "
                                  "r_dmc_ad_cc larger than 0",
                        now(),
                        trans->task,
                        trans->address,
                        conf->perf.ad_conf_cnt);
                }
                if (conf->dmc.ad_conf_cnt > 0) {
                    // ERROR(setw(10), now(), trans->task);
                    assert(0);
                }
                continue;
            }
            if (trans->fast_rd) {
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd blocked by fast_rd :: task={} fast_rd={} act_only={}",
                        now(),
                        trans->task,
                        trans->fast_rd,
                        trans->act_only);
            ...
            if (trans->bp_by_tout) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd blocked by timeout, task={}, qos={}, timeAdded={}",
                        now(),
                        trans->task,
                        trans->qos,
                        trans->timeAdded);
            ...
            if (linked_ptcs_[ptcID(trans->channel)]->get_refresh_state(trans->bankIndex)) {  // rename
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd blocked by refresh, task={}, qos={}, rank={}, group={} ,bank={}, "
                                  "row={}, ch={}, pbrw={}, pbr={}, pbr_pre={}, abrW={}, abr={}",
                        now(),
                        trans->task,
                        trans->qos,
                        trans->rank,
                        ...
                        trans->row,
                        trans->channel,
                        linked_ptcs_[ptcID(trans->channel)]->refreshPerBank[bankIndex].refreshWaiting,
                        linked_ptcs_[ptcID(trans->channel)]->refreshPerBank[bankIndex].refreshing,
                        linked_ptcs_[ptcID(trans->channel)]->refreshPerBank[bankIndex].refreshWaitingPre,
                        linked_ptcs_[ptcID(trans->channel)]->refreshALL[rank][sub_channel].refreshWaiting,
                        linked_ptcs_[ptcID(trans->channel)]->refreshALL[rank][sub_channel].refreshing);
                }
                continue;
            }
            ...
            if (PERF_RANK_PRI_EN) {
                pri_info.m_rank_pri = getRankGroupPri(trans);
                pri_info.m_bg_pri   = getBankGroupPri(trans);
            }
            if (PERF_STATE_PRI_EN) {
                pri_info.m_state_pri = trans->state_pri;
                
            }
            pri_info.m_ori_pri   = trans->pri;
            if (PERF_RHIT_PRI_EN)
                pri_info.m_rowhit_pri = realRowhit(trans);

            if (cur_cmd == nullptr) {
                cur_cmd = cmd;
                lastselcmd_idx = i;
            } else if (cmd->pri > cur_cmd->pri) {
                cur_cmd = cmd;
                lastselcmd_idx = i;
            } else if (cmd->pri == cur_cmd->pri) {
                if (limitSameSidRead(trans, PerfQue[lastselcmd_idx]) ||
                    (!limitSameSidRead(PerfQue[lastselcmd_idx], trans) &&
                     (preferSameSidRead(trans, PerfQue[lastselcmd_idx]) ||
                      (!preferSameSidRead(PerfQue[lastselcmd_idx], trans) && cmd->timeAdded < cur_cmd->timeAdded)))) {
                    cur_cmd = cmd;
                    lastselcmd_idx = i;
                }
        ...
        if (ArbCmd[subq_idx] != nullptr) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd enter ArbCmd Lv1 arb, task={}, subq={}, qos={}, pri={}, type={}, "
                              "timeAdded={}, rank={}, group={}, bank={}, bankIndex={} Pri_info={},{},{},{},{},{},{}",
                    now(),
                    ArbCmd[subq_idx]->task,
                    subq_idx,
                    ArbCmd[subq_idx]->qos,
                    ArbCmd[subq_idx]->pri,
                    ArbCmd[subq_idx]->type,
                    ArbCmd[subq_idx]->timeAdded,
                    ArbCmd[subq_idx]->rank,
                    ArbCmd[subq_idx]->group,
                    ArbCmd[subq_idx]->bank,
                    ArbCmd[subq_idx]->bankIndex,
                    ArbCmd[subq_idx]->pri_info.m_bg_pri,
                    ArbCmd[subq_idx]->pri_info.m_bg_num,
                    ArbCmd[subq_idx]->pri_info.m_bank_num,
                    ArbCmd[subq_idx]->pri_info.m_ori_pri,
                    ArbCmd[subq_idx]->pri_info.m_rowhit_pri,
                    ArbCmd[subq_idx]->pri_info.m_rank_pri,
                    ArbCmd[subq_idx]->pri_info.m_state_pri);
            }
        }
    }
}
...
bool PFQ:: rankGroupPass(Transaction *trans)
{
    ...
    if (trans->timeout)
        return true;
    if (trans->timeout && trans->dummy_timeout)
        return true;
    ...
    if (PTC_MAX_RANK > 0) {
        ...
        if (ptc_exist_rank.size() > PTC_MAX_RANK) {
            ...
            if (ptc_exist_rank.count(trans->rank) == 0) {
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, 
                        "{:10} -- PERF cmd blocked by rankpolicy1, task={}, qos={}, rank={}, group={} ,bank={}, row={}",
                        now(),
                        trans->task,
                        trans->qos,
                        trans->rank,
    ...
    if (PTC_MAX_CNT_PER_RANK > 0) {
        unsigned rank_cnt_rw = linked_ptcs_[ptcID(trans->channel)]->get_rank_cnt(trans->rank, 0) +
                               linked_ptcs_[ptcID(trans->channel)]->get_rank_cnt(trans->rank, 1);
        if ((rank_cnt_rw - perf_timeout_rank_cnt) > PTC_MAX_CNT_PER_RANK) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd blocked by rankpolicy2, task={}, qos={}, rank={}, group={} ,bank={}, "
                              "row={}, ptc_rank_cmd={}, max_per_rank={}",
                    now(),
                    trans->task,
                    trans->qos,
                    trans->rank,
    ...
    if (PERF_RANK_WMERGE_EN || PERF_RANK_RMERGE_EN) {
        if (((trans->rank != perf_sch_wrank && PERF_RANK_WMERGE_EN && wbuff_state == WBUFF_WRITE) ||
                (trans->rank != perf_sch_rrank && PERF_RANK_RMERGE_EN && wbuff_state == WBUFF_IDLE)) &&
            (wbuff_rnkgrp_state == WBUFF_IN_RNK_GROUP)) {
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- PERF rcmd blocked by rankpolicy3, task={}, qos={}, rank={}, group={} ,bank={}, "
                              "row={}, rw_state={}, rnk_state={}, rd_rnkgrp={}, wr_rnkgrp={}",
                    now(),
                    trans->task,
                    trans->qos,
                    trans->rank,
...
bool PFQ:: bankStatePolicyPass(Transaction *trans)
{
    ...
    if (!((trans->timeout && trans->dummy_timeout) || trans->dummy_hqos)) {  // non-dummy_timeout/non-dummy_hqos
        ...
        if (cur_bank_state == CurrentBankState::Refreshing || cur_bank_state == CurrentBankState::Idle ||
            cur_bank_state == CurrentBankState::RowActive) {
            state_timing_met = true;
        }
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, "{:10} -- Dummy Timeout/Hqos Pass act_timing, task={}, qos={}, rank={}, group={} ,bank={}, "
                          "row={}, bank_state={}, state_met={}, act_met={} dummy_timeout={} dummy_hqos={}",
                now(),
                trans->task,
                trans->qos,
                trans->rank,
                ...
                trans->bankIndex,
                trans->row,
                linked_ptcs_[ptcID(trans->channel)]->bank_state_opcode(cur_bank_state),
                state_timing_met,
                act_timing_met,
                trans->dummy_timeout,
                trans->dummy_hqos);
    ...
    if (DEBUG_BUS && !state_timing_met) {
        PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd blocked by act_timing, task={}, qos={}, rank={}, group={} ,bank={}, row={}, "
                      "bank_state={}, state_met={}, act_met={}",
            now(),
            trans->task,
            trans->qos,
            trans->rank,
...
bool PFQ:: bankConfPolicyPass(Transaction *trans)
{
    ...
    if (row_hit) {
        if (trans->timeout)
            return true;  // perf timeout && rowhit exclude
        if (trans->hqos_timeout)
            return true;  // perf hqos && rowhit exclude
        if (trans->transactionType == DATA_READ) {
    ...
    if (same_bank_cnt < max_same_bank_cnt) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd blocked by bankpolicy, task={}, qos={}, rank={}, group={} ,bank={}, "
                          "row={}, same_bank_cnt={}, max_same_bank_cnt={}",
                now(),
                trans->task,
                trans->qos,
                trans->rank,
...
bool PFQ:: ptcRowhitBreakPolicyPass(Transaction *trans)
{
    ...
    if (!rhit_break_bankbp) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd blocked by rhitbreak policy, task={}, qos={}, rank={}, group={} ,bank={}, "
                          "row={}, openrow={}",
                now(),
                trans->task,
                trans->qos,
                ...
                trans->bankIndex,
                trans->row,
                bank_state.state->openRowAddress);
        }
        return false;
    }
}
...
bool PFQ:: ptcDummyToutPolicyPass(Transaction *trans)
{
    ...
    if (!dummy_tout_bankbp) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd blocked by dummy_tout policy, task={}, qos={}, rank={}, group={} "
                          ",bank={}, row={}, openrow={}",
                now(),
                trans->task,
                trans->qos,
...
bool PFQ:: bankGroupPolicyPass(Transaction *trans)
{
    ...
    if (bg_cnt >= max_bg_cnt) {
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd blocked by bgpolicy, task={}, qos={}, rank={}, group={} ,bank={}, row={}, "
                          "bg_cnt={}, max_bg={}",
                now(),
                trans->task,
                trans->qos,
...
void PFQ:: arb_node()
{
    ...
    if (DEBUG_BUS) {
        ...
        PRINT_SPD_PFQ(ch, "{:10} -- PTC CMD :: {}", now(), ptc_cmd_str);
    }
    if (wbuff_state == WBUFF_WRITE) {
        if (!ArbCmd_notempty())
            no_cmd_sch_cnt++;
    ...
    for (unsigned i = 0; i < (NUM_CHANS / NUM_PFQS); i++) {
        if (!linked_ptcs_[i]->fastrd_fifo.empty() && PERF_CLK_MODE == 0)
            return;  // deal with fastrd & pfq arb cmd in same cycle
    }
    ...
    if (DEBUG_BUS) {
        PRINT_SPD_PFQ(ch, "{:10} -- vld cmd cnt={}, first_vld_idx={}", now(), vld_cmd_cnt, first_vld_idx);
    }
    ...
    if (selected_cmd != nullptr) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, "{:10} -- PERF cmd selected Lv2 arb, task={}, qos={}, type={}, timeAdded={}, rank={}, "
                          "group={}, bank={}, bankIndex={} Pri_info={},{},{},{},{},{},{}",
                now(),
                selected_cmd->task,
                selected_cmd->qos,
                selected_cmd->type,
                selected_cmd->timeAdded,
                selected_cmd->rank,
                selected_cmd->group,
                selected_cmd->bank,
                selected_cmd->bankIndex,
                selected_cmd->pri_info.m_bg_pri,
                selected_cmd->pri_info.m_bg_num,
                selected_cmd->pri_info.m_bank_num,
                selected_cmd->pri_info.m_ori_pri,
                selected_cmd->pri_info.m_rowhit_pri,
                selected_cmd->pri_info.m_rank_pri,
                selected_cmd->pri_info.m_state_pri);
        }
    } else {
        return;
    }
    ...
    for (auto &cmd : PerfQue) {
        if (cmd == nullptr)
            continue;
        if (selected_cmd->type == DATA_READ && cmd->transactionType == DATA_WRITE)
            continue;
        if (selected_cmd->type == DATA_WRITE && cmd->transactionType == DATA_READ)
            continue;
        if (cmd->task != selected_cmd->task)
            continue;
        setRowhitMiss(cmd);
        ...
        if (!linked_ptcs_[ptcID(ptc_trans->channel)]->addTransaction(ptc_trans, false)) {
            ...
            if (selected_cmd->type == DATA_READ) {
                if (rd_sch_rhit_cnt >= 1)
                    rd_sch_rhit_cnt = 0;
                else if (cmd->conflict_state->rowhit && LastArbCmd.bankIndex == cmd->bankIndex)
                    rd_sch_rhit_cnt++;  // rd_sch_rhit_cnt useless?
            }
            if ((selected_cmd->type == DATA_READ && CMD_ROW_ORDER == 2) ||
                (selected_cmd->type == DATA_WRITE && RO_HIT_EN)) {
                for (auto &other_cmd : PerfQue) {
                    if (other_cmd == nullptr)
                        continue;
                    if (other_cmd->transactionType == DATA_WRITE && selected_cmd->type == DATA_READ)
                        continue;
                    if (other_cmd->transactionType == DATA_READ && selected_cmd->type == DATA_WRITE)
                        continue;
                    if (other_cmd->conflict_state->rowhit)
                        continue;
                    if (other_cmd->task == cmd->task)
                        continue;
                    if (other_cmd->bankIndex != cmd->bankIndex)
                        continue;
                    if (other_cmd->row != cmd->row)
                        continue;
            ...
            if (selected_cmd->type == DATA_READ) {
                ser_read_cnt++;
                if (IS_HBM3 && SIMPLE_GRP_SID_EN && selected_cmd->rank < sid_group_serial_count.size()) {
                    unsigned rank = selected_cmd->rank;
                    if (sid_group_target[rank] == selected_cmd->sid)
                        sid_group_serial_count[rank]++;
                    else
                        sid_group_serial_count[rank] = 0;
                    if (selected_cmd->sid < NUM_SIDS)
                        sid_group_timeout[rank][selected_cmd->sid] = 0;
                }
                rcmd_cnt--;
                rel_rcmd_cnt++;
                ...
                if (cmd->timeout)
                    rd_timeout_cnt++;
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- PERF_SCH :: READ (R:{}|W:{}|RR:{}|RW:{}|QR:{}|QW:{}) task={} addr={:x} "
                                  "rank={} group={} bank={} row={} col={} addr_col={} data_size={} qos={} pri={} "
                                  "blen={} rhit={} rbc={} wbc={} timeout={} perf_rd_rowhit={} perf_rd_rowmiss={} "
                                  "perf_wr_rowhit={} perf_wr_rowmiss={} perf_maxbg_hit={} perf_maxbank_hit={}",
                        now(),
                        rcmd_cnt,
                        wcmd_cnt,
                        rel_rcmd_cnt,
                        rel_wcmd_cnt,
                        linked_ptcs_[ptcID(cmd->channel)]->Read_Cnt(),
                        linked_ptcs_[ptcID(cmd->channel)]->Write_Cnt(),
                        cmd->task,
                        cmd->address,
                        cmd->rank,
                        cmd->group,
                        ...
                        cmd->pri,
                        cmd->burst_length,
                        cmd->conflict_state->rowhit,
                        linked_ptcs_[ptcID(cmd->channel)]->r_bank_cnt[cmd->bankIndex],
                        linked_ptcs_[ptcID(cmd->channel)]->w_bank_cnt[cmd->bankIndex],
                        cmd->timeout,
                        cmd->perf_rd_rowhit,
                        cmd->perf_rd_rowmiss,
                        cmd->perf_wr_rowhit,
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, 
                        "{:10} -- PERF_SCH :: WRITE (R:{}|W:{}|RR:{}|RW:{}|QR:{}|QW:{}) task={} addr={:x} rank={} "
                        "group={} bank={} row={} col={} addr_col={} data_size={} qos={} pri={} blen={} "
                        "data_ready_cnt={} rhit={} rbc={} wbc={} timeout={} perf_rd_rowhit={} perf_rd_rowmiss={} "
                        "perf_wr_rowhit={} perf_wr_rowmiss={} perf_maxbg_hit={} perf_maxbank_hit={}",
                        now(),
                        rcmd_cnt,
                        wcmd_cnt,
    ...
    for (auto &trans : PerfQue) {
        ...
        if ((now() - trans->timeAdded) > 100000) {
            ERROR(setw(10) << now() << " -- PERF task=" << trans->task << " mask=" << trans->mask_wcmd << " address="
                           << hex << trans->address << dec << " rank=" << trans->rank << " bank=" << trans->bankIndex
                           << " row=" << trans->row << " matgrp=" << (trans->row & (NUM_MATGRPS - 1)));
            ERROR(setw(10) << now() << " -- error, qos=" << trans->qos << ", pri=" << trans->pri);
            ERROR(setw(10) << now() << " -- FATAL ERROR == big latency"
                           << ", chnl:" << channel);
            assert(0);
        }
        if (now() - trans->timeAdded > 100000 && trans->transactionType == DATA_WRITE &&
...
string PFQ:: getPTCCmdSituation()
{
    ...
    for (unsigned rank_id = 0; rank_id < NUM_RANKS; rank_id++) {
        ...
        for (unsigned bg_id = 0; bg_id < NUM_GROUPS; bg_id++) {
            ret << linked_ptcs_[0]->get_bg_cnt(rank_id, bg_id, true) +
                       linked_ptcs_[0]->get_bg_cnt(rank_id, bg_id, false)
                << " ";
        }
    }
    return ret.str();
...
void PFQ:: setRowhitMiss(Transaction *trans)
{
    ...
    for (auto &t : PerfQue) {
        if (t == nullptr)
            continue;
        if (trans->transactionType != t->transactionType)
            continue;
        if (t->in_ptc)
            continue;
        if (t->fast_rd)
...
if (!exist_task) {
        // 1. 先打印当前正在寻找的 trans 的信息
        ERROR(setw(10) << now() << " Target trans not found! trans->task=" << trans->task 
              << " in_ptc=" << trans->in_ptc 
              << " fast_rd=" << trans->fast_rd 
              << " perf_addrconf=" << trans->perf_addrconf)
        
        // 2. 遍历打印当前 PerfQue 中所有非空命令的信息
        ERROR(setw(10) << now() << " === Dumping current PerfQue contents ===");
        for (auto &t : PerfQue) {
            if (t != nullptr) {
                ERROR("  -> Queued task=" << t->task 
...
void PFQ:: stateTransition()
{
    ...
    bool queue_balance_w2r = W2R_BALANCE_EN && has_read_cmd && total_read_pressure && ptc_write_not_far_ahead
    ...
    switch (wbuff_state) {
        case WBUFF_NO_GROUP: {
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, 
                    "{:10} -- PERF_ST STATE, WB[state_wcmd_cnt={} state_rcmd_cnt={} rel_wcmd_cnt={} rel_rcmd_cnt={}] DMC[R:{} "
                    "W:{}] availability={} state_state_trig={} ser_sch_write={} no_cmd_sch_th={} wr_tout_cnt={} ser_read={} "
        ...
        case WBUFF_IDLE: {
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, 
                    "{:10} -- PERF_ST STATE, WB[state_wcmd_cnt={} state_rcmd_cnt={} rel_wcmd_cnt={} rel_rcmd_cnt={}] DMC[R:{} "
                    "W:{}] availability={} state_state_trig={} ser_sch_write={} no_cmd_sch_th={} wr_tout_cnt={} ser_read={} "
                    "has_read_cmd={} has_write_cmd={} high_qos_trig={} ser_write={} ptc_emp_cnt={}",
                    now(),
                    wcmd_cnt,
                    rcmd_cnt,
                    rel_wcmd_cnt,
        ...
        case WBUFF_WRITE: {
            ...
            if (GetPerfQsize() < PERF_EXGRP_LEVEL) {
                ...
                wbuff_state     = WBUFF_IDLE;
                        ...
                        ||(!has_write_cmd) || (ptc_empty_cnt > PTC_EMPTY_WR_TH) || high_qos_trig || queue_balance_w2r) && has_read_cmd) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, 
                        ...
                        "wr_tout_cnt={} has_read_cmd={} has_write_cmd={} high_qos_trig={} queue_balance_w2r={} ptc_emp_cnt={}",
                        now(),
                        wcmd_cnt,
                        rcmd_cnt,
                        rel_wcmd_cnt,
...
void PFQ:: update_rnkgroup_state()
{
    ...
    for (auto &trans : PerfQue) {
        ...
        unsigned sub_channel         = (trans->bankIndex % NUM_BANKS) / (linked_ptcs_[ptcID(trans->channel)]->sc_bank_num);
        if (!is_cur_rank_read && !is_cur_rank_write && !is_other_rank_read && !is_other_rank_write)
            continue;
        if (trans->perf_addrconf)
            continue;
        if (now() < trans->arb_time)
        ...
        if (has_rank_rcmd && has_rank_wcmd && has_other_rank_rcmd && has_other_rank_wcmd)
            break;
    }
    ...
    switch (wbuff_rnkgrp_state) {
        case WBUFF_NO_RNK_GROUP: {
            ...
            if (que_read_cnt >= PERF_RRNK_ENGRP_LEVEL && PERF_RANK_RMERGE_EN && wbuff_state == WBUFF_IDLE) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- R_RNKGRP_ENTER :: que_read_cnt={}, rnk_state={} perf_sch_rrank={}",
                        now(),
                        que_read_cnt,
                        wbuff_rnkgrp_state,
                        perf_sch_rrank);
            ...
            if (que_write_cnt >= PERF_WRNK_ENGRP_LEVEL && PERF_RANK_WMERGE_EN && wbuff_state == WBUFF_WRITE) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- W_RNKGRP_ENTER :: que_write_cnt={}, rnk_state={} perf_sch_wrank={}",
                        now(),
                        que_write_cnt,
                        wbuff_rnkgrp_state,
                        perf_sch_wrank);
            ...
            switch (wbuff_state) {
                case WBUFF_IDLE: {
                    if (!perf_rd_in_rnkgrp) {
                        ...
                        if (DEBUG_BUS) {
                            PRINT_SPD_PFQ(ch, 
                                "{:10} -- R_RNKGRP_EXIT0 :: que_read_cnt={}, wbuff_state={}, wbuff_state_pre={}",
                                now(),
                                que_read_cnt,
                                wbuff_state,
                                wbuff_state_pre);
                    ...
                    if (wbuff_state_pre == WBUFF_NO_GROUP || wbuff_state_pre == WBUFF_WRITE) {
                        if (((unsigned)accumulate(rank_cmd_high_qos.begin(), rank_cmd_high_qos.end(), 0) == 0) ||
                            ((unsigned)accumulate(rank_cmd_high_qos.begin(), rank_cmd_high_qos.end(), 0) ==
                                NUM_RANKS)) {  // no rank/all ranks meet hqos rank conditions
                            perf_sch_rrank = get_max_rank(true);
                        ...
                        if (DEBUG_BUS) {
                            PRINT_SPD_PFQ(ch, "{:10} -- R_RNKGRP_START :: perf_sch_rrank={} hqos_state={}",
                                now(),
                                perf_sch_rrank,
                                rank_cmd_high_qos[perf_sch_rrank]);
                        }
                        ...
                        if (que_read_cnt < PERF_RRNK_EXGRP_LEVEL) {
                            ...
                            if (DEBUG_BUS) {
                                PRINT_SPD_PFQ(ch, 
                                    "{:10} -- R_RNKGRP_EXIT1 :: que_read_cnt={}, wbuff_state={}, wbuff_state_pre={}",
                                    now(),
                                    que_read_cnt,
                                    wbuff_state,
                                    wbuff_state_pre);
                            ...
                            if (DEBUG_BUS) {
                                PRINT_SPD_PFQ(ch, 
                                    "{:10} -- R_RNKGRP_CHG_HQOS :: rank{} to rank{}", now(), perf_sch_rrank, nxt_rrank);
                            }
                            perf_sch_rrank = nxt_rrank;
                        } else if (!has_rank_rcmd && has_other_rank_rcmd) {
                            if (DEBUG_BUS) {
                                PRINT_SPD_PFQ(ch, 
                                    "{:10} -- R_RNKGRP_CHG_CMD :: rank{} to rank{}", now(), perf_sch_rrank, nxt_rrank);
                            }
                            perf_sch_rrank = nxt_rrank;
                        } else if ((no_rcmd_sch_cnt > PERF_READ_SCH_THH) &&
                                   rrel_rank_cnt[perf_sch_rrank] < PERF_SCH_RRANK_LEVELH && has_other_rank_rcmd &&
                                   rb_rank_cnt[nxt_rrank] > PERF_HOLD_RRANK_LEVELH) {
                            if (DEBUG_BUS) {
                                PRINT_SPD_PFQ(ch, 
                                    "{:10} -- R_RNKGRP_CHG_H :: rank{} to rank{}", now(), perf_sch_rrank, nxt_rrank);
                            }
                            perf_sch_rrank = nxt_rrank;
                        } else if ((no_rcmd_sch_cnt > PERF_READ_SCH_THL) &&
                                   rrel_rank_cnt[perf_sch_rrank] < PERF_SCH_RRANK_LEVELL && has_other_rank_rcmd &&
                                   rb_rank_cnt[nxt_rrank] > PERF_HOLD_RRANK_LEVELL) {
                            if (DEBUG_BUS) {
                                PRINT_SPD_PFQ(ch, 
                                    "{:10} -- R_RNKGRP_CHG_L :: rank{} to rank{}", now(), perf_sch_rrank, nxt_rrank);
                            }
                            perf_sch_rrank = nxt_rrank;
                        }
                    }
                ...
                case WBUFF_WRITE: {
                    if (!perf_wr_in_rnkgrp) {
                        ...
                        if (DEBUG_BUS) {
                            PRINT_SPD_PFQ(ch, 
                                "{:10} -- W_RNKGRP_EXIT0 :: que_write_cnt={}, wbuff_state={}, wbuff_state_pre={}",
                                now(),
                                que_write_cnt,
                                wbuff_state,
                                wbuff_state_pre);
                    ...
                    if (wbuff_state_pre == WBUFF_NO_GROUP || wbuff_state_pre == WBUFF_IDLE) {
                        perf_sch_wrank = get_max_rank(false);
                        if (DEBUG_BUS) {
                            PRINT_SPD_PFQ(ch, "{:10} -- W_RNKGRP_START :: perf_sch_wrank={}", now(), perf_sch_wrank);
                        }
                        ...
                        if (que_write_cnt < PERF_WRNK_EXGRP_LEVEL) {
                            ...
                            if (DEBUG_BUS) {
                                PRINT_SPD_PFQ(ch, 
                                    "{:10} -- W_RNKGRP_EXIT1 :: que_write_cnt={}, wbuff_state={}, wbuff_state_pre={}",
                                    now(),
                                    que_write_cnt,
                                    wbuff_state,
                                    wbuff_state_pre);
                            ...
                            if (DEBUG_BUS) {
                                PRINT_SPD_PFQ(ch, 
                                    "{:10} -- W_RNKGRP_CHG_CMD :: rank{} to rank{}", now(), perf_sch_wrank, nxt_wrank);
                            }
                            perf_sch_wrank = nxt_wrank;
                        } else if ((no_cmd_sch_cnt > PERF_WRITE_SCH_THH) &&
                                   wrel_rank_cnt[perf_sch_wrank] < PERF_SCH_WRANK_LEVELH && has_other_rank_wcmd &&
                                   wb_rank_cnt[nxt_wrank] > PERF_HOLD_WRANK_LEVELH) {
                            if (DEBUG_BUS) {
                                PRINT_SPD_PFQ(ch, 
                                    "{:10} -- W_RNKGRP_CHG_H :: rank{} to rank{}", now(), perf_sch_wrank, nxt_wrank);
                            }
                            perf_sch_wrank = nxt_wrank;
                        } else if ((no_cmd_sch_cnt > PERF_WRITE_SCH_THL) &&
                                   wrel_rank_cnt[perf_sch_wrank] < PERF_SCH_WRANK_LEVELL && has_other_rank_wcmd &&
                                   wb_rank_cnt[nxt_wrank] > PERF_HOLD_WRANK_LEVELL) {
                            if (DEBUG_BUS) {
                                PRINT_SPD_PFQ(ch, 
                                    "{:10} -- W_RNKGRP_CHG_L :: rank{} to rank{}", now(), perf_sch_wrank, nxt_wrank);
                            }
                            perf_sch_wrank = nxt_wrank;
                        }
                    }
...
void PFQ:: update_rpsram_state()
{
    ...
    if (rp_fifo.empty()) {
        ...
        for (unsigned i = 0; i < (NUM_CHANS / NUM_PFQS) && !exit; i++) {
            ...
            for (auto it = read_data_buffer.begin(); it != read_data_buffer.end();) {
                if (it->delay == 0) {
                    ...
                    if (DEBUG_BUS) {
                        PRINT_SPD_PFQ(ch, "{:10} -- T_SYSC :: Issuing to SYSC, task={}", now(), task);
                    }
                    ...
                    if (msg.burst_cnt == (msg.burst_length + 1))
                        rdata_type | = 1;  // bit[0] is rdata_end
                    if (msg.burst_cnt == 0)
                        rdata_type |                   = (1 << 1);               // bit[1] is rdata_start
                    ...
                    if (it->mask_wcmd && RMW_ENABLE_PERF && IS_LP6) {
                        if (rp_fifo.empty() && !rmw_rdata_fifo_full) {
                            ...
                            if (DEBUG_BUS) {
                                PRINT_SPD_PFQ(ch, "{:10} -- RMW_Data Bybass RPFIFO to RMWFIFO, task={} burst_cnt={}",
                                    now(),
                                    task,
                                    it->cnt);
                            }
                            ...
                            if (DEBUG_BUS) {
                                PRINT_SPD_PFQ(ch, 
                                    "{:10} -- RMW_Data To RPFIFO, task={} burst_cnt={}", now(), task, it->cnt);
                            }
                        }
                    } else {
                        if ((!IECC_ENABLE || !linked_ptcs_[ptcID(it->channel)]->tasks_info[task].rd_ecc) &&
                            (!RMW_ENABLE || (!it->mask_wcmd && RMW_ENABLE))) {
                            it->readDataEnterDmcTime = now() * tDFI;
                            sys_bp                   = !returnReadDataResp(it->channel,
                                task,
                                it->readDataEnterDmcTime,
                                msg.reqAddToDmcTime,
                                msg.reqEnterDmcBufTime);
                            if (sys_bp) {
                                if (DEBUG_BUS) {
                                    PRINT_SPD_PFQ(ch, 
                                        "{:10} -- Rdata Back Pressure :: task={} ch={}", now(), task, it->channel);
                                }
                                if (!RPFIFO_EN) {
                                    return;
                                } else {
                                    ...
                                    if (DEBUG_BUS) {
                                        PRINT_SPD_PFQ(ch, "{:10} -- Rdata to rp_fifo, task={}, rp_fifo_size={}",
                                            now(),
                                            task,
                                            rp_fifo.size());
                                    }
                                ...
                                if (PRINT_IDLE_LAT) {
                                    DEBUG(setw(10)
                                          << now() << " -- Rdata Received :: task=" << task << ", latency="
                                          << ceil(((it->readDataEnterDmcTime - msg.reqEnterDmcBufTime) / tDFI)));
                                }
                                if (DEBUG_BUS) {
                                    PRINT_SPD_PFQ(ch, "{:10} -- Rdata Received :: task={}, latency={}",
                                        now(),
                                        task,
                                        ceil(((it->readDataEnterDmcTime - msg.reqEnterDmcBufTime) / tDFI)));
                                }
                        ...
                        if (!sys_bp) {
                            if (it->is_last) {
                                linked_ptcs_[i]->ReturnData_statistics(
                                    task, msg.time, msg.qos, msg.mid, msg.pf_type, msg.rank);
                                // return latency
                                linked_ptcs_[i]->pending_TransactionQue.erase(task);
                                if (IECC_ENABLE)
                                    linked_ptcs_[ptcID(it->channel)]->tasks_info[task].rd_finish = true;
                                if (DEBUG_BUS) {
                                    double latency = ceil(((it->readDataEnterDmcTime / tDFI) - msg.reqEnterDmcBufTime));
                                    PRINT_SPD_PFQ(ch, 
                                        "{:10} -- READ_COMPLETE :: task={} latency={} cycles", now(), task, latency);
                                }
                            } else {
            ...
            for (unsigned i = 0; i < (NUM_CHANS / NUM_PFQS) && !exit; i++) {
                ...
                for (auto it = read_data_buffer.begin(); it != read_data_buffer.end();) {
                    if (it->delay == 0) {
                        ...
                        if (DEBUG_BUS) {
                            PRINT_SPD_PFQ(ch, "{:10} -- T_RDS :: Issuing to RDS, task={}", now(), it->task);
                        }
                        if (pending_it == linked_ptcs_[i]->pending_TransactionQue.end()) {
                            uint32_t ch = getID() * (NUM_CHANS / NUM_PFQS) + i;
                            ERROR(setw(10)
                                  << now() << " -- [DMC channel=" << ch << "] mismatch read data, task=" << it->task);
                            assert(0);
                        }
                        TRANS_MSG msg = pending_it->second;
                        msg.burst_cnt++;
                        ...
                        if (rp_fifo.size() < RPFIFO_AMFULL_TH) {
                            ...
                            rpfifo_write_this_cycle = true;
                            if (DEBUG_BUS) {
                                PRINT_SPD_PFQ(ch, "{:10} -- Rdata to rp_fifo, task={}, rpfifosize={}",
                                    now(),
                                    it->task,
                                    rp_fifo.size());
                            }
                            ...
                            rcmd_bp_byrp            = true;
                            if (DEBUG_BUS) {
                                PRINT_SPD_PFQ(ch, "{:10} -- Rcmd bp, Rdata to rp_fifo, task={}, rpfifosize={}",
                                    now(),
                                    it->task,
                                    rp_fifo.size());
                            }
        ...
        if (!rpfifo_write_this_cycle) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- T_MSYSC :: Issuing to MSYSC, from rp_fifo, task={}", now(), task);
            }
            if (pending_it == linked_ptcs_[ptcID(return_rdata.channel)]->pending_TransactionQue.end()) {
                ERROR(setw(10) << now() << " -- [DMC" << return_rdata.channel << "]"
                               << " mismatch data, task=" << task);
                assert(0);
            }
            ...
            if (return_rdata.mask_wcmd && RMW_ENABLE_PERF && IS_LP6) {
                if (!rmw_rdata_fifo_full) {
                    ...
                    rmw_bp_rdata_path = false;
                    if (DEBUG_BUS) {
                        PRINT_SPD_PFQ(ch, "{:10} -- RMW_DATA from RPFIFO TO RMWFIFO, task={} burst_cnt={}",
                            now(),
                            task,
                            return_rdata.cnt);
                    }
                    ...
                    if (DEBUG_BUS) {
                        PRINT_SPD_PFQ(ch, 
                            "{:10} -- RMW_DATA backpress RPFIFO, task={} burst_cnt={}", now(), task, return_rdata.cnt);
                    }
                }
            } else {
                if ((!IECC_ENABLE || !linked_ptcs_[ptcID(return_rdata.channel)]->tasks_info[task].rd_ecc) &&
                    (!RMW_ENABLE || (!return_rdata.mask_wcmd && RMW_ENABLE))) {
                    return_rdata.readDataEnterDmcTime = now() * tDFI;
                    sys_bp                            = !returnReadDataResp(return_rdata.channel,
                        task,
                    ...
                    if (sys_bp) {
                        if (DEBUG_BUS) {
                            PRINT_SPD_PFQ(ch, "{:10} -- Rdata from rpfifo Back Pressure :: task={} ch={}",
                                now(),
                                task,
                                return_rdata.channel);
                        }
                        return;
                    } else {
                        ...
                        if (PRINT_IDLE_LAT) {
                            DEBUG(setw(10)
                                  << now() << " -- Rdata Received from rpfifo :: task=" << task << ", latency="
                                  << ceil(((return_rdata.readDataEnterDmcTime - msg.reqEnterDmcBufTime) / tDFI)));
                        }
                        if (DEBUG_BUS) {
                            PRINT_SPD_PFQ(ch, "{:10} -- Rdata Received from rpfifo:: task={}, latency={}",
                                now(),
                                task,
                                ceil(((return_rdata.readDataEnterDmcTime - msg.reqEnterDmcBufTime) / tDFI)));
                        }
                ...
                if (!sys_bp) {
                    if (return_rdata.is_last) {
                        linked_ptcs_[ptcID(return_rdata.channel)]->ReturnData_statistics(
                            task, msg.time, msg.qos, msg.mid, msg.pf_type, msg.rank);
                        // return latency
                        linked_ptcs_[ptcID(return_rdata.channel)]->pending_TransactionQue.erase(task);
                        if (IECC_ENABLE)
                            linked_ptcs_[ptcID(return_rdata.channel)]->tasks_info[task].rd_finish = true;
                        if (DEBUG_BUS) {
                            double latency = 
                                ceil(((return_rdata.readDataEnterDmcTime / tDFI) - msg.reqEnterDmcBufTime));
                            PRINT_SPD_PFQ(ch, "{:10} -- READ_COMPLETE :: task={} latency={} cycles", now(), task, latency);
                        }
                    } else {
                        pending_it->second.burst_cnt = msg.burst_cnt;
                    }
                    ...
                    rp_fifo.pop_front();
                    if (DEBUG_BUS) {
                        PRINT_SPD_PFQ(ch, "{:10} -- Rdata from rp_fifo to upstream, task={}, rpfifosize={}",
                            now(),
                            task,
                            rp_fifo.size());
                    }
...
void PFQ:: process_rmw_data_flow()
{
    ...
    for (auto &rmw_data : rmw_rdata_fifo) {
        if (rmw_data.rmw_processing_started) {
            ...
            if (cycles_passed == CLKH2CLKL) {  // modify
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, 
                        "{:10} -- RMW_READ_STAGE1, task={} stage=1 (rmw read wsram in progress)", now(), rmw_data.task);
                }
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- RMW_READ_STAGE2, task={} stage=2 (data returned, merge and write)",
                        now(),
                        rmw_data.task);
                }
            }
    ...
    if (!rmw_rdata_fifo.empty()) {
        ...
        if (front_rmw.rmw_ready_for_wds) {
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- RMW_SEND_TO_WDS, task={} burst_cnt={}", now(), front_rmw.task, front_rmw.cnt);
            }
        ...
        for (auto &rmw_data : rmw_rdata_fifo) {
            if (!rmw_data.rmw_processing_started) {
                ...
                if (!wsram_busy_this_cycle) {
                    ...
                    if (DEBUG_BUS) {
                        PRINT_SPD_PFQ(ch, "{:10} -- RMW_READ_START, task={} stage=0 (initiate read) bp_next_cycle={}",
                            now(),
                            rmw_data.task,
                            rmw_write_bp_next_cycle);
                    }
                    break;
                }
            }
...
void PFQ:: wsram_release_wdata()
{
    for (unsigned slot_idx = 0; slot_idx < WSRAM_QUEUE_DEPTH; slot_idx++) {
        if (wsram_slt[slot_idx].state != READING)
            continue;
        if (now() >= wsram_slt[slot_idx].wdda) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- Wsram released slt, slt_idx={}, task={}, channel={}",
                    now(),
                    slot_idx,
                    wsram_slt[slot_idx].task,
                    wsram_slt[slot_idx].channel);
            }
...
void PFQ:: send_wdata()
{
    if (!WdataToSend.empty() && !Perf_WdataSram.empty()) {
        ...
        for (size_t i = 0; i < WdataToSend.size(); i++) {
            if (WdataToSend[i] != Perf_WdataSram[0].task)
                continue;
            linked_ptcs_[ptcID(Perf_WdataSram[0].channel)]->receive_wdata(0, task);
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- PERF_SEND_WDATA :: task={} data_cnt={}",
                    now(),
                    WdataToSend[i],
...
void PFQ:: perf_release_rd(uint64_t task, bool release_state)
{
    ...
    for (auto it = PerfQue.begin(); it != PerfQue.end(); it++) {
        ...
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, 
                "{:10} -- PERF RELEASE R :: task={} in_ptc={} fast_rd={}", now(), task, r->in_ptc, r->fast_rd);
        }
        exist_task = true;
        if (!r->fast_rd) {
            rel_rcmd_cnt--;
        ...
        if (r->mask_wcmd && RMW_ENABLE_PERF && IS_LP6) {
            ...
            if (DEBUG_BUS) {
                PRINT_SPD_PFQ(ch, "{:10} -- RMW_CMD R2W_conversion completed for task={}", now(), r->task);
            }
...
void PFQ:: perf_release_wr(uint64_t task, bool release_state)
{
    ...
    for (auto it = PerfQue.begin(); it != PerfQue.end(); it++) {
        ...
        if (w->transactionType == DATA_WRITE && WSRAM_MAP_EN) {
            ...
            for (unsigned i = 0; i < need_size; i++) {
                ...
                if (DEBUG_BUS) {
                    PRINT_SPD_PFQ(ch, "{:10} -- Wcmd released, wsram_slt set, slt_idx={}, task={}, channel={}, wdda={}",
                        now(),
                        slot_idx,
                        wsram_slt[slot_idx].task,
                        wsram_slt[slot_idx].channel,
                        wsram_slt[slot_idx].wdda);
                }
            }
        }
    ...
    if (DEBUG_BUS) {
        PRINT_SPD_PFQ(ch, "{:10} -- PERF RELEASE W :: task={} rel_wcnt={}", now(), task, rel_wcmd_cnt);
    }
    return;
}
...
bool PFQ:: perf_conflict_intf(uint64_t task)
{
    ...
    for (auto &r : PerfQue) {
        ...
        exist_task = true;
        if (now() < r->conflict_state->conf_time)
            continue;
        if (r->conflict_state->perf.ad_conf_cnt == 0)
            continue;
    ...
    if (ptc_rd_cancel) {
        if (DEBUG_BUS) {
            PRINT_SPD_PFQ(ch, "{:10} -- Rcmd Ready For Cancel :: task={} conf_time={} ad_cc={}",
                now(),
                task,
                fr_cancel_cmd->conflict_state->conf_time,
                fr_cancel_cmd->conflict_state->perf.ad_conf_cnt);
        }
    }
...
bool PFQ::returnReadDataResp(unsigned int channel_num, unsigned long long task, double readDataEnterDmcTime,
    double reqAddToDmcTime, double reqEnterDmcBufTime)
{
    if (mpfq_->ReturnReadData != NULL) {
        return (*mpfq_->ReturnReadData)(channel_num, task, readDataEnterDmcTime, reqAddToDmcTime, reqEnterDmcBufTime);
    } else {
        return false;
    }
}
...
bool PFQ:: sidGroupPass(Transaction *trans) const
{
    if (!IS_HBM3 || !SIMPLE_GRP_SID_EN || trans == nullptr || trans->transactionType != DATA_READ)
        return true;
    if (trans->timeout || trans->dummy_timeout || trans->hqos_timeout || trans->dummy_hqos)
        return true;
    if (trans->rank >= sid_group_target.size())
        return true;
    unsigned target_sid = sid_group_target[trans->rank];
    return target_sid >= NUM_SIDS || trans->sid == target_sid;
}

bool PFQ:: preferSameSidRead(Transaction *candidate, Transaction *current) const
{
    if (!IS_HBM3 || !PREFER_SAME_SID_READ || candidate == nullptr || current == nullptr)
        return false;
    if (candidate->transactionType != DATA_READ || current->transactionType != DATA_READ)
        return false;
    if (candidate->timeout || current->timeout || candidate->channel != current->channel)
        return false;
    PTC *ptc = getPTC(candidate->channel);
    if (ptc == nullptr || ptc->PreCmd.type != READ_CMD)
        return false;
    if (candidate->rank != ptc->PreCmd.rank || current->rank != ptc->PreCmd.rank)
        return false;
    bool candidate_same_sid = candidate->sid == ptc->PreCmd.sid;
    bool current_same_sid = current->sid == ptc->PreCmd.sid;
    if (candidate_same_sid == current_same_sid)
        return false;
    if (candidate_same_sid && MAX_SAME_SID_READ > 0 && ptc->ser_sid_cnt >= MAX_SAME_SID_READ)
        return false;
...
bool PFQ:: preferSameSidRead(arb_cmd *candidate, arb_cmd *current) const
{
    if (!IS_HBM3 || !PREFER_SAME_SID_READ || candidate == nullptr || current == nullptr)
        return false;
    if (candidate->type != DATA_READ || current->type != DATA_READ || candidate->channel != current->channel)
        return false;
    PTC *ptc = getPTC(candidate->channel);
    if (ptc == nullptr || ptc->PreCmd.type != READ_CMD)
        return false;
...
{
    if (!IS_HBM3 || !LIMIT_SAME_SID_READ || MAX_CONTINUOUS_SAME_SID_READ == 0 ||
        candidate == nullptr || current == nullptr)
        return false;
    if (candidate->transactionType != DATA_READ || current->transactionType != DATA_READ)
        return false;
    if (candidate->timeout || current->timeout || candidate->channel != current->channel)
        return false;
    PTC *ptc = getPTC(candidate->channel);
    if (ptc == nullptr || ptc->PreCmd.type != READ_CMD || ptc->ser_sid_cnt < MAX_CONTINUOUS_SAME_SID_READ)
        return false;
    if (candidate->rank != ptc->PreCmd.rank || current->rank != ptc->PreCmd.rank)
        return false;
    return candidate->sid != ptc->PreCmd.sid && current->sid == ptc->PreCmd.sid;
}
...
void PFQ:: updateSidGroupState()
{
    if (!IS_HBM3 || !SIMPLE_GRP_SID_EN || NUM_SIDS == 0)
        return;
    std:: vector<std:: vector<unsigned>> pending(NUM_RANKS, std:: vector<unsigned>(NUM_SIDS, 0));
    for (Transaction *trans : PerfQue) {
        if (trans == nullptr || trans->in_ptc || trans->transactionType != DATA_READ)
            continue;
        if (trans->rank < NUM_RANKS && trans->sid < NUM_SIDS)
            pending[trans->rank][trans->sid]++;
    }
    for (unsigned rank = 0; rank < NUM_RANKS; rank++) {
        unsigned total = 0;
        unsigned max_sid = 0;
        for (unsigned sid = 0; sid < NUM_SIDS; sid++) {
            total += pending[rank][sid];
            if (pending[rank][sid] > pending[rank][max_sid])
                max_sid = sid;
            if (pending[rank][sid] == 0)
                sid_group_timeout[rank][sid] = 0;
            else if (sid_group_timeout[rank][sid] != UINT_MAX)
                sid_group_timeout[rank][sid]++;
        }
        unsigned target_sid = sid_group_target[rank];
        unsigned next_target = target_sid;
        if (target_sid >= NUM_SIDS) {
            if (total >= ENGRP_LR_LEVEL)
                next_target = max_sid;
        } else if (total < EX_LR_LEVEL) {
            next_target = NUM_SIDS;
        } else {
            unsigned timeout_sid = NUM_SIDS;
            if (TIMEOUT_SID_ENABLE) {
                for (unsigned sid = 0; sid < NUM_SIDS; sid++) {
                    if (sid != target_sid && pending[rank][sid] > 0 && sid_group_timeout[rank][sid] >= TIMEOUT_SID) {
                        timeout_sid = sid;
                        break;
                    }
                }
            }
            bool other_sid_high = false;
            for (unsigned sid = 0; sid < NUM_SIDS; sid++) {
                if (sid != target_sid && pending[rank][sid] >= LR_LEVELH) {
                    other_sid_high = true;
                    break;
                }
            }
            bool normal_switch = sid_group_serial_count[rank] >= SERIAL_LR_LEVELH ||
                (sid_group_serial_count[rank] >= SERIAL_LR_LEVELL &&
                 (pending[rank][target_sid] <= LR_LEVELL || other_sid_high)) || pending[rank][target_sid] == 0;
            if (timeout_sid < NUM_SIDS)
                next_target = timeout_sid;
            else if (normal_switch)
                next_target = max_sid;
        }
        if (next_target != target_sid) {
            if (sid_group_next_target[rank] != next_target) {
                sid_group_next_target[rank] = next_target;
                sid_group_switch_count[rank] = 1;
            } else {
                sid_group_switch_count[rank]++;
            }
            unsigned switch_delay = std:: max(1U, std:: max(SID_GRP_PIPE, SERIAL_PRE_SIDGRP));
            if (sid_group_switch_count[rank] >= switch_delay) {
                sid_group_target[rank] = next_target;
                sid_group_serial_count[rank] = 0;
                sid_group_switch_count[rank] = 0;
            }
        } else {
            sid_group_next_target[rank] = target_sid;
            sid_group_switch_count[rank] = 0;
        }
    }
}
