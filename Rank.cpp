...
SvZMzBtMrQ+w1lxbG5+EFWux51V2bvtZUTAAA+en/pM7ZB5Cy3u0JTs1VqxXwgSWbbmX47mp4oJ1
3YAA79H5qU7b3HKoja1yfxarTk6DD6yJ7EevwRVR3vBViJ4IhwgG69YBkCMMJzYruOoZCQi2M8RN
5BBIb8oTJwik9Wzr# */
/* $$$!!Warning: Deleting or modifying the preceding information is prohibited.$$$ */
/*
 * Copyright @ Huawei Technologies Co., Ltd. 2019-2029. All rights reserved.
 * Description: Rank.cpp
 * Author     : l00434636
 * Create     : 2020-10-27
 */
...
Rank::Rank(unsigned index, unsigned index_in_ptc, PTC *ptc, ofstream &DDRSim_log_, ofstream &dram_log_)
    : id_(index), index_in_ptc_(index_in_ptc), ptc_(ptc), id(-1), DDRSim_log(DDRSim_log_), dram_log(dram_log_)
{
    // ptc = NULL;
...
void Rank::rdata_return_pipe(const BusPacket &packet)
{
    ...
    switch (packet.type) {
        ...
        case READ_P_CMD: {
            ...
            for (size_t i = 1; i <= data_ratio; i++) {
                if ((IS_LP5 || IS_LP6 || IS_GD2) && NUM_GROUPS != 1 &&
                    unsigned((cycle_min * i) * WCK2DFI_RATIO) > BasicBL) {
    ...
    if (PRINT_DRAM_TRACE && channel_ohot == (channel_ohot & PRINT_CH_OHOT)) {
        ...
        switch (packet.type) {
            ...
            case PRECHARGE_PB_CMD: {
                if (packet.fg_ref) {
                    DRAM_PRINT(cur_time << ",PREpbfg," << bank);
                } else {
                    DRAM_PRINT(cur_time << ",PREpb," << bank);
                }
                break;
            }
...
void Rank::update_bank_state()
{  // todo: revise for e-mode
    switch (RankStat[0].state) {
        case RANK_IDLE: {
            bool is_refab = true;
            for (size_t i = 0; i < NUM_BANKS; i++) {
                if (DramBankState[i].state == BANK_REF)
                    continue;
                is_refab = false;
                break;
            }
            ...
            for (size_t i = 0; i < NUM_BANKS; i++) {
                if (DramBankState[i].state == BANK_IDLE)
                    continue;
                all_bank_pre = false;
                RankStat[0].act_cycle_bank++;
            }
            if (all_bank_pre)
                RankStat[0].pre_standby_cycle++;
            else
                RankStat[0].act_standby_cycle++;
            break;
        }
        case RANK_PD: {
    ...
    for (size_t i = 0; i < NUM_BANKS; i++) {
        if (now() < DramBankState[i].state_change_time)
            continue;
        if (DramBankState[i].state == BANK_IDLE)
            continue;
...
void Rank::CalcDramPower()
{
    ...
    DramPower[0].IddEnergy["IDD5"] =
        DramPower[0].refpb_energy + DramPower[0].refab_energy + DramPower[0].asref_refab_energy;
    DramPower[0].IddEnergy["IDD6"] = DramPower[0].srpd_energy;
    double IddTotalEnergy = 0;
    for (auto idd : DramPower[0].IddEnergy)
        IddTotalEnergy += idd.second;
    ...
    // Energy efficiency
    DramPower[0].energy_efficiency =
        (RankStat[0].total_byte == 0) ? 0 : DramPower[0].total_energy / 8 / (double)RankStat[0].total_byte;
    ...
    if (DEBUG_BUS) {
        PRINTN("-------- Rank" << ptc_->getID() % NUM_RANKS << " Print Power PMU Event And Power Consumption(pJ)"
                               << " --------" << endl);
        PRINTN("Active opertion -- Number: " << RankStat[0].act_num << ", Cycle: " << act_op_cycle
                                             << ", Energy: " << DramPower[0].act_energy << endl);
        ...
        PRINTN("Precharge Standby -- Cycle: " << pre_standby_cycle << ", Energy: " << DramPower[0].pre_standby_energy
                                              << endl);
        PRINTN("Perbank Refresh -- Number: " << RankStat[0].refpb_num << ", Cycle: " << refpb_cycle
                                             << ", Energy: " << DramPower[0].refpb_energy << endl);
        PRINTN("Allbank Refresh -- Number: " << RankStat[0].refab_num << ", Cycle: " << refab_cycle
                                             << ", Energy: " << DramPower[0].refab_energy << endl);
        ...
        PRINTN("Auto Self Refresh -- Number: " << RankStat[0].sre_num << ", Ref_cycle: "
                                               << ", Asref_ycle: " << asref_cycle
                                               << ", Ref_energy: " << DramPower[0].asref_refab_energy
                                               << ", Idle_energy: " << DramPower[0].asref_pre_energy
                                               << ", Asref_energy: " << DramPower[0].asref_energy << endl);
        PRINTN("Power Down -- Act_pd_cycle: " << act_pd_cycle << ", Idle_pd_cycle: " << idle_pd_cycle
                                              << ", Srpd_cycle: " << srpd_cycle
                                              << ", Act_pd_energy: " << DramPower[0].act_pd_energy
                                              << ", Idle_pd_energy: " << DramPower[0].idle_pd_energy
                                              << ", Srpd_energy: " << DramPower[0].srpd_energy << endl);
        PRINTN("VDD Energy -- VDD1: " << DramPower[0].VddEnergy["VDD1"]
                                      << ", VDD2H: " << DramPower[0].VddEnergy["VDD2H"]
                                      << ", VDD2L: " << DramPower[0].VddEnergy["VDD2L"]
                                      << ", VDDQH: " << DramPower[0].VddEnergy["VDDQH"]
                                      << ", VDDQL: " << DramPower[0].VddEnergy["VDDQL"] << endl);
        PRINTN("IDD Energy -- IDD0: " << DramPower[0].IddEnergy["IDD0"]
                                      << ", IDD2N: " << DramPower[0].IddEnergy["IDD2N"]
                                      << ", IDD2P: " << DramPower[0].IddEnergy["IDD2P"]
                                      << ", IDD3N: " << DramPower[0].IddEnergy["IDD3N"]
                                      << " IDD3P: " << DramPower[0].IddEnergy["IDD3P"] << endl);
        PRINTN("IDD Energy -- IDD4W: " << DramPower[0].IddEnergy["IDD4W"]
                                       << ", IDD4R: " << DramPower[0].IddEnergy["IDD4R"] << ", IDD5: "
                                       << DramPower[0].IddEnergy["IDD5"] << ", IDD6: " << DramPower[0].IddEnergy["IDD6"]
                                       << ", IddTotalEnergy: " << IddTotalEnergy << endl);
        PRINTN("Averge Current -- VDD1: "
               << DramPower[0].AvgCurrent["VDD1"] << ", VDD2H: " << DramPower[0].AvgCurrent["VDD2H"]
               << ", VDD2L: " << DramPower[0].AvgCurrent["VDD2L"] << ", VDDQH: " << DramPower[0].AvgCurrent["VDDQH"]
               << ", VDDQL: " << DramPower[0].AvgCurrent["VDDQL"] << endl);
        PRINTN("Total Energy -- Energy: " << DramPower[0].total_energy
                                          << "(pJ), Effi: " << DramPower[0].energy_efficiency
